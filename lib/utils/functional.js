...
// `memoize`, except a result the caller cannot yet stand behind is returned
// without being stored, so the next call gets a chance to do better.
//
// `pageType()` is why this exists. On current Reddit before `shreddit-app` has
// been parsed there is nothing to go on but the path, and the path is wrong for
// any route the regex table has no pattern for. Caching that guess pins it for
// the life of the page — which is how a share link ends up classed as a listing
// and every comments-scoped module sits it out.
export function memoizeUnsettled<A, R>(fn: (a: A) => R, settled: () => boolean): (a: A) => R {
	const cache = new Map();
	function memoized(arg: A): R {
		if (cache.has(arg)) return (cache.get(arg): any);
		const result = Reflect.apply(fn, this, [arg]);
		if (settled()) cache.set(arg, result);
		return result;
	}
	memoized.cache = cache;
	return memoized;
...
// The lodash signature, because both call sites were already written against
// it: `throttle(fn, 100, { leading: true, trailing: false })`. The third argument
// used to be ignored, so a call site asking for the leading edge only also got a
// trailing call - the opposite of what it asked for. Defaults are lodash's, so
// the behaviour of a two-argument call is unchanged.
export function throttle<F:(...args: any[]) => any>(fn: F, wait: number, { leading = true, trailing = true }: { leading?: boolean, trailing?: boolean } = {}): F {
	let last = leading ? 0 : Date.now();
	let timer;

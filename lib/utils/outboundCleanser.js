...
// Like cleanseUrl, but always returns a usable absolute URL (normalised, tracking
// params stripped) rather than null when there was nothing to strip. Used by the
// "copy clean link" affordance. Returns null only for an unparseable href.
export function toCleanLink(rawHref: string, base: string = 'https://old.reddit.com/'): string | null {
	if (!rawHref) return null;
	const cleaned = cleanseUrl(rawHref, base);

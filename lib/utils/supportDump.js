...
export function formatSupportDump(input: SupportDumpInput): string {
	...
	// see and the maintainer cannot. Nothing here is content: a fullname, a rule
	// id the reader wrote, a host.
	...
	if (actions === undefined || actions === null) {
		...
		const shown = actions.slice(0, ACTION_LIMIT);
		lines.push(...section(
			`Page activity (${shown.length} of ${actions.length})`,
			shown.map(entry => {

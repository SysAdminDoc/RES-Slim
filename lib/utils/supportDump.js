...
import { summariseActionLog } from './actionLog';
...
export type SupportDumpInput = {|
	...
	drift: DriftState,
	// What the extension did to the page, if there was a page to ask. Optional so
	// a caller that has not gathered it produces the same report it always did.
	pageActions?: ?$ReadOnlyArray<ActionEntry>,
	// How many the page had recorded, which is not how many were sent: the ask is
	// capped well below the ring. Optional; falls back to what arrived.
	pageActionsTotal?: ?number,
|};
...
export function formatSupportDump(input: SupportDumpInput): string {
	...
	// What happened on the page, which is the half of a bug report the reader can
	// see and the maintainer cannot. Counts, not rows: the paragraph above this
	// section promises that anything which could carry something private is
	// described rather than printed, and a reddit fullname does not stop being
	// content because it is an id -- one public `api/info` call turns it back
	// into the post. The settings panel shows the rows, in the reader's browser.
	const actions = input.pageActions;
	if (actions === undefined || actions === null) {
		lines.push('', 'Page activity: unavailable (open the settings console from a reddit page)');
	} else if (!actions.length) {
		lines.push('', 'Page activity: nothing recorded');
	} else {
		const total = Number.isFinite(input.pageActionsTotal) ?
			Math.max(actions.length, Math.floor((input.pageActionsTotal: any))) :
			actions.length;
		const heading = total > actions.length ?
			`Page activity (${total} recorded, most recent ${actions.length} counted)` :
			`Page activity (${total} recorded)`;
		lines.push(...section(heading, summariseActionLog(actions)));
	}

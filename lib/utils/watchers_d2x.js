...
export function initD2xWatcher() {
	...
	const register = (root: ParentNode) => {
		...
		// One subtree scan for what arrived, not one per thing inside it.
		//
		...
		// prepared element therefore registered every comment once per ancestor it
		// had: a five-hundred comment thread at average depth eight issued about
		// four thousand `registerThing` calls -- each a `Thing.checkedFrom`, a
		// `getFullname` and a `dupeSet` round trip -- to register five hundred
		// things. The watchers themselves were never confused by it, because each
		// is guarded by its own WeakSet; the work was simply done and thrown away.
		//
		// The owner is scanned separately because it is an *ancestor* of the root, so
		// the root's own scan cannot reach the rest of it.
		if (root instanceof HTMLElement) registerPage(root);
		else if (document.body) registerPage(document.body);
		if (owner instanceof HTMLElement) registerPage(owner);

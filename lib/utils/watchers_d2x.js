...
import { registerElement, registerThings } from './watchers';
...
export function initD2xWatcher() {
	...
	const register = (root: ParentNode) => {
		...
		if (root instanceof HTMLElement) {
			owner = root.closest(SHREDDIT_THING_SELECTOR);
			if (owner instanceof HTMLElement && !elements.includes(owner)) {
				// The full pass only when slotted content actually arrived. Reddit
				// streams children into an existing post throughout hydration, on a
				// vote, and on an expando change; re-decorating for a node with no
				// slot in it re-runs nine selectors and the shadow-part exposure for
				// nothing. A node that brings a slot genuinely needs it — that is the
				// title, credit bar and flair appearing.
				const broughtSlots = root.matches('[slot]') || !!root.querySelector('[slot]');
				if (broughtSlots) prepareShredditThing(owner);
				else refreshShredditThing(owner);
				elements.push(owner);
			} else {
				owner = null;
			}
		...
		// One Thing scan for what arrived, not one per thing inside it.
		//
		// `registerPage` scans the whole subtree of whatever it is handed, and a
		// `shreddit-comment` nests its replies as descendants. Calling it once per
		// prepared element registered every comment once for each ancestor above it:
		// a five-hundred comment thread at average depth eight issued about four
		// thousand registrations -- each a `Thing.checkedFrom`, a `getFullname` and a
		// `dupeSet` round trip -- to register five hundred things.
		//
		// The owner covers the root when there is one, because it is an *ancestor*
		// of it; scanning both would register everything under the root twice, which
		// is the bug this is fixing in miniature.
		// Nothing to register is nothing to scan for. `prepareShredditTree` has just
		// walked this subtree and found no shreddit element, and the only things
		// `Thing.thingSelector` adds are old-Reddit shapes that do not occur here --
		// so a thing-free streamed node, which is the common shape of churn on this
		// renderer, costs one query rather than two.
		const scanRoot = owner instanceof HTMLElement ? owner : root;
		if (elements.length && scanRoot instanceof HTMLElement) registerThings(scanRoot);

		// The element watchers keep the scope they had: once per Thing, not once
		// over the root. Handing them the root instead swept the whole document for
		// ten selectors on every streamed node, which on a renderer whose common
		// shape of churn is thing-free nodes cost more than the scan above saves.
	...
	observer.observe(document.documentElement, {
		...
		subtree: true,
		attributes: true,
		attributeFilter: ['score', 'comment-count', 'open', 'collapsed', 'author', 'subreddit-name', 'domain'],
	});

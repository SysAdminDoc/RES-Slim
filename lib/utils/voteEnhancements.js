...
/**
 * The score colour, moved only in lightness until it is readable on `ground`.
 *
 ...
 * accent. A colour that already clears the floor is returned untouched, and one
 * that cannot be read (or a ground that cannot) is returned as it came, because
 * refusing to colour at all would be a bigger change than the reader asked for.
 */
export function readableScoreText(color: mixed, ground: ?[number, number, number]): string {
	if (typeof color !== 'string') return '';
	const rgb = scoreColorRgb(color);
	if (!rgb || !ground) return color;
	// Bound to a const so the refinement survives into the closure below.
	const behind: [number, number, number] = ground;
	if (contrastRatio(rgb, behind) >= TEXT_CONTRAST_TARGET) return color;

	const [h, s, lightness] = rgbToHsl(rgb);
	const found = lightnessesAround(lightness)
		.map(l => hslToRgb(h, s, l))
		.find(candidate => contrastRatio(candidate, behind) >= TEXT_CONTRAST_TARGET);
	// Every hue has a readable lightness at both ends of the range, so `found` is
	// only ever missing for a ground that is itself mid-grey enough to beat both
	// -- and there the honest answer is the colour the reader configured.

...
// Records are keyed `<renderer>:<pageType>`, because `comments` exists on both
// renderers and one would otherwise overwrite the other. That key is storage,
// not something to put in front of a reader, so both display paths go through
// here. Older records predate the prefix and are shown as they are.
const DRIFT_RENDERER_NAMES = { r2: 'Old Reddit', d2x: 'Current Reddit' };

export function describeRenderer(app: ?string): ?string {
	return app ? DRIFT_RENDERER_NAMES[app] : null;
}

export function describeDriftScope(key: string): string {
	const [renderer, ...rest] = key.split(':');
	const name = describeRenderer(renderer);
	if (!name || !rest.length) return key;
...
export function formatDriftReport(state: DriftState, version: string = ''): string {
	...
	for (const record of records) {
		lines.push(`${describeDriftScope(record.pageType)}, first seen ${isoDay(record.firstSeen)}, last seen ${isoDay(record.lastSeen)}`);

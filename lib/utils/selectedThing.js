...
// The subtree a selection belongs to. `.sitetable` was the whole list, on a
// helper that runs on both renderers, so on current Reddit `currentContainer`
// was always null and the guard it exists for was a no-op there: a comment
// streamed into one tree could take the selection from another.
const CONTAINER_SELECTOR = '.sitetable, shreddit-comment-tree, shreddit-feed';

function containerOf(element: ?Element): ?Element {
	return element ? element.closest(CONTAINER_SELECTOR) : null;
}
...
export function set(_new: Thing, options: SelectOptions = { scrollStyle: 'none' }, force: boolean = false) {
	...
	previous = current;
	current = _new;
	currentContainer = containerOf(current.element);
...
watchForThings(['comment'], throttle(thing => {
	...
	if (currentContainer && currentContainer !== containerOf(thing.element)) return;
	set(thing);
}, 100, { leading: true, trailing: false }), { immediate: true });

// Read at call time, not once at import. Current Reddit navigates without a
// document load, so a key interpolated when the module first ran belonged to
// whichever route the reader happened to open first: every later route wrote its
// selection under that route's key and read that route's remembered id back.
...
// Nothing released the selection between routes. On current Reddit a listing to
// post to listing trip is one document, so `current` still pointed at the first
// route's Thing: the watcher above returns early while `current` is set, so the
// new route never got a selection at all, and keyboard navigation and the
// comment navigator had no anchor. The three module-level references also
// pinned the old route's detached subtree for the rest of the session.
export function releaseSelection(): void {
	if (current) {
		current.entry.classList.remove('res-selected');
		current.element.classList.remove('res-selected');
	}
	current = null;
	previous = null;
...
document.addEventListener('reddit.urlChanged', releaseSelection);

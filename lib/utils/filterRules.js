...
// `*`, `+` and any `{…}` repeat the group enough times to backtrack; a lone `?`
// cannot, so `(a+)?` stays allowed.
function groupIsRepeated(value: string, index: number): boolean {
	const ch = value[index];
	if (ch === '*' || ch === '+') return true;
	if (ch === '{') return (/^\{\d*(?:,\d*)?\}/).test(value.slice(index));
	return false;
}

// Whether a group's body can match the same input more than one way, which is
// what makes repeating it exponential. A quantifier, a brace repeat, or an
// alternation all do it; the same characters inside a character class do not.
function bodyIsAmbiguous(body: string): boolean {
	...
	while (i < body.length) {
		...
		if (ch === '{' && (/^\{\d*(?:,\d*)?\}/).test(body.slice(at))) return true;
	}
	return false;
}
...
export function isLikelyCatastrophicRegex(value: mixed): boolean {
	...
	while (i < pattern.length) {
		...
		if (!groupIsRepeated(pattern, at + 1)) continue;
		if (bodyIsAmbiguous(pattern.slice(start + 1, at))) return true;
	}

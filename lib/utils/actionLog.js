...
export function summariseActionLog(rows: $ReadOnlyArray<ActionEntry>): string[] {
	// Keyed on the pair, not on a joined string that has to be taken apart again.
	// A module id is a value from the page as far as this function is concerned,
	// and one containing a colon would have split in the wrong place.
	const counts: Map<string, {| moduleID: string, outcome: string, count: number |}> = new Map();
	const reasons: Map<string, Set<string>> = new Map();

	for (const row of rows) {
		const key = JSON.stringify([row.moduleID, row.outcome]);
		const entry = counts.get(key) || { moduleID: row.moduleID, outcome: row.outcome, count: 0 };
		entry.count += 1;
	...
	return [...counts.values()]
		...
		.map(({ moduleID, outcome, count }) => {
			const distinct = (reasons.get(moduleID) || new Set()).size;
			const times = `${count} time${count === 1 ? '' : 's'}`;
			const head = `${moduleID}: ${outcome} ${times}`;
			if (!distinct) return head;
			return `${head} across ${distinct} rule${distinct === 1 ? '' : 's'} or host${distinct === 1 ? '' : 's'}`;
		});
}

...
export function isRedditOrigin(origin: mixed): boolean {
	...
	// path or a query is not one, and accepting it is how a hostile value that
	// merely *contains* a reddit hostname in its query string gets through a
	// sloppier check. (Written without a literal example URL on purpose: the
	// privacy contract snapshots every http(s) string under lib/, and a made-up
	// attacker host in a comment reads as a real outbound endpoint there.)

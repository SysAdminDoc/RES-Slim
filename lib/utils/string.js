...
// `fast-levenshtein` was a wrapper that delegated to this package, so the tree
// already shipped both. Its `useCollator` option is the only thing it added, and
// nothing here used it.
import { distance as levenshteinDistance } from 'fastest-levenshtein';
import { setTrustedHTML } from '../core/dom/trustedHtml';
import { zipWith } from './functional';
import { downcast } from './flow.js';
import { escapeHTML } from './html.js';
import type { Iteratee } from './array.js';
...
export function areSimilar(a: string, b: string): boolean {
	return levenshteinDistance(a, b) <= MAX_DISTANCE_RATIO * Math.max(a.length, b.length);
}
...
export const html = (s: string[], ...values: HtmlTagValues[]) => {
	...
	setTrustedHTML(template, markup);

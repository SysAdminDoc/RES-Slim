...
function setupItem(item: Element, handleSelector: string, container: HTMLElement) {
	...
	// therefore unchecked.
	const el = item;
	// Set up once per element, marked on the element itself.
	//
	// The guard used to be `if (el.draggable) return`, which only holds while a
	// drag is in progress: the flag is set on `mousedown` and cleared on `dragend`.
	// That was survivable when every listener here was idempotent. It stopped
	// being survivable when the keyboard path arrived, because moving a row is a
	// remove and an insert, the `childList` observer sees the insert, and a second
	// `keydown` closure was registered every time. Holding ArrowDown then walked
	// the row 1, 2, 4, 8 rows at a time and swept the whole options panel once per
	// duplicated listener.
	if (el.dataset.rsmSortableItem) return;
	el.dataset.rsmSortableItem = 'true';
	...
	if (!handleEl) {
		...
		handleEl.addEventListener('keydown', (e: KeyboardEvent) => {
			...
			// A modified arrow belongs to the browser or the platform. Taking
			// Ctrl+Down or Shift+Up here would swallow a shortcut to move a row,

...
// Control characters go, and a newline is the one that matters.
//
// These strings are printed into the plaintext support report, which is joined
// on newlines and pasted into a bug tracker by a person. A `moduleID` carrying
// `\n` therefore writes whole sections of that report -- a convincing "Stored
// options" block naming a password, say -- from a single `postMessage` by
// page-world script on reddit.com, which is exactly the sender this function
// exists to distrust. Bounding the type and the length is not enough when the
// output format is line-oriented.

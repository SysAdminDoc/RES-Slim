...
// Four files carried a private copy of this: commentTreeExport, fencedCode,
// textDiff and subRules, all of them assembling markup as a string. Three were
// character-identical; the subRules one had drifted, with no rule for the
// apostrophe.
//
// Not `escapeHTML` above, which is a different function: it also escapes `/` and
// spells the apostrophe `&apos;`. Both are correct, and the difference is
// visible in the output these four produce and their tests assert, so the two
// stay separate rather than one quietly changing what the other emits.
const textEscapeLookups = {
	'&': '&amp;',
	'"': '&quot;',
	"'": '&#39;', // eslint-disable-line quotes
	'<': '&lt;',
...
export function escapeHtmlText(value: mixed): string {
	return String(value == null ? '' : value).replace(/[&"'<>]/g, m => textEscapeLookups[m]);
}

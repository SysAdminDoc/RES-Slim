...
export const currentMultireddit = memoize((): string | void => {
	...
	// Canonical, because this value is a key and a thing to be parsed rather than
	// only compared: reddit serves the same multireddit under `/u/bob/m/x` and
	// `/user/bob/m/x`, and a caller that splits it on an optional `user/` prefix
	// reads the owner of the short form as `u`. `fullLocation` normalises the same

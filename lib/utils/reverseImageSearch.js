...
// The expando's own "search this image" control needs the same lookup without
// the module's engine picker, so it reads the one builder rather than spelling a
// second Google URL that can rot independently.
export function lensSearchUrl(imageUrl: string): string {
	const lens = ENGINES.find(engine => engine.id === 'lens');
	if (!lens) throw new Error('the lens engine is missing from ENGINES');
	return lens.build(imageUrl);
}

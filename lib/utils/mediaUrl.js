...
// The same boundary for a plain link or image built outside `showImages`.
//
// Four modules take a URL straight out of reddit's own JSON and put it in an
// `href` or an `src` without passing through `assertSafeMediaUrls`: the search
// gallery's thumbnails, the crosspost list's permalinks, the saved-content
// manager's titles and the processing-image replacement. `new URL(value, base)`
// does not constrain the scheme - an absolute `javascript:` value wins over the
// base - so the only thing standing between reddit's JSON and a click-to-run
// script was reddit's JSON. Data is reddit-controlled rather than
// user-controlled, so this is depth, not a live hole.
//
// Stricter than `isSafeMediaUrl`: a link is navigated, so `blob:` and
// `data:image/` have no business here.
export function isSafeLinkUrl(value: mixed, base: string): boolean {

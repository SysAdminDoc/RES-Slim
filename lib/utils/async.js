...
import { pull, debounce, once, curryRight, remove } from './functional';
...
export function batch<T, V>(callback: (xs: T[]) => Promise<Iterable<Error | Promise<V> | V> | void> | void, { size = 100, delay = 50, flushBeforeUnload = false }: {| size?: number, delay?: number, flushBeforeUnload?: boolean |} = {}): (x: T) => Promise<V> {
	...
	function* batchAccumulator() {
		...
		const timeout = delay ? debounce(invoke, delay) : microtaskThrottle(invoke);
...
//
// Named for what it does, because `functional.js` exports a `throttle` too and
// that one is a time throttle taking `(fn, wait)`. Two call sites had already
// imported the wrong one's signature.
export function microtaskThrottle(callback: () => void | Promise<void>): () => Promise<void> {
	let promise: ?Promise<*>;

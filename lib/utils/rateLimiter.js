...
|}): RateLimiter {
	...
	function drain() {
		while (queue.length && available > 0 && active < maxConcurrent) {
			...
			available -= 1;
			active += 1;
			// Wrap in Promise.resolve so a job that throws *synchronously* still
			// releases its slot and rejects its promise — otherwise `active` would
			// never decrement and the limiter would wedge below maxConcurrent.

...
export function desiredThemeClasses(opts: ThemeOptions): string[] {
	...
	if (opts.refinedLayout) classes.push('res-pageTheme--refined');
...
// The user's own hue, lightened just far enough to clear the target. Every
// shipped palette is dark, so lightening is the direction that helps; a colour
// that cannot get there even at full lightness returns null rather than a shade
// that still fails.
export function nearestReadableAccent(accent: mixed, theme: mixed, target: number = TEXT_CONTRAST_TARGET): string | null {
	...
	// Whole-percent lightness steps: the first one that clears the target is the
	// closest shade to what the user picked that does.

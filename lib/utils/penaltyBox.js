...
export function formatSuspensionMessage(host: string, until: number, now: number, strikes: number): string {
	return `${host} failed repeatedly and will be skipped for ${formatBackoff(Math.max(0, until - now))} (consecutive backoff #${strikes}). It recovers on its own, so no action is needed.`;
}

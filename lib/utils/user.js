...
export const loggedInUserInfo = once((): Promise<CurrentRedditUser | void> =>
	!isLoggedIn() ? Promise.resolve() : ajax({ url: '/api/me.json', type: 'json' })
		...
		// A failed lookup is the same outcome as a logged-out one: we do not know
		// the modhash. Resolving to undefined rather than rejecting keeps that a
		// single case for every consumer — the return type already allows it.
		//
		// This used to reject, and the one caller (context.js) fires it without
		// awaiting, so any offline or blocked request surfaced an uncaught
		// "TypeError: Failed to fetch" in the page console on every load.
		// Consumers that genuinely need a modhash, such as markAllRead, already
		// fail loudly when it is missing.
		.catch(e => {
			console.error('RES-Slim: could not read the logged-in user info', e); // eslint-disable-line no-console
			return undefined;
		}));

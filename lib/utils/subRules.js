...
export function formatRulesHtml(rules: SubRule[], subreddit: string): string {
	...
	const items = rules.map((r, i) => {
		const desc = r.description ? `<div class="rsm-subrules-desc">${escapeHtmlText(r.description)}</div>` : '';
		return `<li class="rsm-subrules-item"><strong>${i + 1}. ${escapeHtmlText(r.short_name)}</strong>${desc}</li>`;
	}).join('');
	return `<div class="rsm-subrules-title">r/${escapeHtmlText(subreddit)} rules</div><ul class="rsm-subrules-list">${items}</ul>`;
}

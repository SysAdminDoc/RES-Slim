...
function drawBuilderItem(data: *, customOptionsFields: * = [], cases: *) {
	...
	const editButton = drawIconButton('builderControls builderTrailingControls', '&#xF061;', {
		label: 'filterBuilderShareCondition',
		hint: 'filterBuilderShareConditionHint',
	});
	editButton.addEventListener('click', async () => {
		const data = readBuilderItem(item, customOptionsFields, cases);
		const element = string.html`<div>${i18n('filterBuilderShareConditionHint')}<br><br><textarea rows="5" cols="50"></textarea></div>`;
	...
	deleteButton.addEventListener('click', () => {
		...
		item.remove();
		undo(i18n('filterBuilderRestoreItem')).then(() => { if (parent) { parent.appendChild(item); parent.dispatchEvent(new Event('change', { bubbles: true })); } });
	});
	...
	const noteTextarea = createElement('textarea', { name: 'builderNote', rows: '1', cols: '40', placeholder: i18n('filterBuilderNotePlaceholder') });
...
// The builder's three controls were `div`s carrying a click handler and a
// hardcoded English `title`. A div is not focusable and announces as nothing, so
// removing a condition, sharing one and reordering one were all mouse-only, and
// the only text describing them never went through `i18n`. A real button gets
// focus, the right role and Enter/Space without any ARIA; the move handle also
// picks up the arrow keys from `makeSortable`.
function drawIconButton(className: string, glyph: string, { label, hint }: {| label: string, hint?: string |}) {
	const button = createElement('button', {
		type: 'button',
		class: `res-icon-button res-icon ${className}`,
		html: glyph,
		// The name is what gets read on every row, so it stays short. The title is
		// the accessible description when there is an `aria-label`, and it is also
		// the only thing a pointer user gets on a button with no text -- so where a
		// control has nothing more to say than its name, it keeps the tooltip and
		// screen readers drop the description for repeating the name.
		title: i18n(hint || label),
	});
	button.setAttribute('aria-label', i18n(label));
	return button;
}

function drawHandle() {
	const handle = drawIconButton('handle builderControls', '&#xF0AA;', {
		label: 'filterBuilderMoveCondition',
		hint: 'filterBuilderMoveConditionHint',
	});
	// Enter and Space genuinely do nothing on this one, because it is not an
	// activation control. Naming the keys that do work is how that is said.
	handle.setAttribute('aria-keyshortcuts', 'ArrowUp ArrowDown');
	return handle;
}

function drawDeleteButton() {
	return drawIconButton('builderControls', '&#xF056;', { label: 'filterBuilderRemoveCondition' });
}
...
export function drawBuilderBlock(data: *, cases: *, addBaseControls: boolean = true) {
	...
	deleteButton.addEventListener('click', () => {
		...
		wrap.remove();
		undo(i18n('filterBuilderRestoreBlock')).then(() => { if (parent) { parent.appendChild(wrap); parent.dispatchEvent(new Event('change', { bubbles: true })); } });
	});
		...
		draw(data, field, cases = {}) {
			...
			const addCaseSelect = downcast(string.html`
				<select class="addBuilderBlock">
					<option>${i18n('filterBuilderAddCondition')}</option>

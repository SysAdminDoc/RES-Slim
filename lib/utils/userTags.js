...
export async function commitTagImport(transaction: TagImportTransaction): Promise<UserTagMap> {
	...
	if (!stored) {
		// Deliberately before anything else is touched: nothing was written, and the
		// stored tags belong to whoever did write.
		throw new TagImportConflict('The stored tags changed while this import was being committed. Preview it again.');
	}

	// The snapshot is taken *after* the write lands, not before it.
	//
	// Before, a window that lost the race still overwrote the rollback with a
	// snapshot of a map that was no longer stored -- so "Undo last import" would
	// then write that map back and silently delete the import that had won, and
	// the real previous snapshot was gone with it. An undo of an import that never
	// happened is worse than no undo.
	//
	// Failing here does not undo the write: the tags the reader asked for are
	// stored and correct, and the only thing missing is the ability to change
	// their mind. That is what the message says.
	try {
		await transaction.saveRollback(original);
	} catch (error) {
		const failure = normalizeError(error);
		failure.message = `The tags were imported, but the copy that Undo would restore could not be saved, so Undo will not put this import back. ${failure.message}`;
		throw failure;
	}

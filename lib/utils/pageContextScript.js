...
// Straight from the modules rather than through the barrel: this file is inside
// `lib/utils`, and `watchForChildren` is not re-exported any more because
// nothing outside imports it.
import { downcast } from './flow';
import { waitForEvent, watchForChildren, waitForDescendant, watchForDescendants } from './dom';
...
export function stopPageContextScript(test: HTMLScriptElement => boolean, _parent: string | HTMLElement | Promise<HTMLElement>, onlyChildrenOfParent: boolean) {
	const undo = [];
	let stopped = false;
	// Set once the watcher below is attached, so `undo()` can take the observer
	// down. It used to stay on `head` or `#siteTable` for the life of the page,
	// waking on every added node only to hit the `stopped` guard and return.
	let disconnect = null;
	...
	(async () => {
		...
		disconnect = (onlyChildrenOfParent ? watchForChildren : watchForDescendants)(parent, 'script', ele => {
			if (stopped) return;
		...
		// `undo()` can run before the parent turned up, in which case there was
		// nothing to disconnect at the time.
		if (stopped) {
			disconnect();
			disconnect = null;
		}

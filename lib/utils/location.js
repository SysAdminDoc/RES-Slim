...
// Reddit serves every profile-shaped route under `/u/` as well as `/user/`, and
// has for as long as old Reddit has existed. `comments` and `commentsLinklist`
// below already accept both; `profile`, `search` and `multireddit` did not, so
// `/u/bob` was not a profile, `/u/bob/m/x` was not a multireddit, and
// `currentUserProfile()` answered nothing on a page that plainly was one.
//
// `profileCommentsPage` used to sit here between them. It matched
// `/user/<name>/comments/<id>`, which is a post permalink -- a strictly narrower
// case of `comments`, which is listed first in both renderers and therefore
// always won. Nothing could reach it, no module scoped to it, and reddit's own
// `pagetype` calls that page `post_detail`, which this file maps to `comments`.
// Making it reachable would have taken every comments-scoped module off those
// URLs on old Reddit and made the path fallback contradict what current Reddit
// says the page is, so it is gone instead.
...
export const regexes: { [string]: RegExp } = {
	...
	inbox:            /^\/(?:r\/([\w\.]+)\/)?message(?:\/|$)/i,
	profile:          /^\/u(?:ser)?\/([\w\-]+)(?:\/(?:(?!m\/)(\w+)))?\/?$/i,
	submit:           /^\/(?:r\/([\w\.\+]+)\/)?submit(?:\/|$)/i,
	prefs:            /^\/prefs(?:\/|$)/i,
	account:          /^\/account-activity(?:\/|$)/i,
	wiki:             /^\/(?:r\/([\w\.]+)\/)?wiki(?:\/|$)/i,
	stylesheet:       /^\/(?:r\/([\w\.]+)\/)about\/stylesheet(?:\/|$)/i,
	search:           /^\/(?:r\/[\w\.\+]+\/|(?:me|u(?:ser)?\/[\w\-]+)\/[mf]\/[\w\.\+]+\/|domain\/[^\/]+\/)?search(?:\/|$)/i,
...
export const appPageTypes: { [AppType]: {| default?: PageType, pageTypes: PageType[] |} } = {
	...
	d2x: {
		default: 'linklist',
		pageTypes: ['wiki', 'search', 'modqueue', 'subredditAbout', 'comments', 'commentsLinklist', 'profile', 'inbox', 'submit'],
	},
	options: {
		pageTypes: [],
	},
...
export const fullLocation = (pathname: string = location.pathname) => {
	// One spelling. Reddit serves the same page under `/u/` and `/user/`, and this
	// value is a key: the two spellings of one multireddit produced two of them.
	pathname = pathname.replace(/^\/u\//i, '/user/');

...
import { CACHED_MESSAGES_KEY, CACHED_MESSAGES_TOKEN_KEY, STALE_CACHED_LANG_KEY } from '../../constants/localStorage';
...
export async function _loadI18n(): Promise<void> {
	...
	//
	// Keyed on the build token alone. It also required the cached reddit locale to
	// match, which threw the whole dictionary away every time a reader switched
	// reddit's language - to fetch a dictionary that is identical in every
	// language, because there is only one.
	if (localStorage.getItem(CACHED_MESSAGES_TOKEN_KEY) === buildToken) {
		try {
	...
	messages = await sendMessage('i18n');
	...
	try { // Fails if remaining localStorage space is insufficient
		...
		// Left behind by every build before this one. Removed here rather than in a
		// migration because this is the only code that ever wrote it.
		localStorage.removeItem(STALE_CACHED_LANG_KEY);
	} catch (e) {

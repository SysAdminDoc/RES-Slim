...
import { wrapFeatureBlob } from './storage';
import { canPersistFeatureData } from './privateBrowsing';

const store = wrapFeatureBlob('visitedLinks', 'RES.visitedLinks', (): number => 0);
...
export async function addURLToHistory(url: string): Promise<void> {
	...
	if (!canPersistFeatureData('visitedLinks')) return;

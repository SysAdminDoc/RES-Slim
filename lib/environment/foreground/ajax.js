...
type AjaxOptions<Ty: ResponseType | void> = {|
	...
	// Milliseconds before the request is abandoned. `0` waits forever, which no
	// caller should want and none currently asks for.
	timeout?: number,
|};

// A host that accepts the connection and never answers is indistinguishable
// from a slow one until you decide how long to wait. Fifteen seconds is longer
// than any of these endpoints takes when it is healthy and short enough that a
// reader notices the expando resolve rather than abandoning the page.
//
// Without it, `handleLink` stayed pending forever: the expando button was
// already in the page reading "Expando is not yet ready", `initialize()` never
// ran, and because nothing rejected, `linkScanner`'s catch never fired — so the
// penalty box never counted the failure and the dead host was retried on every
// page, indefinitely.
...
export async function ajax(options: AjaxOptions<*>) {
	...
	const { method, url, headers, data, type, cacheFor, sameOrigin, credentials, timeout } = params;
	...
	const response = await (sameOrigin ?
		fetch(url, {
			...
			credentials,
			body: data,
			// A signal cannot cross the message boundary, so the proxied path is
			// given the number instead and builds its own.
		...
		sendMessage('ajax', { method, url, headers, data, credentials, timeoutMs: timeout })
	);
...
function buildRequestParams({ method = 'GET', url, query = {}, headers = {}, data, type = 'text', credentials, cacheFor = 0, timeout = DEFAULT_REQUEST_TIMEOUT_MS }): {|
	method: MethodType,
	url: string,
	headers: { [string]: string },
	data: string | void,

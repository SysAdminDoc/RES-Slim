...
// Keep the newest `keep` records by `index` order and delete the rest, without
// reading any of them into this context. Resolves with how many went.
export function trimRecords(store: FeatureStoreId, index: string, keep: number): Promise<number> {
	return sendMessage('featureDb-trim', { store, index, keep });
}

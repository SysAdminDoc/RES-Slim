...
class PrefixWrapper<T> {
	...
	// Replaces the named top-level keys and leaves the rest of the stored object
	// alone, reading and writing under the same lock. `patch` deep-extends, which
	// is wrong for restoring a value that is an array: a shorter array merged over
	// a longer one keeps the extra elements.
	patchShallow(key: string, value: $Shape<T>): Promise<void> {
		return this._mutate(() => patchShallow(this._keyGen(key), (value: any)), undefined);
	}

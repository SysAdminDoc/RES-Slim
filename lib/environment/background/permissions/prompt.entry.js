...
// background watches `tabs.onUpdated` for it and closes the tab. That works
// exactly once, and only while the background is listening.
//
// If it is not -- the tab was reopened from history, or the worker was torn down
// -- the navigation reloads this page with the same parameters plus `result`,
// which reports again, which reloads again. With malformed parameters it never
// even gets as far as the button: the report happens at load, so the page spins
// on its own. A `load` wait against `prompt.html?permissions=notjson` never
// settles.
//
// A URL that already carries a result has already answered. There is nothing
// left to do but say so.
const alreadyReported = url.searchParams.has('result');
...
if (alreadyReported) {
	// Whoever asked has their answer. Saying so beats a page that looks like it is
	// still waiting for something.
	if (button instanceof HTMLButtonElement) {
		button.disabled = true;
		button.removeAttribute('aria-busy');
		button.textContent = 'Nothing left to do';
	}
	setPromptStatus('This request has already finished. You can close this tab.');

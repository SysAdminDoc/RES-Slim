...
module.description = 'Records every media download triggered from a reddit post (downloadButtons / galleryZip / cobalt / localCompanion) into a local IndexedDB manifest. Downloads from private windows are never recorded. Browseable + exportable. Nothing leaves the browser.';
...
function openDb(): Promise<any> {
	if (!canPersistFeatureData('mediaArchiveManifest')) return Promise.reject(new Error('Media archive history is unavailable in private browsing'));

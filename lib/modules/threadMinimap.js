...
export const module: Module<*> = new Module('threadMinimap');
...
function render() {
	...
	for (const c of comments) {
		...
		stripe.style.background = mode === 'score' ? scoreColor(score) : depthColor(depth);
		stripe.title = depth ? `Depth ${depth} · score ${score}` : `Top-level · score ${score}`;

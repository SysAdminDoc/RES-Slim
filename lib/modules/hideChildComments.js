...
export const module: Module<{ [string]: any }> = new Module('hideChildComments');
...
function getChildrenContainer(comment) {
	return comment.element.querySelector('div.child > .sitetable') ||
		comment.element.querySelector('div.child');
}
...
function addToggleChildrenButton(comment) {
	...
	const children = getChildrenContainer(comment);
	const menu = comment.getButtons();
	if (!children || !menu) return;

	function toggle(action) {
		if (action === 'hide') {
			// Collapsing a subtree hides whatever was playing inside it.
			hideAndSilence(children);

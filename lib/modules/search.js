...
function drawSearchResults(query, results: Array<*>) {
	...
	$resultsContainer.off('.searchResults')
		.on('click.searchResults', '.SearchRES-result-item', handleSearchResultClick)
		.on('keydown.searchResults', '.SearchRES-result-item', handleSearchResultKeydown)
		.on('keydown.searchResults', '.SearchRES-result-copybutton', handleSearchResultCopyKeydown);
...
const drawSearchResultItem = memoize(result => {
	...
	element.setAttribute('aria-label', `Open ${result.title} in settings`);
	if (result.advanced) {
...
function handleSearchResultClick(event: any) {
	...
	if (event.target && event.target.classList && event.target.classList.contains('SearchRES-result-copybutton')) {
...
function handleSearchResultKeydown(event: any) {
	if (event.key !== 'Enter' && event.key !== ' ') return;
	event.preventDefault();
	Reflect.apply(handleSearchResultClick, this, [event]);
}

function handleSearchResultCopyKeydown(event: any) {
	if (event.key !== 'Enter' && event.key !== ' ') return;
	event.preventDefault();
	event.stopPropagation();

	const parent = downcast(this.parentElement, HTMLElement);

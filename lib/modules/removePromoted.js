...
const COUNTABLE_PROMOTED_SELECTOR = PROMOTED_SELECTOR.split(', ')
	.filter(selector => selector !== 'shreddit-comment-tree-ads')
	.join(', ');
...
function hidePromoted(el: HTMLElement) {
	...
	// A promoted post is the most likely thing on the page to be autoplaying.
	hideAndSilence(el);
	// Layout wrappers are not placements, and an ad host can contain other
	// markers such as `shreddit-dynamic-ad-link`. Count the outer countable host
	// once while still hiding every marker that streams in.
	const parent = el.parentElement;
	const countableAncestor = parent && parent.closest(COUNTABLE_PROMOTED_SELECTOR);
	if (!el.matches('shreddit-comment-tree-ads') && !countableAncestor) {
		removedCount += 1;
		// Only the countable ones. A nested marker inside a placement already
		// counted would put the same removal in the log two or three times, which
		// is the same inflation the badge count exists to avoid.
		recordAction({
			moduleID: 'removePromoted',
			outcome: 'removed',
			target: el.getAttribute('data-fullname') || el.id || el.tagName.toLowerCase(),
			reason: el.tagName.toLowerCase(),
		});
	}
	refreshBadge();
}

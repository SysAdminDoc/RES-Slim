...
function hidePromoted(el: HTMLElement) {
	...
	hideAndSilence(el);
	// The plural comment-tree element is only Reddit's layout wrapper. Its
	// countable child is observed separately, so counting both inflates the
	// badge even though only one ad was removed.
	if (!el.matches('shreddit-comment-tree-ads')) removedCount += 1;
	refreshBadge();
}

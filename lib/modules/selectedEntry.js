...
import { idleThrottle } from '../utils/async';
...
export const module: Module<{ [string]: any }> = new Module('selectedEntry');
...
module.contentStart = () => {
	...
	const throttledMouseUp = throttle((target: Element) => {
		const thingEl = target.closest(Thing.thingSelector);
		if (!thingEl) return;
		const thing = Thing.from(thingEl);
		if (thing) SelectedThing.set(thing);
	}, 50, { leading: true, trailing: false });
	document.body.addEventListener('mouseup', (e: MouseEvent) => {
		if (click.isProgrammaticEvent(e)) return;
		if (e.target instanceof Element) throttledMouseUp(e.target);
	});

	if (module.options.autoSelectOnScroll.value) {
		// `selectClosestInView` walks Things and measures rectangles, and this ran
		// synchronously on every scroll event with no throttle and without
		// `passive`, so it could both block scrolling and do that work dozens of
		// times per flick. `idleThrottle` is the same treatment `showImages`
		// already applies to its scroll handler; selection only has to settle once
		// the scroll does.

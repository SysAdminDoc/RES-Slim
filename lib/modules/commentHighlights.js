...
async function highlightFrom(lastVisit: number) {
...
module.contentStart = async () => {
	...
	const last = (await store.getNullable(key)) || 0;
	await highlightFrom(last);

	// Record this visit after a small delay so the user has time to actually see the highlights.

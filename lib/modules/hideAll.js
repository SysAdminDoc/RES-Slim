...
export const module: Module<{ [string]: any }> = new Module('hideAll');
...
const lastRunStorage = Storage.wrapFeature('hideAll', 'RES.hideAll.lastRun', (null: ?HideRun));
...
async function undo(fullnames: string[], uh: string, perSecond: number) {
	...
	if (!remaining.length) {
		await lastRunStorage.delete();
		removeUndoLink();
	} else {
		// The message below tells the reader to use the link again, so it has to
		// still be there — and it has to offer what is actually left, not the
		// whole run. Removing it unconditionally meant the instruction named a
		// control that no longer existed until the page was reloaded.
		//
		// The original run's timestamp is kept: the link's own title promises the
		// offer lasts thirty minutes from the hide, and stamping a fresh one on
		// every retry would extend that indefinitely.
		const previous = await lastRunStorage.get();
		const at = previous && Number.isFinite(previous.at) ? previous.at : Date.now();
		await lastRunStorage.set({ fullnames: remaining, at });
		injectUndoLink(remaining);
	}
	...
	showNotification({
		...
		header: 'Hide all',
		message: `Restored ${restored} post${restored === 1 ? '' : 's'}.${remaining.length ? ` ${remaining.length} couldn\'t be restored and are still hidden on Reddit. Use "undo hide all" again to retry.` : ''}`,
	});
}
...
async function runUndo(fullnames: string[]) {
	...
	if (!uh) {
		...
		// Nothing was attempted, so put the control back the way it was rather
		// than leaving it reading "restoring…" forever.
		injectUndoLink(fullnames);
		return;
	}
...
function buildResultMessage(hiddenCount: number, failed: number, skipped: number, onUndo: () => void): HTMLElement {
	...
	summary.textContent = [
		...
		failed ? ` ${failed} failed, so those are still visible.` : '',

...
async function requestYtdlp(targetUrl: string, status: HTMLAnchorElement): Promise<void> {
	const base = instanceFor();
	if (!isLocalhostUrl(base)) {
		status.textContent = 'companion: not localhost';
		return;
	}
	...
	if (!res.ok) {
		status.textContent = `companion: ${res.status}`;
		return;
	}
	const data = await res.json();
	if (!data || typeof data !== 'object') {
		status.textContent = 'companion: bad response';
		return;
	}
	const url = (data: any).url;
	if (typeof url !== 'string') {
		status.textContent = 'companion: no url';
		return;
	}
	triggerDownload(url, (data: any).filename || '');
	status.textContent = '✓ local DL';
}
...
function injectButton(thingEl: HTMLElement): void {
	...
	a.addEventListener('click', async (e: Event) => {
		e.preventDefault();
		const original = a.textContent || 'local DL';
		a.textContent = 'sending…';
		try {
			await requestYtdlp(url, a);
		} catch (err) {
			a.textContent = `companion: ${String((err && (err: any).message) || 'fail')}`;
		}

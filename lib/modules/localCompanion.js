...
export const module: Module<{ [string]: any }> = new Module('localCompanion');

module.moduleName = 'Local companion (yt-dlp / ffmpeg / ollama)';
module.category = 'productivityCategory';
module.description = 'Optional localhost-only bridge to a user-run helper service that wraps yt-dlp, ffmpeg, and ollama. Adds a `local DL` button next to posts. The bridge URL must be localhost (127.0.0.1 or localhost) — any other origin is rejected. Enabling this asks for permission to reach localhost; without it the browser blocks every request to your helper.';
module.descriptionRaw = true;
module.include = ['r2'];
module.disabledByDefault = true;
module.keywords = ['ytdlp', 'ffmpeg', 'ollama', 'companion', 'local', 'download'];

// Two separate gates stood between this module and its own companion, and both
// failed silently as `TypeError: Failed to fetch` — indistinguishable from the
// helper not running, which is why this went unnoticed.
//
// 1. Every cross-origin request is proxied through the background service worker,
//    so the `extension_pages` CSP governs it. That CSP said `connect-src https:`,
//    which forbids http entirely — including localhost. Fixed in both manifests.
// 2. A worker fetch to an origin the extension has no host permission for is
//    still subject to CORS, so it only worked if the user's helper happened to
//    send `access-control-allow-origin`. Requesting the origin lifts that.
//
// Optional rather than required: this is disabled by default, and nobody who is
// not running a companion should be asked for localhost access at install time.
module.permissions = {
	requiredPermissions: ['http://localhost/*', 'http://127.0.0.1/*'],
};
...
async function requestYtdlp(targetUrl: string, button: HTMLAnchorElement, restoreText: string): Promise<void> {
	...
	if (!looksLikeDownloadUrl(url)) {
		flashStatus(button, 'companion: invalid URL', { restore: restoreText, durationMs: 5000 });
		return;
	}

...
export const module: Module<{ [string]: any }> = new Module('version');
...
const pendingGreetingStorage = Storage.wrapFeature('versionLifecycle', PENDING_KEY, (false: boolean));
...
async function greetOnFirstRun(): Promise<boolean> {
	...
	showNotification({
		...
		message,
		closeDelay: 30000,
		// A greeting that shows once, on install, does not need an "always show
		// this notification type" checkbox — there is no second one to suppress,
		// and offering the control makes a one-off read as recurring.
		noDisable: true,
	});
	return true;
}
...
const pendingUpdateStorage = Storage.wrapFeature('versionLifecycle', PENDING_UPDATE_KEY, (null: PendingUpdate | null));

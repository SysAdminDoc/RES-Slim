...
export const module: Module<{ [string]: any }> = new Module('cobaltDownloader');
...
module.options = {
	instance: {
		...
		description: 'Comma- or newline-separated Cobalt API base URLs, tried in order. <strong>Empty by default and required</strong>: the hosted cobalt instances use bot protection and the project documents them as not intended for use by other projects without explicit permission, so RES-Slim ships no default. Run your own: see <code>https://github.com/imputnet/cobalt/blob/main/docs/run-an-instance.md</code>.',
	},
	companionFallback: {
		type: 'boolean',
		value: false,
...
// How many files one picker answer may start.
//
// A picker is a list of parts the instance thinks the reader wants. It is the
// instance's list, and an instance that answers with five hundred entries gets
// five hundred `a.click()` calls -- every one of them a download prompt or a
// file, from a host the reader chose only in the sense that it was in a list of
// defaults.
const MAX_PICKER_DOWNLOADS = 10;

function triggerDownload(url: string, filename: ?string): void {
	const a = document.createElement('a');
	a.href = url;
	// Through the same sanitiser the expando's own download control uses. The name
	// comes from the instance, and `a.download` is where a path separator or a
	// reserved Windows name would land.
	a.download = downloadFilename(filename, url);
...
async function postToInstance(instance: string, body: { [string]: mixed }): Promise<?Object> {
	...
	try {
		res = await fetch(instance, {
			...
			// An instance that accepts the connection and never answers used to hold
			// the button on "sending…" for the life of the page, and the next
			// instance in the list was never tried.
...
async function requestCobalt(targetUrl: string, button: HTMLAnchorElement, restoreText: string): Promise<void> {
	...
	if (!instances.length) {
		// Not configured. Say so rather than firing a request that cannot succeed
		// and reporting it as "all instances failed", which reads like an outage.
		flashStatus(button, 'cobalt: no instance configured', { restore: restoreText, durationMs: 6000 });
		showNotification({
			moduleID: 'cobaltDownloader',
			notificationID: 'cobaltDownloader-unconfigured',
			header: 'Cobalt downloader',
			message: `This module needs a Cobalt instance you control. Cobalt's hosted instances use bot protection and its documentation states they are not intended for use by other projects, so RES-Slim ships no default. Set "Cobalt instance URL(s)" in settings. Self-hosting guide: ${SELF_HOSTING_DOCS}`,
			closeDelay: 20000,
		});
		return;
	}
	...
	if (kind === 'picker') {
		...
		if (Array.isArray(picker) && picker.length) {
			const usable = picker.filter(item => item && looksLikeStreamUrl(item.url));
			const taking = usable.slice(0, MAX_PICKER_DOWNLOADS);
			for (const item of taking) triggerDownload(item.url, item.filename);
			const withheld = usable.length - taking.length;
			const note = withheld ? ` (${withheld} more not taken)` : '';
			flashStatus(
				button,
				taking.length ? `✓ picker (${taking.length})${note}` : 'cobalt: empty picker',
				{ restore: restoreText, durationMs: 5000 },
			);
		} else {

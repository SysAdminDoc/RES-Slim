...
async function requestCobalt(targetUrl: string, button: HTMLAnchorElement, restoreText: string): Promise<void> {
	...
	try {
		...
		flashStatus(button, `cobalt: network error`, { restore: restoreText, durationMs: 5000 });
		return;
	}

	if (!res.ok) {
		flashStatus(button, `cobalt: HTTP ${res.status}`, { restore: restoreText, durationMs: 5000 });
		return;
	}
	...
	try {
		...
		flashStatus(button, 'cobalt: non-JSON response', { restore: restoreText, durationMs: 5000 });
		return;
	}
	if (!data || typeof data !== 'object') {
		flashStatus(button, 'cobalt: empty response', { restore: restoreText, durationMs: 5000 });
		return;
	}

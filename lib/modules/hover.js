...
export const module: Module<{ [string]: any }> = new Module('hover');
...
module.options = {
	...
	fadeSpeed: {
		// Was `noconfig: true, // broken`, inherited and never explained. It is
		// read on every hover card - `beforeLoad` copies it into the defaults, and
		// `_startCloseFade` uses it for both the close timer and the CSS
		// transition - and `showParent` exposes the same knob. So the option
		// worked and only the settings entry was missing.
		type: 'text',
		value: '0.7',
...
// No `string`. The branch that took one assigned it to `innerHTML` raw, and the
// type was the only thing inviting a future provider to return one -- the single
// caller has always passed elements. Anything that wants markup here builds it,
// or goes through `setTrustedHTML` with DOMPurify the way `notifications` does.
type HoverContents = HTMLElement | DocumentFragment | null;
...
class Hover {
	...
	// An abort controller rather than a resolve-only promise. The old shape could
	// only ever say "stop" by resolving, which `waitForDetach` had to special-case;
	// a signal is the contract every waiter in `dom.js` already takes.
	_closeController: AbortController;

	// Which open's controller the fixed-parent scroll listener was bound for.
	// `populate()` runs at least twice per open -- once for the load indicator,
	// once for the real content, and again for every `refresh()` -- so binding
	// unconditionally stacked a listener per repopulate on top of the per-open
	// leak. Comparing against the live controller means one listener per open,
	// released with it.
	_scrollBound: ?AbortController;
	...
	open() {
		...
		if (this._closeController) this._closeController.abort();
		...
		waitForDetach(this.getCheckedTarget(), { signal: this._closeController.signal }).then(() => { this.close(); })
	...
	populate(items: HoverContents[]) {
		...
		// Reset, not left as it was. This latched on: one hover over a fixed
		// ancestor set it, nothing ever cleared it, and every later hover from the
		// same instance positioned itself `fixed` while its coordinates were still
		// being computed without the scroll offset -- so the popover stuck to the
		// viewport at a position that had nothing to do with what was hovered.
		...
		if (fixedParent) {
			...
			// Removed by the close controller rather than by the handler itself.
			// Unsubscribing on the first scroll only cleans up when a scroll
			// actually happens: a hover opened over a fixed ancestor and closed by
			// moving the pointer away left its listener attached, holding the
			// instance and the target, and the next hover added another. The
			// controller is aborted in `remove()` and replaced on every `open()`, so
			// binding to its signal ties the listener to exactly the life of one
			// hover.
			//
			// It never preventDefaults, so it has no business holding up a scroll.
			if (this._scrollBound !== this._closeController) {
				this._scrollBound = this._closeController;
				fixedParent.addEventListener('scroll', () => { this.close(false); }, {
					capture: true,
					passive: true,
	...
	remove() {
		...
		// Closing is what releases the detach watcher. Without this the observer
		// outlived every hover that was ever shown.
		if (this._closeController) this._closeController.abort();
		this.visible = false;
...
class HoverInfoCard extends Hover {
	...
	// Built rather than parsed. This was the one thing in the repo still using
	// `populate`'s string branch, which assigned to `innerHTML` raw -- the branch
	// existed for this and the type invited anything else to use it the same way.
	_loadIndicator(): HTMLElement {
		const wrapper = document.createElement('div');
		wrapper.className = 'RESCenteredLoadIndicator';
		const spinner = document.createElement('span');
		spinner.className = 'RESLoadingSpinner';
	...
	_displayLoadIndicator() {
		...
		this.populate([ellipsis, this._loadIndicator()]);
	}

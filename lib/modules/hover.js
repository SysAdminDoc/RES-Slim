...
class Hover {
	...
	populate(items: HoverContents[]) {
		...
		if (fixedParent) {
			...
			// Closes the hover and unsubscribes itself; it never preventDefaults, so
			// it has no business holding up a scroll.
			fixedParent.addEventListener('scroll', onScroll, { capture: true, passive: true });
		}

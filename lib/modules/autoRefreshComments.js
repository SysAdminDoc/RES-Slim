...
async function pollOnce(): Promise<number> {
	...
	const url = `${location.pathname.replace(/\/$/, '')}.json?sort=new&depth=10&limit=200`;
	...
	for (const child of newOnes) {
		...
		placeholder.innerHTML = `<div class="entry">
			<p class="tagline">by <a class="author">${(d.author || '[deleted]').replace(/[<>&]/g, ch => ({ '<': '&lt;', '>': '&gt;', '&': '&amp;' }[ch] || ch))}</a>
			· <span class="score unvoted">${typeof d.score === 'number' ? d.score : '?'} pt</span></p>
			<div class="usertext-body">${d.body_html || `<p>${(d.body || '').replace(/[<>&]/g, ch => ({ '<': '&lt;', '>': '&gt;', '&': '&amp;' }[ch] || ch))}</p>`}</div>
		</div>`;

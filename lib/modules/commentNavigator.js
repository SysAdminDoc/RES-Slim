...
async function installEntryElement() {
	...
	const commentNavToggle = string.html`<button type="button" class="res-commentNavToggle" aria-haspopup="menu" aria-expanded="false">Navigate by</button>`;
	...
	commentNavToggle.addEventListener('mouseenter', async () => {
		commentNavToggle.setAttribute('aria-expanded', 'true');
		empty(choices);
		const categories = await getCategories();
		choices.append(...categories.map(({ category, title, size }) => {
			const element = string.html`
				<button type="button" class="res-commentNavToggle-type" title="${title}" category="${category}">${category} (${size})</button>
			`;
			element.addEventListener('click', () => { setCategory(category); });
			return element;
		}));
	});
	commentNavToggle.addEventListener('mouseleave', () => {
		commentNavToggle.setAttribute('aria-expanded', 'false');
	});
}

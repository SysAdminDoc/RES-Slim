...
export const module: Module<{ [string]: any }> = new Module('commentNavigator');
...
// A WeakSet, not a Set: the only operations are `add` and `has`, and on current
// Reddit the document survives every route change, so a Set kept every Thing the
// reader ever selected - and its element - reachable for the whole session.
const readComments = new WeakSet();

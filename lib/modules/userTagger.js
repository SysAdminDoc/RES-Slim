...
module.options = {
	...
	importActions: {
		...
		values: [
			...
			{ text: 'Undo last import', callback: undoLastTagImport },
...
// Null-prototype: the key is a reddit username read off the page, and every
// lookup here is a plain member read. On a plain object, an author called
// `toString` or `valueOf` would have produced a "tag" that is an
// Object.prototype function, which `renderForAuthor` then reads `.color` off.
const cache: UserTagMap = (Object.create(null): any);
...
// Drop everything the document no longer holds. Cheap enough to run on a route
// change: it walks what is in the index, not what is on the page.
function pruneAuthorIndex(): void {
	for (const [username, set] of authorIndex) {
		for (const authorEl of set) {
			// `document.contains` rather than `.isConnected`, which Flow's DOM libdef
			// does not know about; the index only ever holds nodes from this document.
			if (!document.contains(authorEl)) set.delete(authorEl);
		}
	...
	for (const authorEl of set) {
		...
		if (thing instanceof HTMLElement) {
			if (hideIgnored && tag && tag.ignore) {
				...
				// Ignoring a user mid-playback otherwise left their video audible.
				hideAndSilence(thing);
...
async function previewTagImport(): Promise<void> {
	...
	pendingTagImport = {
		raw,
		conflictMode,
		// The tags themselves, not a string of them. `stringifyTags` includes `ts`,
		// and `normalizeTag` stamps `Date.now()` on any record that arrives without
		// a valid one -- which the migration path, the data workspace and this
		// module's own `ts: 0` default all produce. Two independent normalisations
		// of such a map differ unless they land in the same millisecond, so a
		// string signature refused almost every previewed import as "the stored
		// tags changed". `sameTags` compares identity with the timestamps out.
		baseTags: current,
...
async function commitPreviewedTagImport(): Promise<void> {
	...
	const raw = String(liveOptionValue('importJson') || '');
	const conflictMode: TagImportConflictMode = liveOptionValue('importConflicts') === 'replace' ? 'replace' : 'keep';
	// Both shapes: the normalised map for the plan and the restore, and exactly
	// what storage held for the compare-and-set. Normalising fills in defaults, so
	// a normalised map is not what is stored and would never compare equal to it.
	const storedTags = await tagMapStore.get();
	const current = normalizeTagMap(storedTags);
	if (raw !== pending.raw || conflictMode !== pending.conflictMode || !sameTags(current, pending.baseTags)) {
		pendingTagImport = null;
	...
	try {
		await commitTagImport({
			original: current,
			storedOriginal: storedTags,
			next: pending.next,
			saveRollback: snapshot => rollbackStore.set({ createdAt: Date.now(), tags: snapshot }),
			compareAndSetMap: (expected, value) => tagMapStore.compareAndSet((expected: any), value),
		...
		pendingTagImport = null;
		setImportStatus(`Imported ${importCountText(pending.counts)}. Undo last import restores the tags as they were.`, 'success');
	} catch (error) {
		pendingTagImport = null;
		const message = error instanceof Error ? error.message : String(error);
		// A conflict is not a failure with a restore behind it. Nothing was written
		// and nothing was put back -- the stored tags are another window's import,
		// and telling the reader theirs was restored is both false and the opposite
		// of what they need to do next, which is preview again against what is there
		// now. The message the conflict carries already says that.
		setImportStatus(
			error instanceof TagImportConflict ? message : `Import failed. The previous tag map was restored. ${message}`,
			'error',
		);
		throw error;
	}
...
// The rollback snapshot has been written on every import since it shipped, and
// nothing ever read it — so the success message promised a recovery path the
// settings page did not have. This is that path, the same shape
// `subredditBlacklist.undoLastListImport` uses.
async function undoLastTagImport(): Promise<void> {
	if (!canPersistFeatureData('userTagger')) {
		setImportStatus('User-tag changes are not stored from private windows.', 'error');
		throw new Error('User-tag changes are not stored from private windows.');
	}

	const rollback = await rollbackStore.get();
	if (!rollback || !rollback.tags) {
		// Covers both "no import yet" and "the last one has already been undone":
		// the snapshot is cleared on use, so the two are indistinguishable here
		// and claiming the first would be a guess.
		setImportStatus('There is nothing to undo: no import is waiting to be rolled back.', 'error');
		throw new Error('There is nothing to undo.');
	}

	const restored = normalizeTagMap(rollback.tags);
	const current = normalizeTagMap(await tagMapStore.get());
	// Compared without timestamps: `normalizeTag` stamps `ts` with the current
	// time for any record that lacks one, so comparing two normalisations of the
	// same tags answers "were these normalised in the same millisecond?" rather
	// than "are these the same tags?".
	if (sameTags(restored, current)) {
		setImportStatus('There is nothing to undo: the tags already match the saved pre-import copy.', 'error');
		throw new Error('There is nothing to undo.');
	}

	await tagMapStore.set(restored);
	// The page is annotating from the cache, not from storage, so a restore that
	// only writes leaves every tag on screen showing the imported value.
	replaceCache(restored);
	// One snapshot, one undo. Keeping it would let a second press "restore" over
	// work done since, which is not what undo means.
	await rollbackStore.set(null);
	pendingTagImport = null;

	const when = Number.isFinite(rollback.createdAt) ? new Date(rollback.createdAt).toLocaleString() : 'the last import';
	setImportStatus(`Restored the ${Object.keys(restored).length} tag records saved before ${when}.`, 'success');
}

async function exportCommittedTags(): Promise<void> {
	const committed = normalizeTagMap(await tagMapStore.get());
	downloadText(stringifyTags(committed), `res-slim-user-tags-${new Date().toISOString().slice(0, 10)}.json`);
...
export const _internal = { cache, stringifyTags, authorIndex, pruneAuthorIndex };
...
module.contentStart = () => {
	...
	// Old Reddit reloads on every navigation, so the index died with the page.
	// Current Reddit keeps one document for the whole session, and an entry is
	// only pruned when that exact username is re-tagged - so every author anchor
	// of every post and comment ever rendered stayed reachable through it. The
	// same route signal `watchers.js` already clears its own dupe set on.
	document.addEventListener('reddit.urlChanged', pruneAuthorIndex);

...
module.description = 'Tag users with a label, a colour, and/or an ignore flag. Tags are stored locally; nothing is sent off-host. Imports are previewed before one atomic commit.';
...
const tagStore = Storage.wrapBlob('RESmodules.userTagger.tags', (): UserTag | null => null);
const tagMapStore = Storage.wrap('RESmodules.userTagger.tags', ({}: UserTagMap));
type TagRollback = {| createdAt: number, tags: UserTagMap |};
const rollbackStore = Storage.wrap('RESmodules.userTagger.tags.rollback', (null: ?TagRollback));

...
function openPopover(anchorEl: HTMLAnchorElement, username: string, triggerBtn: HTMLButtonElement): void {
	...
	saveBtn.addEventListener('click', async () => {
		...
		try {
			...
			setPopoverBusy(pop, false, 'Couldn’t save — the tag is still only in this box. Try again.', 'error');
		}
	});
	clearBtn.addEventListener('click', async () => {
		...
		try {
			...
			setPopoverBusy(pop, false, 'Couldn’t clear — the tag is still saved. Try again.', 'error');
		}
	});

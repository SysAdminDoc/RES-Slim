...
import type { UserTag, UserTagMap } from '../utils/userTags';

export const module: Module<*> = new Module('userTagger');

module.moduleName = 'User tagger';
module.category = 'usersCategory';
module.description = 'Tag users with a label, a colour, and/or an ignore flag. Tags are stored locally; nothing is sent off-host. Import/export via the JSON box.';
...
module.options = {
	...
	colorizeUsername: {
		type: 'boolean',
		value: true,
		title: 'Colorise tagged usernames',
		description: 'Apply the saved colour to the username itself, not just the badge.',
	},
	hideIgnored: {
		type: 'boolean',
		value: true,
	...
	defaultBadgeColor: {
		type: 'color',
		value: '#5b8def',
		title: 'Default badge colour',
		description: 'Colour used when a tag has no per-user colour set.',
	},
	importJson: {
		type: 'text',
		value: '',
		title: 'Import JSON',
		description: 'Paste a JSON object of { "username": { "tag": "...", "color": "#...", "ignore": false } } here to merge into your tag store on next page load. Existing entries with matching usernames are overwritten.',
	},
};

const store = Storage.wrapBlob('RESmodules.userTagger.tags', (): UserTag | null => null);
...
let activePopover: ?{| element: HTMLElement, cleanup: () => void |} = null;
...
function closeActivePopover(): void {
	...
	const { cleanup, element } = activePopover;
	activePopover = null;
	cleanup();
	if (element.isConnected) element.remove();
}

function openPopover(anchorEl: HTMLAnchorElement, username: string): void {
	...
	pop.setAttribute('aria-label', `Tag ${username}`);
	...
	row2.append(colorInput);
	pop.append(row2);

	const row3 = document.createElement('label');
	row3.className = `${POPOVER_CLASS}-row`;
	...
	row3.append(ignoreInput, ' Ignore (hide their posts and comments)');
	pop.append(row3);
	...
	activePopover = { element: pop, cleanup };
...
async function loadInitialCache(): Promise<void> {
	...
	for (const k of Object.keys(all)) {
		const norm = normalizeTag((all: any)[k]);
		if (norm) cache[normalizeUsername(k)] = norm;
	}
}

async function applyImportJson(): Promise<void> {
	const raw = (module.options.importJson.value: any) || '';
	if (!raw || !raw.trim()) return;
	const incoming = parseTagsJson(raw);
	if (!Object.keys(incoming).length) return;
	const merged = mergeTags(cache, incoming);
	for (const username of Object.keys(merged)) {
		const v = merged[username];
		cache[username] = v;
		await store.set(username, v);
	}
}
...
module.contentStart = async () => {
	...
	try { await applyImportJson(); } catch (e) { /* import is best-effort */ }

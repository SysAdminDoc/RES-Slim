...
function placePopover(pop: HTMLElement): void {
	...
	const pad = 12;
	if (rect.right > window.innerWidth - pad) pop.classList.add('is-flipped');
	if (rect.bottom > window.innerHeight - pad) pop.classList.add('is-above');
}

function setPopoverBusy(pop: HTMLElement, busy: boolean, message: string): void {
...
function openPopover(anchorEl: HTMLAnchorElement, username: string, triggerBtn: HTMLButtonElement): void {
	...
	const onResize = () => { if (pop.isConnected) placePopover(pop); };
	const cleanup = () => {
	...
	saveBtn.addEventListener('click', async () => {
		...
		try {
			...
			setPopoverBusy(pop, false, 'Could not save. Try again.');
		}
	});
	clearBtn.addEventListener('click', async () => {
		...
		try {
			...
			setPopoverBusy(pop, false, 'Could not clear. Try again.');
		}
	});
...
async function applyImportJson(): Promise<void> {
	...
	await Promise.all(Object.keys(merged).map(async username => {

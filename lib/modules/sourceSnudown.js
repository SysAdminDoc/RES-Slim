...
const sourceButton = (e => () => preventCloning(e().cloneNode(true)))(once(() => {
	document.body.addEventListener('click', (e: Event) => {
		const viewSourceLink = e.target instanceof Element && e.target.closest('li.viewSource a');
		if (viewSourceLink) {
			e.preventDefault();
			viewSource(viewSourceLink);
		}
		const cancelBtn = e.target instanceof Element && e.target.closest('.usertext-edit.viewSource .cancel');
		if (cancelBtn) {
			const viewSourceEdit = cancelBtn.closest('.usertext-edit.viewSource');
...
const viewSource = keyedMutex(async button => {
	...
	if (button.dataset.sourceOpen) {
		const viewSourceEdit = button.closest('.thing').querySelector('.usertext-edit.viewSource');
		if (viewSourceEdit) {

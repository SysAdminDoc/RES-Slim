...
import { actionLogSize, readActionLog, recordAction } from '../utils/actionLog';
...
function onPreviewRequest({ origin, data, source }: any) {
	...
	// Two questions over one channel, because they have the same shape and the
	// same origin check, and a second listener would be a second place to get
	// that check wrong.
	if (data && data.requestActionLog) {
		try {

...
const OUTCOME_BY_ACTION = { hide: 'hidden', dim: 'dimmed', collapse: 'collapsed', badge: 'badged' };
...
function applyMatches(thing: Thing, kind: 'post' | 'comment', matches) {
	...
	for (const rule of matches) {
		...
		// Written before the action, not after, because `hide` returns out of the
		// loop and the decision is what is being recorded rather than its
		// completion. This is the entry the reader is looking for when they ask
		// why a post is not there.
		recordAction({
			moduleID: 'filterRules',
			outcome: (OUTCOME_BY_ACTION[rule.action] || 'hidden'),
			target: thing.getFullname() || el.id || kind,
			reason: String(rule.id || rule.field || 'rule'),
			detail: rule.field ? `${String(rule.field)} ${String(rule.op)} ${String(rule.value)}` : '',
		});
		switch (rule.action) {
			case 'hide':
...
// Exported for the contract. The counts are the whole product of this function
// and they are not observable from the message handler that wraps it.
export function previewRules(rulesJson: mixed, reveal: boolean): {|
	scanned: number,
	matched: number,
...
|} {
	const parsed = parseFilterRules(rulesJson);
	// No prototype, because the keys are the reader's own rule ids and one of them
	// can be `__proto__`. Assigning a number to that on an ordinary object is a
	// silent no-op: the rule got no entry, and the panel that exists to say what a
	// filter set would do reported "No rules to try" for a set that had matched.
	// Nothing was polluted; the answer was just wrong, which is worse in a surface
	// whose whole job is to be checked against.
...
//
// The action log is answered from `settingsNavigation` instead, and not for
// tidiness: this module is not `alwaysEnabled`, so `contentStart` does not run
// when the filter builder is switched off -- and sharing the channel meant
// switching off one feature silently took out a panel that reports on four
// modules, three of which were still writing to it.

...
export const module: Module<{ [string]: any }> = new Module('readComments');
...
const entryStorage = Storage.wrapFeaturePrefix('readComments', 'readComments.', (): {|
	updateTime: number,
	ids: { [string]: true },
|} => ({
	updateTime: Date.now(),
	ids: {},
}), { allowPrivate: () => module.options.monitorWhenIncognito.value });
...
module.beforeLoad = async () => {
	...
	if (module.options.monitorSelected.value) {
		if (!canPersistFeatureData('readComments', module.options.monitorWhenIncognito.value)) return;
...
// Read state is opaque-key set membership by fullname (see isRead) — no id
// arithmetic or ordering — so Reddit dropping monotonic comment ids is a non-issue.

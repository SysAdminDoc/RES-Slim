...
export const module: Module<{ [string]: any }> = new Module('markAllRead');
...
async function markAll(btn: HTMLButtonElement) {
	...
	const modhash = (document.querySelector('input[name="uh"]'): any)?.value ||
		(window: any).modhash;
	if (!modhash) {
		// Without a modhash the POST will either be silently ignored by reddit or
		// rejected with a 403 — in both cases the user gets no feedback and thinks
		// the action worked. Fail loudly instead.
		btn.textContent = 'Could not read your login token \u2717';
		btn.title = 'Reload the page and try again.';
		btn.disabled = false;
		return;
	}
	try {
		...
		btn.textContent = 'Failed, reload and retry \u2717';
		btn.disabled = false;
	}
}

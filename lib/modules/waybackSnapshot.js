/* @flow */
// RES-Slim: snapshot-now button. Adds an `archive` link to every post that
...
export const module: Module<{ [string]: any }> = new Module('waybackSnapshot');

module.moduleName = 'Wayback snapshot-now';
module.category = 'productivityCategory';
module.description = 'Adds an `archive` link to posts. Checks the Wayback Machine for an existing snapshot; offers to trigger a fresh save if none is recent enough. External integration. Defaults off.';
module.descriptionRaw = true;
module.include = ['r2'];
module.disabledByDefault = true;
module.permissions = { requiredPermissions: ['https://web.archive.org/*'] };
...
module.options = {
	...
	stalenessDays: {
		...
		description: 'Snapshots older than this trigger a fresh save in "check" mode. Defaults to 90 days.',
	},
	target: {
		type: 'enum',
		value: 'permalink',

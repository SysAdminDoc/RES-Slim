...
module.go = () => {
	...
	// See savedBackup: 15px tall in the userbar, under WCAG 2.5.8's 24x24 target.
...
async function togglePanel(e: Event) {
	...
	try {
		...
		const rows = infos.map(info => {
			...
			purgeBtn.disabled = !info.purgeable;
			if (info.available && !info.purgeable) purgeBtn.title = 'Saved content is purged per account, from the saved-content manager.';
		...
		if (infos.length) {
			...
			empty.textContent = 'Nothing stored yet. Modules that keep local history, such as the vote log, media manifest and user tags, will appear here once they save something.';
			panel.append(empty);
		}

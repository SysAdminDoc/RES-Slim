...
export const module: Module<{ [string]: any }> = new Module('commentQuickCollapse');
...
function hideCommentsOnHeaderDoubleClick() {
	document.body.addEventListener('dblclick', (e: Event) => {
		const tagline = e.target instanceof Element && e.target.closest('.tagline');
		if (!tagline) return;
		const thing = Thing.checkedFrom(tagline);
...
function scrollOnCollapse() {
	document.body.addEventListener('click', (event: MouseEvent) => {
		if (!(event.target instanceof Element) || !event.target.closest('.expand')) return;

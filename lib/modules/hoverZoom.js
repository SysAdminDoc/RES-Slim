...
import { isSafeMediaUrl } from '../utils/mediaUrl';

export const module: Module<{ [string]: any }> = new Module('hoverZoom');
...
module.options = {
	delayMs: {
		...
		description: 'How long the cursor must rest on a link before the preview opens. Defaults to 180 ms.',
	},
	maxWidth: {
		type: 'text',
		value: '480',
...
function buildPopover(url: string, kind: 'image' | 'video'): ?HTMLElement {
	destroyPopover();
	// The URL comes from an anchor on the page, and with `requireDirectUrl` off it
	// is the raw href -- so `javascript:` and `data:text/html` are both things a
	// post can put in front of a hover. Nothing else in this module looks at the
	// scheme.
	if (!isSafeMediaUrl(url, location.href)) return null;
	...
	if (kind === 'image') {
		...
		//
		// This module fetches from whatever host the link names, on hover, before
		// any click. Without this the request carries the full Reddit URL the reader
		// is on -- the subreddit, the post, and on a profile the account -- to a host
		// chosen by whoever posted the link. Hovering is not consent to be
		// identified.

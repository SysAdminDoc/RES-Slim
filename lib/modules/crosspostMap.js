...
export const module: Module<{ [string]: any }> = new Module('crosspostMap');
...
function renderList(host: HTMLElement, items: Duplicate[]): void {
	...
	if (!visible.length) {
		...
		for (const dup of visible) {
			...
			// Both hrefs come out of reddit's duplicates listing. The subreddit is
			// interpolated into a path, so it is held to the character set a real
			// one has; the permalink is checked as a whole.
			sub.href = `/r/${encodeURIComponent(dup.subreddit)}`;
			sub.textContent = `r/${dup.subreddit}`;
			sub.title = `Open r/${dup.subreddit}`;

			const link = document.createElement('a');
			link.className = `${LIST_CLASS}-link`;
			link.href = isSafeLinkUrl(dup.permalink, location.origin) ? dup.permalink : '#';
...
function injectWidget(): void {
	...
	const trigger = async () => {
		if (loaded) return;
		if (!articleId) {
			btn.textContent = 'Post ID not found';
			btn.disabled = true;

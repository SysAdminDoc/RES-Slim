...
module.description = 'Colour post and comment scores by vote weight, optionally interpolate custom colour thresholds, and make controversial comments easier to spot. Works on old and current Reddit without guessing vote totals.';
...
module.options = {
	...
	colorLinkScore: {
		...
		value: 'none',
		description: 'Colour listing ranks and post score numbers according to score.',
		bodyClass: true,
	},
	userDefinedLinkColoration: {
		...
		description: 'Ascending score thresholds and their colours.',
		sort([a], [b]) {
			return Number(a) - Number(b) || String(a).localeCompare(String(b), undefined, { numeric: true });
		},
	},
	colorCommentScore: {
		title: 'Comment score colour',
		type: 'enum',
		values: [
			{ name: 'No coloration', value: 'none' },
			{ name: 'Automatic coloration', value: 'automatic' },
		...
		value: 'none',
		description: 'Colour comment scores according to score.',
	},
	userDefinedCommentColoration: {
		...
		description: 'Ascending comment-score thresholds and their colours.',
		sort([a], [b]) {
			return Number(a) - Number(b) || String(a).localeCompare(String(b), undefined, { numeric: true });
		},
	},
	interpolateScoreColor: {
		...
		type: 'boolean',
		value: true,
		description: 'Blend smoothly between adjacent custom colours instead of stepping at each threshold.',
		advanced: true,
	},
	highlightControversial: {
		...
		type: 'boolean',
		value: true,
		description: 'Use a distinct colour for Reddit\'s controversial-comment marker.',
	},
	highlightControversialColor: {
		title: 'Controversial marker colour',
		dependsOn: options => options.highlightControversial.value,
		advanced: true,
		type: 'color',
		value: '#cc0000',
		description: 'Colour used for the controversial marker.',
	},
};

const SHADOW_SCORE_CSS = `
:host([data-res-vote-enhancements-score]) .rpl-vote-button-group > :where(span, faceplate-number):not(:has(button)),
:host([data-res-vote-enhancements-score]) [data-testid='comment-sub-header'] :where(faceplate-number, [data-testid='comment-score']),
:host([data-res-vote-enhancements-score]) [data-testid='comment-action-row'] :where(faceplate-number, [data-testid='comment-score']) {
	color: var(--rsm-vote-enhancements-score) !important;
}

:host-context(.res-voteEnhancements-highlightScores) .rpl-vote-button-group > :where(span, faceplate-number):not(:has(button)),
:host-context(.res-voteEnhancements-highlightScores) [data-testid='comment-sub-header'] :where(faceplate-number, [data-testid='comment-score']),
:host-context(.res-voteEnhancements-highlightScores) [data-testid='comment-action-row'] :where(faceplate-number, [data-testid='comment-score']) {
...
function applyShadowScore(thing: Thing, color: string): void {
	...
	const root = thing.element.shadowRoot;
	if (!root) return;
	for (const scoreElement of root.querySelectorAll([
		...
		if (scoreElement instanceof HTMLElement) scoreElement.style.setProperty('color', color, 'important');
	}
}
...
module.beforeLoad = () => {
	registerShadowStyle('vote-enhancements', SHADOW_SCORE_CSS);

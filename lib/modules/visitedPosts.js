...
import { DEFAULT_EXPIRE_DAYS, fullnameFromActivation, fullnameFromCommentsUrl, isPostFullname, pruneVisited } from '../utils/visitedPosts';
import { hideAndSilence } from '../utils/mediaSilence';

export const module: Module<{ [string]: any }> = new Module('visitedPosts');

module.moduleName = 'Mark visited posts';
module.category = 'browsingCategory';
module.description = 'Remembers which posts you have opened and dims or hides them in later listings. The record is local to this browser and holds nothing but post IDs and timestamps. Private-window activity is not stored.';
...
const store = Storage.wrapFeatureBlob('visitedPosts', 'RESmodules.visitedPosts.seen', (): number => 0);
...
function decorate(thing: Thing) {
	...
	switch (module.options.treatment.value) {
		case 'hide':
			// A post the user already opened can still be playing when the listing
			// re-renders and marks it seen.
			hideAndSilence(el);
			break;
		case 'mark':
			break;
...
function rememberFromEventTarget(target: EventTarget | null) {
	if (!(target instanceof Element)) return;
	remember(fullnameFromActivation(target));
}

function watchClicks() {
	// `mousedown` covers the middle click and the ctrl-click as well as the
	// ordinary one, because all three fire it. What it does not cover is a
	// keyboard user: pressing Enter on a focused link fires no pointer event at
	// all, so every post they opened went unrecorded and the module was inert for
	// them. `remember` is keyed on the fullname and writes the same timestamp
	// twice for one open, so the two paths overlapping costs nothing.
	document.addEventListener('mousedown', (e: MouseEvent) => {
		rememberFromEventTarget(e.target);
	}, { capture: true, passive: true });

	document.addEventListener('keydown', (e: KeyboardEvent) => {
		// Enter activates a link; Space does not, it scrolls. A modifier means the
		// reader is doing something else with the key.

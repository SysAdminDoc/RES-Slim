...
export const module: Module<{ [string]: any }> = new Module('newCommentCount');
...
const entryStorage = Storage.wrapFeaturePrefix2('newCommentCountVisits', 'newCommentCount.', (): {|
	count: number,
	updateTime: number,
|} => {
	throw new Error('Default value should never be retrieved');
}, {
	allowPrivate: () => module.options.monitorPostsVisitedIncognito.value,
	batching: true,
});

const subscriptionStorage = Storage.wrapFeatureBlob('newCommentCountSubscriptions', 'RESmodules.newCommentCount.subscriptions', (): {|
	count: number,
...
// The id is used purely as an opaque storage key (never parsed as base36 or
// compared for ordering/age — counts come from getCommentCount(), age from
// timestamps), so it is unaffected by Reddit dropping monotonic comment ids.
...
function setEntry(id: string, newCommentCount: number) {
	...
	if (!canPersistFeatureData('newCommentCountVisits', module.options.monitorPostsVisitedIncognito.value)) return false;
...
async function displayNewCommentCount(thing) {
	...
	const commentCountEl = thing.getCommentCountElement();
	if (commentCountEl) {
		const span = document.createElement('span');
		span.className = 'newComments';
		span.innerHTML = `&nbsp;(${newCount} new)`;
		commentCountEl.appendChild(span);
	}
}
...
const addSubscriptionButton = id => {
	...
	const refresh = mutex(async () => {
		if (await subscriptionStorage.has(id)) {
			...
			button.title = 'stop receiving notifications';
			...
			button.title = 'notify me of new comments';
	...
	(new Promise(requestAnimationFrame))
		.then(() => waitForDescendant(document.body, ['.commentarea .panestack-title', '.menuarea'], { owner: module.moduleID }))
		.then(e => { e.append(button); })
		.catch(() => {
			// The waiter has already recorded which selector never appeared. There
			// is nowhere to put the button, and that is the whole consequence.
		});
};
...
async function checkThread(id, subscription) {
	...
	if (newCount > count) {
		...
		const notifContent = notification.element.querySelector('.RESNotificationContent');
		if (notifContent) notifContent.appendChild(createButton(id, 'unsubscribe', notification.close));
	}
	if (module.options.notifyEditedPosts.value && newEditedTime > editedTime) {
		...
		// add button to unsubscribe from within notification.
		const notifContent2 = notification.element.querySelector('.RESNotificationContent');
		if (notifContent2) notifContent2.appendChild(createButton(id, 'unsubscribe', notification.close));
	}
}

function createButton(id, type, onClick) {
	const button = document.createElement('span');
	button.className = 'RESSubscriptionButton';
	let action;
	...
	switch (type) {
		...
		case 'renew':
			...
			button.title = `renew for ${module.options.subscriptionLength.value} days`;
			action = async () => {
...
async function addDashboardFunctionality(tabPage) {
	...
	ele.addEventListener('click', (e: Event) => {
		if (e.target instanceof HTMLTableCellElement && e.target.tagName === 'TH') {
			Table.sortByColumn.call(e.target, e);
		}
	});

...
// Long enough for a large image on a slow connection, short enough that one
// unreachable piece does not hold the whole archive open forever.
const IMAGE_TIMEOUT_MS = 30000;

async function fetchImageBlob(url: string): Promise<Blob> {
	const res = await fetch(url, { credentials: 'omit', signal: AbortSignal.timeout(IMAGE_TIMEOUT_MS) });

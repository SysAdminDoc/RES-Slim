...
function injectButton(thing: HTMLElement): void {
	...
	a.addEventListener('click', async (e: Event) => {
		...
		try {
			...
			if (!items.length) {
				a.textContent = 'no images';
				return;
			}
			a.textContent = `zipping ${items.length}…`;
			...
			a.textContent = `✓ ${items.length} images`;
		} catch (err) {
			a.textContent = 'zip failed';
		}
		setTimeout(() => { a.textContent = original || 'ZIP gallery'; }, 4000);
	});
}

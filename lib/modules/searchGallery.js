...
export const module: Module<{ [string]: any }> = new Module('searchGallery');
...
module.options = {
	...
	onlyVisible: {
		...
		description: 'Only load previews for results once you scroll to them.',
	},
};
...
function buildStrip(urls: string[], maxWidth: number): HTMLElement {
	...
	// The URL is reddit's own `media_metadata` / `preview` value, which lands in
	// an href and an img src without ever passing through showImages' allowlist.
	for (const url of urls.filter(url => isSafeLinkUrl(url, location.origin))) {

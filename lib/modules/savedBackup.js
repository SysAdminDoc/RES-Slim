...
async function runBackup(status: HTMLAnchorElement): Promise<void> {
	...
	if (!collected.length) {
		status.textContent = 'no saved items';
		return;
	}
	...
	status.textContent = `✓ ${collected.length} items`;
}
...
function injectTrigger(): void {
	...
	a.addEventListener('click', async (e: Event) => {
		e.preventDefault();
		const previous = a.textContent || 'backup saved';
		try {
			await runBackup(a);
		} catch (err) {
			a.textContent = `failed: ${String((err && (err: any).message) || 'fetch')}`;
		}
		setTimeout(() => { a.textContent = previous; }, 6000);
	});

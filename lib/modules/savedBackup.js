...
import { isSafeLinkUrl } from '../utils/mediaUrl';
...
function renderRecord(record: SavedRecord): HTMLElement {
	...
	// `new URL(value, base)` does not constrain the scheme: an absolute
	// `javascript:` permalink wins over the base and becomes a click-to-run link.
	if (isSafeLinkUrl(record.permalink, location.origin)) {
...
function createManagerPanel(account: string): HTMLElement {
	...
	// This panel exists for the half that needs Reddit: the sync, and tagging an
	// item while you are looking at it. Everything else about these records —
	// every account at once, and with Reddit down — is the settings console's.
	const workspace = makeButton('Open in settings', 'rsm-savedBackup-workspace');
	workspace.title = 'Browse every account\u2019s saved content in the settings console';
	workspace.addEventListener('click', () => { SettingsNavigation.open(DATA_WORKSPACE_ROUTE); });
	toolbar.append(search, tagFilter, sync, exportButton, purge, workspace);

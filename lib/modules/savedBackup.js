...
module.description = 'Adds an account-scoped local saved-content manager in the userbar. Explicitly syncs the logged-in user\'s saved listing, indexes posts and comments on this device, and supports local search and tags. Nothing is transmitted beyond the Reddit sync request.';
...
async function openManager() {
	...
	try {
		...
		if (managerState !== state) return;
		records = loaded;
		if (managerState) managerState.status.textContent = records.length ? `Local index loaded for u/${account}.` : `No local index yet for u/${account}.`;
		renderRecords();

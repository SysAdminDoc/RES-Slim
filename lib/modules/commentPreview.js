...
function markdownToHTML(md) {
	...
	if (!isWiki) {
		...
		for (const header of headers) {
			...
			let aid = contents;
			// decode html entities without a live parse — a textarea's content model is
			// RCDATA (plain text), so markup like `<img onerror=…>` is decoded as text
			// and never instantiated as a live element (guards against wiki-heading XSS)
			const decoder = document.createElement('textarea');
			decoder.innerHTML = contents;
			aid = decoder.value;

...
export const module: Module<{ [string]: any }> = new Module('commentPreview');
...
// The renderer's output goes through `sanitizePreviewHtml` before it is written.
// See that file for what the layer is and is not: snudown escapes everything
// tried against it today, so this is defence in depth rather than a fix.
async function markdownToHTML(md) {
	const { markdown, markdownWiki } = await loadSnudown();
	if (isBan && md.length) {
		md = generateBanMessage(md, currentSubreddit() || '');
	}

	if (!isWiki) {
		return sanitizePreviewHtml(renderMarkdownWithFences(md, markdown));
	} else {
		...
		return sanitizePreviewHtml(docBody.innerHTML);
	}
}
...
async function onTextareaInput(textarea, preview, elements) {
	const markdownText = downcast(textarea, HTMLTextAreaElement).value;

	if (markdownText.length) {
		if (preview) preview.hidden = false;
		try {
			const rendered = await markdownToHTML(markdownText);
			if (downcast(textarea, HTMLTextAreaElement).value !== markdownText) return;
			for (const ele of elements) setTrustedHTML(ele, rendered);
		} catch (error) {
			console.error('Could not load the Reddit Markdown renderer:', error);
			if (downcast(textarea, HTMLTextAreaElement).value !== markdownText) return;
			for (const ele of elements) ele.textContent = 'Markdown preview unavailable. Type again to retry.';
		}
	} else {

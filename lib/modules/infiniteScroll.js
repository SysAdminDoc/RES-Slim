...
import { armOnRoute, getRouteSignal } from '../core/modules/modules';
import { findSurface } from '../core/dom/selectors';
import { isAppType, isPageType, registerPage } from '../utils';
import { waitForDescendant } from '../utils/dom';
...
// The same surface `findSurface('listingFeed', ..., 'd2x')` resolves to, named
// here because the waiter takes a selector rather than a surface id.
const D2X_FEED = 'shreddit-feed';
...
function setUpCurrentRedditLimit(signal: AbortSignal): void {
	...
	armedLimit = teardown;
	// The signal this was armed with, not whichever one is current. A limiter
	// armed after waiting for a feed to render could otherwise attach its teardown
	// to the route after the one it belongs to.
...
//
// The wait is not optional. `reddit.urlChanged` is driven fastest by the
// Navigation API's `navigatesuccess`, which fires when the URL commits — before
// the new view renders, because reddit swaps the URL first. So on arrival the
// feed is routinely not in the document yet, and a straight `findSurface` call
// here would miss it and never look again: subreddit-to-subreddit keeps
// `shreddit-app`'s `pagetype` and `routename` the same, so there is no second
// event to try on.
//
// Bounded, and on the route signal, so a listing that never renders one stops
// waiting and a wait left over from a route the reader has already left cannot
// arm the limiter onto the page they are looking at now.
function armCurrentRedditLimit(signal: AbortSignal = getRouteSignal()): void {
	if (!isPageType('linklist')) return;
	if (!module.options.limitCurrentReddit.value) return;
	if (armedLimit) return;

	if (findSurface('listingFeed', document, 'd2x')) {
		setUpCurrentRedditLimit(signal);
		return;
	}

	waitForDescendant(document.documentElement, D2X_FEED, { timeout: 10000, owner: 'infiniteScroll', signal })
		.then(() => {
			if (!isPageType('linklist')) return;
			if (!module.options.limitCurrentReddit.value) return;
			setUpCurrentRedditLimit(signal);
		}, () => { /* no feed on this route, or the reader left before one arrived */ });
}
...
module.contentStart = () => {
	...
	if (isAppType('d2x')) {
		...
		// gated separately.
		//
		// Every route, through the shared helper. This used to be a private
		// `reddit.urlChanged` listener here, which was the right shape and the wrong
		// place: the only test of it tested this module rather than the mechanism,
		// and the next module to need it would have invented the same thing again.
		armOnRoute(module.moduleID, signal => { armCurrentRedditLimit(signal); });
		return;
	}

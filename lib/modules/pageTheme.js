...
import { accentRoles, desiredThemeClasses, forcedColorsActive, sanitizeAccent, PAGE_THEME_IDS } from '../utils/pageTheme';
...
module.description = 'Give old Reddit and current Reddit the same old-Reddit layout, with a stock Classic Reddit palette plus dark alternatives, dense desktop layouts, an accent color, decluttering, optional rounded corners, and a collapse-to-hover sidebar.';
...
module.options = {
	theme: {
		...
		value: 'classic',
		title: 'Palette',
		description: 'Base color scheme for the page. Classic Reddit reproduces the stock white-and-blue interface; the remaining palettes keep the same layout in dark colors.',
		values: [
		...
		advise(value: mixed, values: { [string]: mixed }) {
			...
			return {
				message: suggestion ?
					...
					`This accent reaches only ${ratio}:1 against the palette and cannot be adjusted enough to fix, so titles and the focus outline fall back to the palette's strongest text color.`,
	...
	declutter: {
		...
		description: 'Hide ads, banners, the prompt to switch to current Reddit, gold prompts, and other clutter.',
	},
	refinedLayout: {
		type: 'boolean',
		value: true,
		title: 'Refined desktop layout',
		description: 'Rebuild current Reddit into old Reddit\'s compact vote, thumbnail and entry rows, and its flat nested comments, keeping every native control.',
	},
	roundedCorners: {
		type: 'boolean',
		value: false,
...
function clearAll() {
	// The anti-FOUC guard is applied unconditionally at document_start, and until
	// now the only two things that took it back down were `apply()` and the
	// early-blocker path in `foreground.entry.js`. Neither runs when this module
	// is simply switched off, so turning the theme off left `:root.rsm-theme-oled
	// body { background: #050608 }` on the page for good: measured in a browser at
	// rgb(5, 6, 8) with reddit's own light-page styling on top of it. The guard's
	// whole job is to cover the gap before the lifecycle decides, and "decided not
	// to theme anything" is a decision.
	dismissAntiFouc();
	// `res-nightmode` is in the same class list but belongs to nightMode, which
	// may well be on, so only this module's own marker classes come off.
	const el = root();
	if (el) {
		el.classList.remove('rsm-theme-dark', 'rsm-theme-oled');
		clearThemeClasses(el);
		for (const prop of ACCENT_PROPERTIES) el.style.removeProperty(prop);
	}
...
module.onInit = () => {
	try {
		...
		if (cached && Array.isArray(cached.classes)) {
			// The cache was written by whichever session last ran `always`, and it
			// knows nothing about the mode this one is in. A reader who turned High
			// Contrast on since then would get the refined layout replayed here — at
			// document_start, before options load, and the shadow sheet keys on the
			// same class — which is the flash of a half-erased layout the stand-down
			// exists to prevent. `always` corrects it a moment later, and a moment is
			// exactly what this replay is for.
			//
			// Only the refined gate is dropped. The palette classes are the whole
			// point of replaying early, and the UA is overriding their colours anyway.
			const forced = forcedColorsActive();
			apply(
				cached.classes.filter(c => typeof c === 'string' && c.startsWith('res-pageTheme'))
					.filter(c => !(forced && c === 'res-pageTheme--refined')),
...
function publishHeaderHeight() {
	const el = root();
	const header = findSurface('header', document, appType());
...
module.contentStart = signal => {
	...
	// Turning Windows High Contrast on or off is a live change, the same shape as
	// the system light/dark switch `systemThemeSync` already follows. Without this
	// the refined layout only stands down on the next page load, and on current
	// Reddit that can be the whole browsing session.
	if (typeof matchMedia !== 'function') return;
	try {
		const mq = matchMedia('(forced-colors: active)');
		const onChange = () => { if (typeof module.always === 'function') module.always(signal); };
		mq.addEventListener('change', onChange);
		if (signal) signal.addEventListener('abort', () => { mq.removeEventListener('change', onChange); }, { once: true });
	} catch (e) {
		/* no matchMedia here — the class list is still decided on load */
	}
};

...
export const module: Module<*> = new Module('systemThemeSync');
...
function applyNightMode(on: boolean) {
	...
	if (typeof nightMode.always === 'function') nightMode.always();
}
...
function sync() {
	...
	applyNightMode(decideNightMode(mq.matches, module.options.direction.value, nightMode.options.nightModeOn.value === true));
}

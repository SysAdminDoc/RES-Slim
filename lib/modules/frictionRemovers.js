...
function watchForLoginWall() {
	...
	wallObserver = new MutationObserver(frameThrottle(() => {
		// A wall arriving is one thing to watch for. The other is the lock being
		// re-applied over a page that has already been uncovered: reddit puts the
		// class back without touching the DOM otherwise, and `dismissLoginWall`
		// would not run for it anyway, because the wall it already hid no longer
		// covers the page. So the lock is released directly whenever it comes back
		// to a document this module has unwalled.
		if (document.documentElement.classList.contains(UNWALLED_CLASS) && isScrollLocked()) releaseScrollLock();
		dismissLoginWall();
	}));
	wallObserver.observe(document.body, { childList: true, subtree: true });
	// A second observer for the attribute. On a live reddit page the callback
	// above usually gets there first, because something in the subtree mutates
	// within a frame of almost anything; this one is what covers a page that has
	// gone quiet, where a class added to `<body>` and nothing else is invisible to
	// a `childList` observer however deep its subtree goes.
	lockObserver = new MutationObserver(frameThrottle(() => {
		if (document.documentElement.classList.contains(UNWALLED_CLASS) && isScrollLocked()) releaseScrollLock();
	}));
	for (const element of [document.documentElement, document.body]) {
		if (element) lockObserver.observe(element, { attributes: true, attributeFilter: ['class', 'style'] });
	}
}
...
const HIDE_SELECTORS = {
	...
	hideAppPrompt: [
		...
		// AdGuard carries this one as `#?##nsfw-qr-dialog:contains(Scan this QR
		// code…)`, and the text qualifier is dropped on purpose rather than by
		// oversight: a `:contains` rule works in one language, which is the failure
		// the rest of this list is shaped to avoid. The id is specific enough to
		// stand without it -- nothing else on reddit is called that -- and the cost
		// of being wrong is a hidden dialog rather than a broken page.
		'#nsfw-qr-dialog',
	],
	hideCookieBanner: [
...
// Undoing the lock means putting the reader back, not just letting the page
// move again.
//
// The lock pins the document with `position: fixed` and a negative `top` equal
// to how far down the reader was. While it is on, the document's own scroll
// offset is 0. So removing the class and deleting `top` restores scrolling and
// lands them at the very top of the feed -- which is not where they were, and is
// worse than the lock in the one way that matters. Reddit's own release path
// reads that offset back and scrolls to it; so does this.
function isScrollLocked(): boolean {
	return [document.documentElement, document.body]
		.some(element => element && SCROLL_LOCK_CLASSES.some(name => element.classList.contains(name)));
}
...
function releaseScrollLock(): void {
	...
	for (const element of [document.documentElement, document.body]) {
		if (!element) continue;
		// Read before it is removed. `parseInt` on `-420px` gives -420, and the
		// scroll position is its negation.
		const top = parseInt(element.style.top, 10);
		if (Number.isFinite(top) && top < 0) offset = Math.max(offset, -top);

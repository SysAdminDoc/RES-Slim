...
function submitWith(form: HTMLFormElement, accept: HTMLElement): void {
	...
	try {
		...
		// `NotFoundError` when it does not belong to the form. Either means the
		// markup is not the shape described above, and an uncaught throw here would
		// abort `contentStart` -- `_runModuleStage` records the module as errored,
		// so the login-wall dismisser below would never start for the rest of the
		// page. Falling through is strictly better than taking the module down.
	}
	if (typeof (accept: any).click === 'function') (accept: any).click();
	else form.submit();
}

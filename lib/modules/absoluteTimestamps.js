...
export const module: Module<*> = new Module('absoluteTimestamps');
...
module.contentStart = () => {
	document.querySelectorAll('time.live-timestamp, time').forEach(t => {
		if (t instanceof HTMLElement) decorate(t);
	});
	watchForElements(['comments', 'posts'], 'time', (ele: HTMLElement) => decorate(ele));
};

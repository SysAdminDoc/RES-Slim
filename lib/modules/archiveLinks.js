...
export const module: Module<*> = new Module('archiveLinks');

module.moduleName = 'Archive site links';
module.category = 'productivityCategory';
module.description = 'Adds Unddit, Reveddit, and Wayback Machine buttons on every comment/thread page for quickly opening the current thread in an archive.';
...
module.options = {
	unddit: {
		...
		value: true,
		title: 'Unddit',
		description: 'Add an Unddit button (undelete.pullpush.io).',
	},
	reveddit: {
		type: 'boolean',
		value: true,
...
function buildBar(): HTMLElement {
	...
	const undditHref = here.replace(/^https?:\/\/(?:old|www|np)\.reddit\.com/, 'https://undelete.pullpush.io/r');
	...
	if (module.options.unddit.value) {
		bar.append(string.html`<a href="${undditHref}" target="_blank" rel="noopener" style="margin-right: 8px;">Unddit</a>`);
	}

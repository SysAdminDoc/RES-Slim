...
module.options = {
	...
	hideCompletely: {
		...
		description: 'Remove matching posts and comments completely. Turn this off to dim them instead.',
	},
};
...
function applyHide(el: HTMLElement, reason: string): void {
	if (module.options.hideCompletely.value === false) {
		// Dimmed, not hidden: the post is still on screen and still the user's to
		// pause, so nothing is silenced here.

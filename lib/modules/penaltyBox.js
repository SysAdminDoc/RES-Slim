...
module.options = {
	...
	// `pardonAll` existed and was wired to nothing, so a host suspended by a
	// network blip the reader has since fixed stayed skipped until the backoff
	// ran out on its own, with no way to say "try again now".
	pardon: {
		type: 'button',
		title: 'Suspended hosts',
		description: 'Clears every suspension immediately, so hosts that are working again are retried on the next page instead of waiting out their backoff.',
		values: [
			{ text: 'Let every host back in', callback: pardonAllAndReport },
		],
	},
};
...
// One queue for every mutation of the record.
//
// `noteFailure` and `noteSuccess` are both read-modify-write, and the failures
// this module exists for never arrive one at a time: `linkScanner` dispatches
// every link on the page through `Promise.allSettled` and calls `noteFailure`
// without awaiting, so twenty links to one dead host produced twenty calls that
// all read the same pre-write state, all computed `failures = 1`, and all wrote
// it back. The strike count never reached the threshold and the host was never
// suspended — the exact scenario in this module's own header.
//
// Shared rather than one queue per host, because `commit` persists the whole
// record under a single key and every write is derived from the state that was
// read: two different hosts interleaving would drop one of them just as surely,
// and so would a success racing a failure.
const serialise = mutex((run: () => Promise<any>) => run());

export async function noteFailure(host: string, now: number = Date.now()): Promise<boolean> {
	if (!host) return false;

	// Only the record's read-modify-write is serialised.
	const result = await serialise(async () => {
		const state = await loadState(now);
		const decision = stateRecordFailure(state, host, now, options());
		await commit(decision.state, now);
		return decision;
	});

	// Reporting happens outside the queue. `recordModuleErrorOnce` chains on a
	// write queue shared with every other module's error log, so awaiting it in
	// here would let one slow or stuck log write block every later penalty-box
	// decision — a second dead host would go unrecorded for as long as it hung.
	if (result.suspended) {
		// The page log, unconditionally: it is in memory for the life of the tab
		// and answers "why did that link stop expanding?", which is a different
		// question from the persisted error log the option below governs.
		recordAction({
			moduleID: 'penaltyBox',
			outcome: 'penalised',
			target: host,
			reason: `strike ${result.strikes}`,
			detail: formatSuspensionMessage(host, result.until, now, result.strikes),
		});
	}
...
export function noteSuccess(host: string, now: number = Date.now()): Promise<void> {
...
export function pardonAll(): Promise<void> {
	// Through the same queue as the other two: this clears the record, and a
	// failure already queued behind it would otherwise land afterwards and
	// re-suspend a host the reader had just let back in.
	return serialise(async () => {
		cached = {};
		try {
			await storage.set({});
...
// What the settings button calls. Counts first, because after the clear there is
// nothing left to report and "nothing was suspended" is the answer a reader
// pressing this most often needs.
async function pardonAllAndReport(): Promise<void> {
	const suspended = listSuspended().length;
	await pardonAll();
	showNotification({
		moduleID: module.moduleID,
		notificationID: 'penaltyBoxPardoned',
		message: suspended ?
			`Cleared ${suspended} suspended media ${suspended === 1 ? 'host' : 'hosts'}. They will be tried again on the next page.` :
			'No media hosts were suspended.',
	});
}

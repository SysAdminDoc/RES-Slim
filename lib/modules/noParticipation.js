...
export const module: Module<{ [string]: any }> = new Module('noParticipation');
...
module.asLongAs = [() => isNpHostname(location.hostname)];
	...
	<p>
		<div class="RES-spoiler">
			...
			<div class="RES-spoiler-contents">
				You came to this page by following a <a href="${urls.moreinfo}" target="_blank" rel="noopener noreferrer">NP</a> link, so you may be interfering with normal conversation.
				Please respect reddit's <a href="https://reddit.com/rules" target="_blank" rel="noopener noreferrer">rules</a> by not commenting or voting. Violating these rules may get you banned.
				<a href="${urls.moreinfo}" target="_blank" rel="noopener noreferrer">Find out more</a>
			</div>
		</div>
	</p>
`;
...
module.contentStart = () => {
	if (module.options.escapeNP.value) {
		document.body.addEventListener('mousedown', (e: Event) => {
			...
			if (a) removeNpFromLink({ currentTarget: a });
		});
	}
...
function notifyNpIrrelevant() {
	const message = isSubscriber() ?
		`You're browsing in <a href="${urls.moreinfo}" target="_blank" rel="noopener noreferrer">No Participation</a> mode, but you're a subscriber here.` :
		`You're still browsing in <a href="${urls.moreinfo}" target="_blank" rel="noopener noreferrer">No Participation</a> mode, but it's no longer necessary.`;
...
export async function notifyNoVote(voteButton?: HTMLElement) {
	...
	if (notification) {
		const revertBtn = notification.element.querySelector('[data-action=revertvote]');
		if (revertBtn) {
...
function applyNoParticipationMode() {
	...
	document.body.addEventListener('keydown', (e: Event) => {
		if (!(e.target instanceof Element) || !e.target.matches(CommentTools.commentTextareaSelector)) return;
		notifyNoComment();
...
function watchForVote() {
	document.body.addEventListener('click', (e: Event) => {
		if (!(e.target instanceof Element) || !e.target.closest('.arrow')) return;
		Object.defineProperty(e, 'currentTarget', { value: e.target.closest('.arrow') });
...
function revertVote(voteButtons) {
	...
	Notifications.showNotification({
		...
		message: `
			...
			<p><a href="${urls.moreinfo}" target="_blank" rel="noopener noreferrer">Find out more</a></p>
		`,
	});
}

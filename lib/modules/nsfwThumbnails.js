...
import { Thing, isAppType, watchForThings } from '../utils';
...
// Current Reddit blurs with a Lit property rather than a placeholder image, so
// nothing in the document can undo it and no CSS can either. The page-world half
// hooks the custom elements that hold that property; see
// `lib/utils/shredditReveal.js`. It is a separate file because the registry it
// needs does not exist in a content script's isolated world.
const PAGE_SCRIPT = 'shredditReveal.entry.js';

let injected = false;

function injectRevealScript() {
	if (injected) return;
	if (!module.options.showNsfw.value && !module.options.showSpoilers.value) return;
	injected = true;

	const script = document.createElement('script');
	script.dataset.resSlimReveal = '';
	if (module.options.showNsfw.value) script.dataset.resSlimRevealNsfw = '1';
	if (module.options.showSpoilers.value) script.dataset.resSlimRevealSpoiler = '1';
	script.src = getURL(PAGE_SCRIPT);
	// A script with a `src` keeps loading once inserted, so the tidy-up waits for
	// the load rather than racing it.
...
module.contentStart = () => {
	if (isAppType('d2x')) {
		// `restore` reads old Reddit's placeholder markup, which current Reddit does
		// not have. Running the watcher here would walk every post to find nothing.
		injectRevealScript();
		return;
	}

...
export async function checkElementForMedia(element: HTMLAnchorElement) {
	...
	for (const siteModule of modulesForHostname(mediaUrl.hostname)) {
		...
		// A host that has been failing is skipped before any DOM or network work,
		// and the loop falls through to the next handler that claims this link
		// rather than giving up on it. Without this, a host that is down costs one
		// dead round-trip per link, on every page, indefinitely — the catch below
		// had no memory of having just done the same thing twenty times.
		// Not always the module id: a federated host counts against the instance
		// the link points at, or one dead server would stand every other one down.
		const penaltyKey = siteModule.penaltyKey ?
			siteModule.penaltyKey(mediaUrl, detectResult) :
			siteModule.moduleID;

		if (PenaltyBox.isHostSuspended(penaltyKey)) continue;
		...
		try {
			...
			PenaltyBox.noteSuccess(penaltyKey);
			recordAction({
				moduleID: 'showImages',
				outcome: 'built',
				target: mediaUrl.hostname,
				reason: siteModule.moduleID,
			});
			break;
		} catch (e) {
			recordAction({
			...
			// Not awaited: the user is waiting on the next handler, not on a storage
			// write, and a rejected write must not become this link's failure.
			PenaltyBox.noteFailure(penaltyKey).catch(() => {});

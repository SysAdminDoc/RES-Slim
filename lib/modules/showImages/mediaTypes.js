...
import { debounce, pull, without, once } from '../../utils/functional';
import type {
	ExpandoMedia,
	GalleryMedia,
	ImageMedia,
...
import { assertSafeMediaUrls } from '../../utils/mediaUrl';
import { lensSearchUrl } from '../../utils/reverseImageSearch';
...
export function generateMedia(options: ExpandoMedia, context: {| href: string |}): Media {
	...
	assertSafeMediaUrls(options, location.href);
...
export class Media {
	...
	// `keepContent` means "this is not the reader collapsing you, it is a filter
	// hiding the post you are in". Two of the implementations below destroy their
	// content on collapse, which is the right answer for a deliberate collapse the
	// reader can undo with a click, and the wrong one for a post that may be
	// unhidden a moment later — dropping a CodePen mid-edit or a filled-in poll.
	collapse(options?: {| keepContent?: boolean |}): void { this.setLoaded(false); } // eslint-disable-line no-unused-vars
	...
	// The expando is going away for good, as opposed to being collapsed.
	//
	// `_unload()` cannot stand in for this: it early-returns once the element is
	// detached, and by the time an expando is destroyed it usually is. Anything
	// held outside this object — an entry in a manager's list, a running
	// observer, an object URL, a decoder — is released here or never.
	destroy(): void {}
			...
			switch (e.target.dataset.action) {
				...
				case 'download':
					Permissions.request(['downloads']).then(() => {
						...
						return download(downloadUrl, downloadFilename(thing && thing.getTitle(), downloadUrl));
					}).catch(e => {
					...
					break;
				case 'imageLookup':
					// An engine can only fetch an absolute URL, so resolve it first.
					lookupUrl = new URL(downcast(lookupUrl, 'string'), location.href).href;

					// `images.google.com/searchbyimage` was retired and answers 404
					// (verified 2026-08-25), so this control had quietly stopped
					// working. Lens is the current entry point, and the fork already
					// builds it correctly for the reverseImageSearch module - reuse
					// that builder rather than keeping a second spelling here.
					openNewTab(lensSearchUrl(lookupUrl));
					break;
...
class Gallery extends Media {
	...
	collapse(options?: {| keepContent?: boolean |}) {
		for (const { media } of this.pieces) {
			if (media) media.collapse(options);
		}
	}
...
class Image extends Media {
	...
	constructor({
		...
		this.src = src;
		this.downloadSrc = downloadSrc || src;
		...
		// Looked up by what is on screen, saved as what the host says the file is.
		// They differ when a host shows a preview to cap the bytes an expando pulls:
		// reddit's preview is re-encoded to webp behind a URL whose path still ends
		// in `.png`, so saving it produced a `.png` full of webp.
		const wrapper = this.addControls(anchor, src, this.downloadSrc);
...
class Iframe extends Media {
	...
	collapse(options?: {| keepContent?: boolean |}) {
		...
		// Four of the thirty-five iframe hosts declare a pause command. For the
		// other thirty-one — codepen, jsfiddle, pastebin, the poll hosts, twitch,
		// instagram — removal is the only way to stop one, and it takes the
		// reader's in-progress work with it. A deliberate collapse can do that,
		// because the reader asked and re-expanding rebuilds. A filter hiding the
		// post cannot: nobody asked, and the post may come back a moment later.
		if (options && options.keepContent) return;
...
class Generic extends Media {
	...
	// unless this is a hide rather than a collapse, in which case see `Iframe`.
	collapse(options?: {| keepContent?: boolean |}) {
		if (options && options.keepContent) return;
		this.element.remove();
	}
...
class Video extends Media {
	static volumeStorage = Storage.wrapFeature('mediaVolume', 'showImages.video.volume', (1: number));
	...
	constructor({
			...
			if (this.video.canPlayType(v.type)) {
				...
				if (v.type === 'application/dash+xml') {
					...
					this.dashPlayer = loadScript('/dash.mediaplayer.min.js').then(() => {
						// v4 needed `dashjs.skipAutoCreate = true` here: the library
						// installed a DOMContentLoaded listener and adopted every
						// `[data-dashjs-player]` video on the page. v5 removed that
						// machinery outright - no listener, no scan, and no such flag -
						// so setting it now would only set an unused property on the
						// namespace while reading as a guard that is still doing work.
						...
						return {
							...
							// The manifest blob outlives `attachSource(null)`, and
							// the player keeps its own listeners on the video
							// element, so both need releasing when the expando is
							// destroyed rather than merely collapsed.
							destroy: () => { player.destroy(); URL.revokeObjectURL(url); },
						};
					});
		...
		if (!sourceElements.length) {
			if (fallback) {
				...
				// Stop here. Everything below assumes at least one source, and
				// `addControls` reads `sourceElements[0]` for the download URL —
				// so falling through threw a TypeError that `expand()` caught and
				// turned into a box that silently re-hid itself, hiding the very
				// message just written into it.
				//
				// `unplayable` is what keeps the load lifecycle away from it: with
				// no source there is nothing to restore, and `_restore()` waits on
				// `loadedmetadata`, which such an element never fires.
		...
		this.controlsObserver.observe(this.video, { attributes: true });
	...
	addVideoControls() {
		...
		if (ctrlVolume) {
			...
			// Applied at once, remembered on a delay.
			//
			// This is bound to `mousemove` while the button is held, so a one-second
			// drag issued about sixty messages to the background and sixty
			// `chrome.storage.local` writes -- for one setting, whose only reader is
			// the next video to load. Nothing in `Wrapper.set` debounces, so the
			// whole chain ran each time. Only the level the reader stopped on is
			// worth keeping.
			const persistVolume = debounce(level => { Video.volumeStorage.set(level); }, 250);
	...
	supportsUnload() {
		// Nothing to unload without a source, and unloading would invite a
		// restore that can never finish.
	...
	async _restore() {
		// A video with no playable source never fires `loadedmetadata`, so the
		// wait below would never settle — one pending promise and one live
		// listener per expand, for a box that is only showing an error.
	...
	destroy() {
		// The manager holds a strong reference to every video it observes and
		// polls each one on a 100ms interval, so a destroyed video that stays in
		// its list keeps both the element and the timer alive for the life of the
		// tab. `_unload()` releases it too, but only for an attached element, and
		// a destroyed expando is detached by the time anyone asks.
		mutedVideoManager().unobserve(this);
		...
		if (this.dashPlayer) {
			// The handle is behind a promise; the player may still be loading.
			// Either way nothing is left holding the manifest blob afterwards.
			this.dashPlayer.then(handle => { handle.destroy(); }, () => {});

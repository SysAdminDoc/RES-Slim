...
// otherwise take them away", and that is wrong twice over: `sandbox` has no
// token for any of the three, and all three are Permissions Policy features
// whose default allowlist is `self`, so a cross-origin frame has never had them.
// Granting them would have been a widening, and an autoplay grant in particular
// would let a host that starts playing by itself do so with the reader's
// `autoplayVideo` option switched off. Fullscreen still works through the

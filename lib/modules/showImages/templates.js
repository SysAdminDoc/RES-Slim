...
// Thirty-five hosts are framed by this one template, and every one of them is a
// third party running its own script inside a reddit.com page.
//
// `sandbox` withholds exactly one thing that matters: neither
// `allow-top-navigation` nor `allow-top-navigation-by-user-activation` is
// granted, so a framed player cannot navigate the tab out from under the
// reader. Everything the embeds already had is granted back explicitly, because
// a `sandbox` attribute starts from nothing: `allow-same-origin` because a
// cross-origin frame without it gets a null origin and loses its own cookies and
// storage (YouTube and Twitch simply fail), `allow-popups` plus
// `allow-popups-to-escape-sandbox` for "watch on ..." and share menus, `forms`
// for the poll hosts and the code playgrounds, `modals` for players that call
// `confirm`, `presentation` for casting, and `downloads` for the download
// buttons on the paste and playground hosts.
//
// There is deliberately no `allow` attribute. It was briefly added here listing
// autoplay, encrypted-media and picture-in-picture "because the sandbox would
// otherwise take them away", and `sandbox` has no token for any of the three, so
// it never took anything. Two of them are Permissions Policy features whose
// default allowlist is `self`, which a cross-origin frame is not, so listing
// them granted a capability rather than restoring one: an autoplay grant lets a
// host that starts playing by itself do so with the reader's `autoplayVideo`
// option switched off. Picture-in-picture defaults to `*` and so was already
// available with or without the attribute, which makes listing it pointless
// rather than harmful. Fullscreen still works through the
// `allowFullScreen` attribute below, which predates all of this.
//
// `referrerpolicy` pins what is already both browsers' default rather than
// leaving it to inherit: a document-level `<meta name="referrer">` of
// `unsafe-url` on the embedding page would otherwise hand every one of these
// hosts the full post URL. `no-referrer` would be stricter still and is not
// used, because an embed whose owner restricted it to specific domains needs
// the origin to load at all.
const IFRAME_SANDBOX = 'allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox allow-forms allow-modals allow-presentation allow-downloads';

// The one host that frames reddit's own origin.
//
// `allow-scripts` with `allow-same-origin` lets framed content take its own
// sandbox off, and that is only reachable when the frame is genuinely
// same-origin with the embedder. Thirty-four of these hosts are third parties
// and cannot be; `redditpoll` frames `www.reddit.com` from a `www.reddit.com`
// document, so it can, and the sandbox it is given means nothing.
//
// It does not need the grant. The poll page renders from the URL it is given,
// and losing `allow-same-origin` costs it a real origin -- which for a page on
// the origin the reader is already signed in to is a cost it can afford, where
// for YouTube or Twitch it is the difference between loading and not.
const SAME_ORIGIN_SANDBOX = IFRAME_SANDBOX.replace(' allow-same-origin', '');

export const iframeTemplate = ({ url, width, height, sameOrigin }: {| url: string, width: string, height: string, sameOrigin?: boolean |}) => string.html`
	<div class="res-iframe-expando">
		<div>
			<iframe src="${url}" style="width: ${width}; height: ${height}" allowFullscreen="true" sandbox="${sameOrigin ? SAME_ORIGIN_SANDBOX : IFRAME_SANDBOX}" referrerpolicy="strict-origin-when-cross-origin"></iframe>
...
export const mediaControlsTemplate = ({ x, y, downloadUrl, lookupUrl, clippy }: {| x: string, y: string, downloadUrl?: string, lookupUrl?: string, clippy: boolean |}) => string.html`
	<div class="res-media-with-controls-wrapper">
		<div class="res-media-controls res-media-controls-${x} res-media-controls-${y}" role="group" aria-label="Media controls">
			<button type="button" class="res-icon gearIcon" aria-label="Media settings" title="Settings" data-action="showImageSettings"></button>
			<button type="button" class="res-media-controls-rotate res-media-controls-rotate-left res-icon" aria-label="Rotate image counter-clockwise" title="Rotate image counter-clockwise" data-action="rotateLeft"></button>
			<button type="button" class="res-media-controls-rotate res-media-controls-rotate-right res-icon" aria-label="Rotate image clockwise" title="Rotate image clockwise" data-action="rotateRight"></button>
			${downloadUrl && string._html`
			<button type="button" class="res-media-controls-download res-icon" aria-label="Download image" title="Download image" data-action="download"></button>
			`}
			${lookupUrl && string._html`
			<button type="button" class="res-media-controls-lookup res-icon" aria-label="Reverse image search" title="Reverse image search" data-action="imageLookup"></button>
			`}
			${clippy && string._html`
			<button type="button" class="res-media-controls-clippy" aria-label="Show educational info" title="Show educational info" data-action="clippy"></button>
			`}
		</div>
	</div>
`;

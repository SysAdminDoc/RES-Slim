...
// reader. Everything the embeds actually need is granted — `allow-same-origin`
// because a cross-origin frame without it gets a null origin and loses its own
// cookies and storage (YouTube and Twitch simply fail), `allow-popups` plus
...
// `confirm`, `presentation` for casting.
//
...
const IFRAME_SANDBOX = 'allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox allow-forms allow-modals allow-presentation';
...
export const iframeTemplate = ({ url, width, height }: {| url: string, width: string, height: string |}) => string.html`
	<div class="res-iframe-expando">
		<div>
			<iframe src="${url}" style="width: ${width}; height: ${height}" allowFullscreen="true" sandbox="${IFRAME_SANDBOX}" allow="${IFRAME_ALLOW}" referrerpolicy="strict-origin-when-cross-origin"></iframe>

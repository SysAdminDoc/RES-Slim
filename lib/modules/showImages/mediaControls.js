...
// Marks the anchor as visited. `addURLToHistory` used to write into the real
// browsing history purely so the browser would repaint the link in its `:visited`
// colour; that is what the `history` permission bought, and it is now a local
// store instead. Chrome deliberately hides `:visited` from script and restricts
// which properties it may set, so the visual half has to be an explicit class —
// there is no way to make the browser paint it for us.
export const VISITED_CLASS = 'res-visited-link';

export function markLinkVisited(link: *) {
	if (link && link.classList) link.classList.add(VISITED_CLASS);
}

// Re-apply the mark to links visited on an earlier page load.
//
// Without this the whole feature is write-only: the class was only ever added at
// the moment a link was expanded, so a reload lost every mark and the local set
// accumulated entries nothing ever read. That made the replacement for the
// browser's `:visited` painting session-only, which is not a replacement at all.
//
// Cheap despite being per-link: `isURLVisited` reads through the memoised page
// cache in `environment/foreground/history`, so a listing of a hundred links
// costs one storage read, not a hundred.
export async function markVisitedIfKnown(link: *) {
	if (!module.options.markVisited.value) return;
	if (!link || !link.href || !link.classList) return;
	if (link.classList.contains(VISITED_CLASS)) return;

	if (await isURLVisited(link.href)) markLinkVisited(link);
}

...
import { compact } from '../../utils/functional';
import {
	click,
	filterMap,
} from '../../utils';
...
export class Expando {
	...
	constructor(href: *) {
		...
		//
		// Kept on the instance so `destroy()` can disconnect it. This used to be an
		// anonymous observer with no reference held: one per expando button, never
		// disconnected, on a page that appends 25 more buttons per infinite-scroll
		// page. Each one also pinned its button, so neither the observer nor the
		// detached button could be collected.
		this.buttonObserver = new MutationObserver(() => { this.updateButton(); });
		this.buttonObserver.observe(this.button, { attributes: true });
	...
	expand() {
		...
		// A site module can refuse at attach time as well as at fetch time: a
		// handler builds its media lazily here, and the URL scheme check runs at
		// this point too. Without this, a throw escaped `expand()` after the box had
		// already been revealed, so the expando sat open and empty while its button
		// still said "collapsed" and every further click threw again. Collapsing is
		// the honest outcome - the same state as never having expanded.
		try {
			this.attachMedia();
		} catch (e) {
			console.error('RES-Slim: could not build media for this expando', this.href, e);
			this.box.hidden = true;
			this.open = false;
	...
	destroy() {
		if (this.buttonObserver) {
			...
			delete this.buttonObserver;
		}
		// Before the button goes. `empty()`'s collapse branch is guarded on
		// `this.button`, so deleting it first made every destroyed expando skip
		// its own teardown — no collapse, no unload, and nothing released from
		// the muted-video manager, which then polled the dead element forever.
		this.empty();
		if (this.box) {
			this.box.remove();
			delete this.box;
	...
	empty() {
		if (this.media) {
			// Unconditional, and before the element is dropped: whatever the media
			// registered outside itself is released here or not at all.

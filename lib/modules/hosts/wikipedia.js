...
function decodeArticle(article: string): string {
	try {
		return decodeURIComponent(article);
	} catch (e) {
		// A stray `%` that is not a valid escape. Leave it as it was found.
		return article;
	}
}
...
export default new Host('wikipedia', {
	...
	// cors: measured 2026-09-02 - the REST API answers 200 with
	...
	async handleLink(href, { language, article, hash }) {
		...
		//
		// Decoding first is not optional: `URL.pathname` is *already* percent-encoded,
		// so encoding it straight turns `Café` into `Caf%25C3%25A9` and every
		// non-ASCII title - the whole of every non-English Wikipedia - into a page
		// that does not exist. Malformed input keeps its raw form rather than
		// throwing, which is no worse than what it was before.
		const page = encodeURIComponent(decodeArticle(article));

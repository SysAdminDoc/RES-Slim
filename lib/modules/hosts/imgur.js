...
export default new Host('imgur', {
	name: 'imgur',
	domains: ['imgur.com'],
	// cors: measured 2026-09-02 - api.imgur.com answers 401 without a Client-ID and
	// sends no Access-Control-Allow-Origin on that response, so what it does with a
	// real key could not be established from here. Declared rather than assumed;
	// the request only happens for a reader who has supplied their own key, and the
	// direct-image path needs no API call at all.
	permissions: ['https://api.imgur.com/3/*'],

...
export default new Host('github', {
	...
	// cors: measured 2026-09-02 - api.github.com answers 200 with
	...
	async handleLink(href, [, id]) {
		...
		let src = '';
		const markdownRenderer = Object.values(files).some(file => file.language === 'Markdown') ?
			(await loadSnudown()).markdown :
			null;
		...
		for (const [filename, { content, language, truncated }] of Object.entries(files)) {
			...
			if (language === 'Markdown') {
				src += markdownRenderer ? markdownRenderer(content) : '';
			} else {

/* @flow */
// RES-Slim: federated Mastodon embed. Detects `<instance>/@<user>/<id>`
// URLs and `<instance>/users/<user>/statuses/<id>` URLs, then asks the
// instance's own oembed endpoint for the embed HTML. Per-instance
// permissions are surfaced via `optional_host_permissions` in the manifest.

import DOMPurify from 'dompurify';
import { Host } from '../../core/host';
import { ajax } from '../../environment';
import { setTrustedHTML } from '../../core/dom/trustedHtml';
import { isPostLevelFailure, unavailableEmbed } from '../../utils/unavailableEmbed';

const MASTODON_PATTERNS = [
	/^https?:\/\/([^/]+)\/@[\w-]+(?:@[\w.-]+)?\/(\d+)(?:[?#].*)?$/i,
	/^https?:\/\/([^/]+)\/users\/[\w-]+\/statuses\/(\d+)(?:[?#].*)?$/i,
];

// Default well-known instances we ship perms for. Users running niche servers
// can add the instance under chrome.permissions in the extension settings.
const KNOWN_INSTANCES: $ReadOnlyArray<string> = Object.freeze([
	'mastodon.social',
	'mastodon.online',
	'fosstodon.org',
	'hachyderm.io',
	'mas.to',
	'infosec.exchange',
	'mstdn.social',
]);

const unavailable = () => unavailableEmbed({
	className: 'mastodon-embed mastodon-embed--unavailable',
	messageKey: 'mastodonExpandoUnavailable',
});

function detect(href: string): ?{| instance: string, id: string |} {
	if (typeof href !== 'string') return null;
	for (const re of MASTODON_PATTERNS) {
		const m = re.exec(href);
		if (m) return { instance: m[1], id: m[2] };
	}
	return null;
}

export default new Host('mastodon', {
	name: 'mastodon',
	logo: '',
	permissions: KNOWN_INSTANCES.map(host => `https://${host}/api/oembed`),
	domains: KNOWN_INSTANCES.slice(),
	detect: ({ href }) => detect(href),
	// Mastodon is not one host, it is every server in the list and any other one
	// a reader has granted. Counting failures against `mastodon` meant three dead
	// links to one instance suspended the embeds for all of them, backing off
	// toward six hours.
	penaltyKey: ({ hostname }) => `mastodon:${hostname}`,
	async handleLink(href) {
		const parsed = detect(href);
		// Nothing to show is a panel that says so, not `undefined`.
		// `completeExpando` reads `.title` off whatever comes back, so `undefined`
		// is a TypeError that the scanner records against this host -- and a run of
		// private or deleted posts then suspends an instance that is working.
		if (!parsed) return unavailable();
		const oembedUrl = `https://${parsed.instance}/api/oembed`;
		let post;
		try {
			post = await ajax({
				url: oembedUrl,
				query: { url: href },
				type: 'json',
			});
		} catch (e) {
			// A 4xx is an answer about the post: private, deleted, or behind a
			// login, and a panel saying so is right. Anything else is the instance
			// -- and a panel counts as success, so answering one would hide a dead
			// instance from the penalty box and cost a failed round trip per link on
			// every page until it came back. What the rethrow suspends is that one
			// instance; see `penaltyKey` above.
			if (!isPostLevelFailure(e)) throw e;
			return unavailable();
		}
		if (!post || typeof post !== 'object' || typeof post.html !== 'string') return unavailable();
		const dummy = document.createElement('div');
		const sanitized = DOMPurify.sanitize(post.html);
		return {
			type: 'GENERIC_EXPANDO',
			muted: true,
			expandoClass: 'selftext',
			generate: () => dummy,
			onAttach: () => { setTrustedHTML(dummy, sanitized); },
		};
	},
});

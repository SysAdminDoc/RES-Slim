...
import { parseSubredditList } from '../utils/subredditBlacklist';
...
module.contentStart = () => {
	document.body.addEventListener('mousedown', (e: Event) => {
		...
		//
		// The stored cell is one comma-separated string, and this used to split it
		// and nothing else: `askreddit, pics` applied to askreddit only, because the
		// second entry was " pics" with a leading space. A trailing comma produced
		// an empty entry that matched nothing. `parseSubredditList` is the reader
		// the blacklist already uses -- it trims, drops empties, deduplicates, and
		// accepts `r/pics`, `/r/pics` and a pasted reddit URL as the same subreddit,
		// which is how a list arrives when it is copied from somewhere else.
		const [, commentDepth, minimumComments] = (
			module.options.subredditCommentDepths.value.find(([subreddits]) => parseSubredditList(subreddits).valid.includes(subreddit)) ||

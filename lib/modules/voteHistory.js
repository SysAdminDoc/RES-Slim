...
export const module: Module<*> = new Module('voteHistory');

module.moduleName = 'Local vote / read history log';
module.category = 'productivityCategory';
module.description = 'Records every vote you cast on old.reddit to IndexedDB along with a snippet, the score-at-time, and a timestamp. Browseable + exportable. Nothing leaves the browser.';
...
function attachVoteListeners(thingEl: HTMLElement): void {
	...
	const handler = (e: MouseEvent) => {
		...
		if (arrow.classList.contains('up')) direction = arrow.classList.contains('upmod') ? 'unvote' : 'up';
		else if (arrow.classList.contains('down')) direction = arrow.classList.contains('downmod') ? 'unvote' : 'down';
		if (!direction) return;
...
function triggerJsonDownload(text: string, ext: string): void {
...
async function exportJson(): Promise<void> {
	...
	triggerJsonDownload(JSON.stringify({ schemaVersion: SCHEMA_VERSION, exportedAt: Date.now(), records }, null, 2), 'json');
}

async function exportCsv(): Promise<void> {
	const records = await allRecords();
	triggerJsonDownload(toCsv(records), 'csv');
}
...
function injectTrigger(): void {
	...
	a.addEventListener('click', async (e: MouseEvent) => {
		e.preventDefault();
		const previous = a.textContent || 'vote log';
		a.textContent = 'exporting…';
		try {
			if (e.shiftKey) await exportCsv();
			else await exportJson();
			const records = await allRecords();
			a.textContent = `✓ ${records.length}`;
		} catch (err) {
			a.textContent = 'export failed';
		}
		setTimeout(() => { a.textContent = previous; }, 4000);
	});
...
export const _internal = { classifyDirection };

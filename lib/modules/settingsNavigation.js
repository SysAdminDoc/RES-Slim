...
module.contentStart = () => {
	...
	//
	// The path, not the whole address. `reddit.urlChanged` fires on any change to
	...
	let loggedPath = location.pathname;
	document.addEventListener('reddit.urlChanged', () => {
		if (location.pathname === loggedPath) return;
		loggedPath = location.pathname;
		clearActionLog();
	});

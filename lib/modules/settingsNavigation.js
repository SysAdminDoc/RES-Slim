...
import { actionLogSize, clearActionLog, readActionLog } from '../utils/actionLog';
...
// The settings console asks from its iframe; this answers from the page.
//
// It lives in this module because this module is `alwaysEnabled`. The answer
// used to share the filter builder's preview listener, which put a panel
// reporting on four modules behind one module's switch: turn the filter builder
// off and the panel timed out and said "open the settings from a Reddit page",
// on a Reddit page, while three other modules carried on writing to the log.
//
// Only the reading half is here. Nothing in the settings bundle grows by more
// than the ring itself, which is an array and two functions.
function onActionLogRequest({ origin, data, source }: any) {
	if (origin !== getOptionsURL().origin) return;
	if (!source) return;
	if (!data || !data.requestActionLog) return;
	try {
		const limit = Number(data.requestActionLog.limit) || 200;
		source.postMessage({ actionLog: { entries: readActionLog(limit), total: actionLogSize() } }, origin);
	} catch (error) {
		// A console that closed between asking and being answered is not worth
		// reporting; the request times out on its side.
		console.warn('RES-Slim: could not answer the action log request', error);
	}
}
...
module.contentStart = () => {
	...
	window.addEventListener('message', onActionLogRequest);

	// Per page, which on current Reddit needs saying out loud. The log claims to
	// be dropped on navigation, and on old Reddit that is free -- the document is
	// replaced. Current Reddit is a single page application whose document lives
	// for the whole session, so without this the ring spans every subreddit and
	// profile the reader visited, and that is what a support report samples.
	//
	// Everything but the fragment. `reddit.urlChanged` fires on any change to
	// `location.href`, and this console changes it itself: every `load()` writes
	// the console's hash back through `setHash`, which is a `pushState`, which the
	// d2x watcher reports as a route change. So opening the settings emptied the
	// log a moment before the panel that exists to read it was told to read it,
	// and the report said nothing had happened on a page where plenty had.
	//
	// The query has to count, though. `/search/?q=alpha` to `/search/?q=beta` is a
	// different page by any measure a reader would use, and so is a flair filter
	// or a feed switch, and none of them change the path. The fragment is the only
	// part this extension writes to itself.
	const pageKey = () => `${location.pathname}${location.search}`;
	let loggedPage = pageKey();
	document.addEventListener('reddit.urlChanged', () => {
		if (pageKey() === loggedPage) return;
		loggedPage = pageKey();
		clearActionLog();
	});
...
export function parseHash(hash: string) {
	...
	// Per segment, and a segment that will not decode is kept as it was typed.
	//
	// A hand-written link to `#res:settings/%E0%A4%A` threw a URIError out of here
	// -- and this is called from a capture-phase click handler before
	// `preventDefault`, so the console never opened, the click fell through to a
	// raw hash change, and in `start()` the throw skipped `offerImportUndo()` and
	// the relative-link rewriter with it. A module id that does not decode simply
	// matches no module, which is the ordinary "unknown module" path.
	const decode = part => {
		try {
	...
	const params = path ? path.split('/').map(decode) : [];
	return {
...
export function setHash(hash: string) {
	if (window.top === window) {
		...
		// Addressed, like every other message here. The frame is the console and
		// its parent is the reddit page it was opened from.
		window.parent.postMessage({ hash }, context.origin);
	}
}
...
export function update(url: { href: string, hash: string } = location) {
	if (isSettingsUrl(url.href)) {
		...
		iframe.contentWindow.postMessage({ close: true }, getOptionsURL().origin);
	}
}
...
export function close() {
	if (isOptionsPage()) {
		window.parent.postMessage({ closing: true }, context.origin);
		window.close();
	} else {

...
function updateCurrentModuleState(mod) {
	...
	if (stateBadge) {
		...
		stateBadge.classList.toggle('is-disabled', !enabled && !mod.alwaysEnabled);
		stateBadge.classList.toggle('is-locked', mod.alwaysEnabled);
	}
...
function drawConfigOptions(mod) {
	...
	$(thisModuleName).html(`${i18n(mod.moduleName)} <span class="moduleKey" translate="no">${mod.moduleID}</span>`);
	const thisModuleCategory = RESConsoleContainer.querySelector('.moduleCategoryBadge');
	thisModuleCategory.textContent = i18n(mod.category);
	const moduleOptionSummary = RESConsoleContainer.querySelector('#moduleOptionSummary');
	moduleOptionSummary.textContent = getModuleOptionSummary(mod, configurableOptions.length, advancedOptionCount);

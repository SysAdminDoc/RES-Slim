...
async function importSettingsFromFile(file: File): Promise<void> {
	try {
		...
		showNotification({
			...
			message: `${i18n('settingsConsoleImportSuccess', moduleCount)} (${describeDiff(diff)})`,
		}, 4000);
...
function liveOptionValues(mod) {
	...
	for (const key of Object.keys(values)) {
		const radio = RESConfigPanelOptions.querySelector(`input[type="radio"][name="${CSS.escape(key)}"]:checked`);
		if (radio instanceof HTMLInputElement) {
			values[key] = radio.value;
			continue;
		}
		const input = RESConfigPanelOptions.querySelector(`#${CSS.escape(key)}`);
...
function refreshOptionAdvice() {
	...
	for (const entry of optionAdviceEntries) {
		...
		entry.action.onclick = () => {
			const input = RESConfigPanelOptions.querySelector(`#${CSS.escape(entry.optionKey)}`);
			if (input instanceof HTMLInputElement) {

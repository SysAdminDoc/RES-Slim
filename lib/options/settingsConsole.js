...
function drawConfigOptions(mod) {
	...
	if (!optCount && mod.alwaysEnabled) {
		...
		$moduleOptionsScrim = $('<div>', { id: 'moduleOptionsScrim', role: 'status' })
			.toggleClass('visible', !getModuleEnabled(mod.moduleID))
			.append(
				$('<span>', { class: 'moduleOptionsScrimIcon', 'aria-hidden': 'true' }),
				$('<strong>', { class: 'moduleOptionsScrimTitle' }).text(i18n('settingsConsoleModuleDisabledTitle')),
				$('<span>', { class: 'moduleOptionsScrimText' }).text(i18n('settingsConsoleEnableModuleScrim')),
			)

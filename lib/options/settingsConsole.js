...
function drawConfigOptions(mod) {
	...
	function addTableButtons(thisTR) {
		...
		thisDeleteButton.addEventListener('click', () => {
			...
			const valuesBefore = Array.from(tbody.children)
				.map(tr => readTableRow(tr))
...
// the row's own inputs and whatever was typed into them. Otherwise the whole
// table as it stood before the delete goes back into the staged option.
//
// The whole table rather than the one row spliced into what is staged now:
// staging is done by a frame-debounced sweep of the panel, so what it holds at
// undo time is a race, and once the panel is gone the table cannot have been
// edited since. What was read at delete time is the answer, and it cannot go
// stale.
function restoreDeletedRow({ row, tbody, moduleID, optionKey, rowIndex, valuesBefore }) {
	...
	if (!moduleID || !optionKey || !valuesBefore) return;
	...
	if (currentModule && currentModule.moduleID === moduleID) drawConfigOptions(currentModule);
}

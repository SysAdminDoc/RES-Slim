...
function isTrustedConsoleOrigin(origin: string): boolean {
	// The extension's own standalone options page posts to itself.
	if (origin === getOptionsURL().origin) return true;
	// Otherwise the only legitimate sender is the Reddit content script in the
	// embedding frame; accept any reddit.com origin (host_permissions scope).
	try {
...
export function start() {
	...
	window.addEventListener('message', ({ origin, data }) => {
		...
		if (!isTrustedConsoleOrigin(origin)) return;
		if (data.close) {
			close();
		} else if (data.load) {
...
function showConsolePrefs(show: boolean = true) {
	...
	if (show) {
		...
		// Restore whichever category tab the current module belongs to.
		syncCategoryTabs(currentModule === Search.module ? SEARCH_TAB_ID : activeCategory);
	}
}

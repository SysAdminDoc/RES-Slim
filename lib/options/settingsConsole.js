...
function drawOptionInput(mod, optionName, optionObject, isTable) {
	...
	switch (optionObject.type) {
		...
		case 'enum':
			...
			// A copy. `getOptions()` shallow-spreads, so `optionObject.values` is the
			// module's own array -- pushing the placeholder into it added a phantom
			// choice to every row drawn afterwards, kept the dropped value
			// re-selectable, and the mutation lived for the session: `search.js`'s
			// domain and `modified.js` both read the same array.
			const enumValues = [...optionObject.values];
			if (optionObject.value && !enumValues.some(({ value }) => value === optionObject.value)) {
				enumValues.push({ name: `${optionObject.value} (not available)`, value: optionObject.value });
			}
		...
		case 'select':
			...
			// A copy, for the same reason as the enum above.
			const selectValues = [...optionObject.values];
			if (optionObject.value && !selectValues.some(({ value }) => value === optionObject.value)) {
				selectValues.push({ name: `${optionObject.value} (not available)`, value: optionObject.value });
			}
...
async function toggleModuleEnabled(moduleID: string) {
	...
	if (enable) {
		...
		if (permissions.length && !await Permissions.has(permissions)) {
			...
			if (!await Permissions.has(permissions)) {
				// A declined prompt, or a prompt tab the reader closed, which resolves
				// the same way. Returning silently left the switch where it was with
				// nothing said, so the only thing the reader learned was that the
				// toggle does not work.
				settingsStatus(
					i18n('settingsConsolePermissionDeclined', i18n(mod.moduleName) || moduleID, permissions.join(', ')),
					{ isError: true, closeDelay: 8000 },
				);
				// And the switch goes back to where it was, because it never moved.
				syncSidebarModuleState(moduleID, getModuleEnabled(moduleID));

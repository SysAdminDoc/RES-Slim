...
// Stored values change on a save, an import, a reset and a restore, and every
// one of those already refreshes the chips.
...
function isTextEntry(target: EventTarget | null): boolean {
	...
	// A keycode field is two inputs: a hidden one holding the value and a
	// `displayonly` one showing "Ctrl+K". Clearing the visible half leaves the
	// stored half untouched, so the field reads empty while still holding a
	// shortcut, until something redraws it. Nothing the reader types goes into it
	// either -- it is filled from a keydown -- so it is not text entry.
	if (target.getAttribute('displayonly') === 'true') return false;

...
async function resetSettingsToDefaults(): Promise<void> {
	try {
		...
		// Same reason as import: every module has to re-read its options, and the
		// undo affordance cannot survive the reload, so it is offered on next load
		// from the persisted restore point.
		setTimeout(() => { location.reload(); }, 600);
	} catch (e) {
		const reason = e instanceof Error ? e.message : String(e);

...
function create() {
	...
	$(RESConsoleContainer).find('#RESConfigPanelModulesPane')
		...
		.on('click', '.categoryButton', function(e: Event) {
			const id = $(this).closest('.RESConfigPanelCategory').data('category');
			if (id) {
				e.preventDefault();
...
function drawSettingsConsole() {
	...
	async function toggleModuleEnabledState() {
		...
		$moduleButton.toggleClass('enabled', enable);
		updateCurrentModuleState(Modules.get(moduleID));
		updateSaveButton();
	}

	saveButton = ((RESConsoleContainer.querySelector('#moduleOptionsSave'): any): HTMLButtonElement);
...
function saveCurrentModuleOptions() {
	...
	try {
		...
		// Commit can throw from a module.onSaveSettings or Modules.setEnabled hook.
		// Log it, leave the stage intact so the user can retry, and surface the
		// failure instead of showing "All changes saved" over unsaved state.
...
function updateSaveButton() {
	...
	const stageStatus = RESConsoleContainer.querySelector('#workspaceStageStatus');
	...
	if (stageStatus) {
		stageStatus.textContent = getWorkspaceStageSummary(changeCount, scopeCount);
		stageStatus.classList.remove('is-saved-pulse');
		stageStatus.classList.toggle('is-dirty', unsavedOptions);
		stageStatus.classList.toggle('is-saved', !unsavedOptions);
	}
...
function openCategoryPanel(category) {
	...
	items.not(selected).removeClass('active').find('.categoryButton').attr('aria-expanded', 'false');
	selected.addClass('active').find('.categoryButton').attr('aria-expanded', 'true');
}
...
function notifyOptionsSaved() {
	const stageStatus = RESConsoleContainer.querySelector('#workspaceStageStatus');
	if (stageStatus) {
		stageStatus.textContent = 'Saved just now';
		stageStatus.classList.remove('is-dirty');
		stageStatus.classList.add('is-saved', 'is-saved-pulse');
		clearTimeout(saveStatusTimer);

...
function drawConfigOptions(mod) {
	...
	function addTableButtons(thisTR) {
		...
		thisDeleteButton.addEventListener('click', () => {
			...
			const rowValues = readTableRow(thisTR);
...
function restoreDeletedRow({ row, tbody, moduleID, optionKey, rowIndex, rowValues }) {
	...
	if (document.contains(tbody)) {
		tbody.insertBefore(row, tbody.children[rowIndex] || null);
	...
	if (!moduleID || !optionKey || !rowValues) return;
	...
	values.splice(Math.min(rowIndex, values.length), 0, rowValues);
	...
	if (currentModule && currentModule.moduleID === moduleID) {
		...
		const restored = RESConfigPanelOptions.querySelector(`#tbody_${optionKey} tr:nth-child(${rowIndex + 1})`);

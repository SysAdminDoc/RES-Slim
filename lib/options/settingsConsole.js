...
import { getModified } from '../core/options/modified';
import {
...
import {
	...
	DATA_WORKSPACE_ROUTE,
	DATA_WORKSPACE_TAB_ID,
	SEARCH_TAB_ID,
...
import { requestActionLog } from '../utils/actionLogBridge';
import { isTrustedConsoleOrigin } from '../utils/trustedOrigin';
import { describeRequestedAccess } from '../utils/permissionSummary';
...
import {
	...
	readSelectorOverrides,
	saveSelectorOverrides,
} from '../core/dom/selectorOverrideStorage';
import {
...
import { renderableValues } from './optionValues';
import * as DataWorkspace from './dataWorkspace';
import { markdown } from './snudown';
import { buildSupportDump } from './supportDump';
import { categoryTabsTemplate, consoleContainerTemplate, moduleSelectorTemplate } from './templates';

const DEFAULT_MODULE_ID = 'nightMode';
const SEARCH_MODULE_ID = 'search';
const SETTINGS_NAVIGATION_MODULE_ID = 'settingsNavigation';

const searchModule = () => Modules.get(SEARCH_MODULE_ID);
const settingsNavigationModule = () => Modules.get(SETTINGS_NAVIGATION_MODULE_ID);
...
let categoryTabsEl;
// Which of the console's two utility pages is open, if either. The module
// workspace is what shows when this is null.
let openUtilityPage: ?string = null;
...
// Which action left the restore point, so the undo notice after the reload can
// name it. Deliberately `localStorage` rather than extension storage: it is
// console-local presentation state that must not outlive the page or end up in
// an exported snapshot, and the write has to survive exactly one reload.
const SETTINGS_RESTORE_REASON_KEY = 'res-settings-restore-reason';
const RESTORE_REASON_RESET = 'reset';
const RESTORE_REASON_IMPORT = 'import';

function writeRestoreReason(reason: string) {
	try {
		localStorage.setItem(SETTINGS_RESTORE_REASON_KEY, reason);
	} catch (e) {
		// A blocked write costs the notice its wording, not its function.
	}
}

// Read once and cleared, so a later visit to the settings page does not describe
// an action from three sessions ago.
function readRestoreReason(): ?string {
	try {
		const reason = localStorage.getItem(SETTINGS_RESTORE_REASON_KEY);
		localStorage.removeItem(SETTINGS_RESTORE_REASON_KEY);
		return reason;
	} catch (e) {
		return null;
	}
...
// One chip in the header, replaced in place.
//
// Every one of these used to go through `showNotification`, which prepends a
// card and deduplicates only on identical HTML. So clicking through the theme
// picker stacked one three-row card per click -- five were on screen after ten
// clicks in four seconds, covering the Advanced panel's switch and the support
// report's Copy button at 1440x900 -- and each card carried an "Always show this
// type" footer offering to disable a notification type that is really just this
// console talking to itself, plus a `settingsConsole` row written into the
// notifications table option on first use.
//
// The console already has two `role="status"` lines of its own for the data
// workspace and the selector editor. This is the third, for everything else.
let settingsStatusTimer;

function settingsStatus(message: string, { closeDelay = 2400, isError = false }: {| closeDelay?: number, isError?: boolean |} = {}) {
	const chip = RESConsoleContainer && RESConsoleContainer.querySelector('#RESConsoleStatus');
	if (!(chip instanceof HTMLElement)) return;
	clearTimeout(settingsStatusTimer);
	// Unhidden first, and the text set on the next frame. A live region populated
	// while it is `display: none` is outside the accessibility tree, and screen
	// readers commonly do not announce the change when it is revealed afterwards.
	chip.hidden = false;
	chip.classList.toggle('is-error', isError);
	requestAnimationFrame(() => { chip.textContent = message; });
...
function setSelectorOverrideStatus(message: string, isError: boolean = false) {
	const status = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSelectorOverrideStatus');
	const editor = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSelectorOverrideEditor');
	if (status instanceof HTMLElement) {
		status.textContent = message;
		status.classList.toggle('is-error', isError);
	}
	if (editor instanceof HTMLTextAreaElement) editor.setAttribute('aria-invalid', isError ? 'true' : 'false');
}

async function refreshSelectorOverrideEditor(message?: string) {
	const editor = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSelectorOverrideEditor');
	if (!(editor instanceof HTMLTextAreaElement)) return;
	const overrides = await readSelectorOverrides();
	editor.value = serializeSelectorOverrides(overrides);
	const count = selectorOverrideCount(overrides);
	setSelectorOverrideStatus(message || `Bundle ${SELECTOR_BUNDLE_VERSION}. ${count} overridden surface${count === 1 ? '' : 's'} saved.`);
}

async function saveSelectorOverrideEditor() {
	const editor = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSelectorOverrideEditor');
	if (!(editor instanceof HTMLTextAreaElement)) return;
	try {
		const saved = await saveSelectorOverrides(parseSelectorOverrides(editor.value));
		editor.value = serializeSelectorOverrides(saved);
		const count = selectorOverrideCount(saved);
		setSelectorOverrideStatus(`Saved ${count} overridden surface${count === 1 ? '' : 's'}. Reload open Reddit tabs to apply it.`);
		settingsStatus('Selector overrides saved.');
	} catch (error) {
		setSelectorOverrideStatus(error instanceof Error ? error.message : 'The selector override could not be saved.', true);
	}
}

async function exportSelectorOverridesToFile() {
	const overrides = await readSelectorOverrides();
	downloadText(
		serializeSelectorOverrides(overrides),
		`res-slim-selector-overrides-${SELECTOR_BUNDLE_VERSION}.json`,
	);
	setSelectorOverrideStatus('Selector overrides exported.');
}

async function importSelectorOverridesFromFile(file: File) {
	const parsed = parseSelectorOverrides(await (file: any).text());
	const saved = await saveSelectorOverrides(parsed);
	const editor = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSelectorOverrideEditor');
	if (editor instanceof HTMLTextAreaElement) editor.value = serializeSelectorOverrides(saved);
	setSelectorOverrideStatus(`Imported ${selectorOverrideCount(saved)} overridden surface${selectorOverrideCount(saved) === 1 ? '' : 's'}. Reload open Reddit tabs to apply it.`);
	settingsStatus('Selector overrides imported.');
}

// What the extension did to the page this console was opened from.
//
// The log lives in the reddit page, so this asks for it across the frame
// boundary the same way the filter preview does. There is deliberately no
// polling: a table that rewrites itself under the reader while they are reading
// it is worse than one with a Refresh button.
let actionLogEntries = [];
// What the page had recorded, which is not what it sent: the ask is capped well
// below the ring, so counting the rows in hand says "200 of 200" on a session
// that recorded four hundred.
let actionLogTotal = 0;
// Nothing is asked for until the reader asks. The panel is one of several on the
// tab and most opens of the settings never look at it, so fetching on create
// pulled two hundred rows into this document for nothing.
let actionLogFetched = false;

function actionLogElements() {
	if (!RESConsoleContainer) return null;
	const list = RESConsoleContainer.querySelector('#RESActionLogList');
	const status = RESConsoleContainer.querySelector('#RESActionLogStatus');
	const filter = RESConsoleContainer.querySelector('#RESActionLogFilter');
	if (!(list instanceof HTMLElement) || !(status instanceof HTMLElement) || !(filter instanceof HTMLSelectElement)) return null;
	return { list, status, filter };
}

function renderActionLog() {
	const parts = actionLogElements();
	if (!parts) return;
	const { list, status, filter } = parts;

	const wanted = filter.value;
	const rows = wanted === '*' ? actionLogEntries : actionLogEntries.filter(entry => entry.moduleID === wanted);

	list.replaceChildren();
	if (!rows.length) {
		status.textContent = i18n(actionLogFetched ? 'settingsConsoleActionLogEmpty' : 'settingsConsoleActionLogIdle');
		return;
	}

	const table = document.createElement('table');
	table.className = 'actionLogTable';
	const head = document.createElement('thead');
	const headRow = document.createElement('tr');
	for (const label of ['Time', 'Module', 'What', 'Where', 'Why']) {
		const cell = document.createElement('th');
		// `setAttribute` rather than the property: Flow's 2018 DOM has no `scope`
		// on `HTMLTableCellElement`, and the attribute is what a screen reader reads.
	...
	for (const entry of rows) {
		...
		// `textContent` for every cell. These strings come from the page: a
		// subreddit name, a rule id the reader wrote, a host. None of it is markup
		// and none of it is going to be treated as any.
		const cells = [
	...
	table.append(body);
	list.append(table);
	status.textContent = i18n('settingsConsoleActionLogCount', String(rows.length), String(actionLogTotal));
}

async function refreshActionLog() {
	const parts = actionLogElements();
	if (!parts) return;
	const { status, filter } = parts;
	status.textContent = '…';

	// The page behind the console is Reddit, not this extension: a request posted
	// to our own origin would never be delivered, and never rejected either.
	const reply = await requestActionLog(context.origin);
	actionLogFetched = true;
	if (!reply) {
		// The standalone options page has no reddit document behind it. Saying so
		// is better than an empty table, which reads as "nothing happened".
		actionLogEntries = [];
		actionLogTotal = 0;
		renderActionLog();
		status.textContent = i18n('settingsConsoleActionLogNoPage');
		return;
	}

	actionLogEntries = reply.entries;
	actionLogTotal = reply.total;

	// The module list is rebuilt from what is actually in the log, so it can
	// never offer a module that recorded nothing.
	const modules = [...new Set(actionLogEntries.map(entry => entry.moduleID))].sort();
	const previous = filter.value;
	filter.replaceChildren();
	const all = document.createElement('option');
	all.value = '*';
	all.textContent = i18n('settingsConsoleFilterAll');
	filter.append(all);
	...
	filter.value = modules.includes(previous) ? previous : '*';

	renderActionLog();
}

function wireActionLog() {
	const parts = actionLogElements();
	if (!parts) return;
	const refresh = RESConsoleContainer.querySelector('#RESActionLogRefresh');
	if (refresh instanceof HTMLElement) refresh.addEventListener('click', () => { refreshActionLog(); });
	parts.filter.addEventListener('change', renderActionLog);
	renderActionLog();
}

function wireSelectorOverrideEditor() {
	const save = RESConsoleContainer.querySelector('#RESSelectorOverrideSave');
	const exportButton = RESConsoleContainer.querySelector('#RESSelectorOverrideExport');
	const importButton = RESConsoleContainer.querySelector('#RESSelectorOverrideImport');
	const reset = RESConsoleContainer.querySelector('#RESSelectorOverrideReset');
	const fileInput: ?HTMLInputElement = (RESConsoleContainer.querySelector('#RESSelectorOverrideFile'): any);
	const editor = RESConsoleContainer.querySelector('#RESSelectorOverrideEditor');

	if (save instanceof HTMLButtonElement) save.addEventListener('click', () => { saveSelectorOverrideEditor(); }, true);
	if (exportButton instanceof HTMLButtonElement) exportButton.addEventListener('click', () => { exportSelectorOverridesToFile().catch(error => setSelectorOverrideStatus(String(error), true)); }, true);
	if (importButton instanceof HTMLButtonElement && fileInput instanceof HTMLInputElement) {
		importButton.addEventListener('click', () => fileInput.click(), true);
		fileInput.addEventListener('change', () => {
			const file = fileInput.files && fileInput.files[0];
			if (file) importSelectorOverridesFromFile(file).catch(error => setSelectorOverrideStatus(error instanceof Error ? error.message : String(error), true));
			fileInput.value = '';
		}, true);
	}
	if (reset instanceof HTMLButtonElement) {
		reset.addEventListener('click', async () => {
			await clearSelectorOverrides();
			await refreshSelectorOverrideEditor('Bundled selectors restored. Reload open Reddit tabs to apply them.');
			settingsStatus('Bundled selectors restored.');
		}, true);
	}
	if (editor instanceof HTMLTextAreaElement) {
		editor.addEventListener('input', () => {
			editor.setAttribute('aria-invalid', 'false');
			const status = RESConsoleContainer.querySelector('#RESSelectorOverrideStatus');
			if (status instanceof HTMLElement) {
				status.textContent = 'Unsaved selector changes.';
				status.classList.remove('is-error');
			}
		});
	}
	refreshSelectorOverrideEditor().catch(error => setSelectorOverrideStatus(String(error), true));
}
...
async function copyModuleErrorLog() {
	...
	try {
		...
		settingsStatus(i18n('settingsConsoleErrorsCopySuccess'));
	} catch (e) {
		output.focus();
		output.select();
		settingsStatus(i18n('settingsConsoleErrorsCopyFailed'), { closeDelay: 6000, isError: true });
	}
}

let supportDumpPending = false;

async function buildSupportReport() {
	const output = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSupportDumpOutput');
	const status = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSupportDumpStatus');
	const build = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSupportDumpBuild');
	const copy = RESConsoleContainer && RESConsoleContainer.querySelector('#RESSupportDumpCopy');
	if (!(output instanceof HTMLTextAreaElement)) return;

	// The page half of this waits on a postMessage round trip, so a second press
	// while the first is in flight would race two writes into the same textarea.
	if (supportDumpPending) return;
	supportDumpPending = true;
	if (build instanceof HTMLButtonElement) build.disabled = true;
	if (status instanceof HTMLElement) status.textContent = i18n('settingsConsoleSupportBuilding');

	try {
		output.value = await buildSupportDump();
		if (copy instanceof HTMLButtonElement) copy.disabled = false;
		if (status instanceof HTMLElement) status.textContent = i18n('settingsConsoleSupportReady');
	} catch (e) {
		output.value = '';
		output.placeholder = i18n('settingsConsoleSupportFailed');
		if (copy instanceof HTMLButtonElement) copy.disabled = true;
		if (status instanceof HTMLElement) status.textContent = i18n('settingsConsoleSupportFailed');
		console.error('RES-Slim: could not build the support report', e);
	} finally {
		supportDumpPending = false;
		if (build instanceof HTMLButtonElement) build.disabled = false;
	}
}
...
async function copySupportReport() {
	...
	try {
		if (!navigator.clipboard || typeof navigator.clipboard.writeText !== 'function') throw new Error('Clipboard API unavailable');
		await navigator.clipboard.writeText(output.value);
		settingsStatus(i18n('settingsConsoleSupportCopySuccess'));
	} catch (e) {
		output.focus();
		output.select();
		settingsStatus(i18n('settingsConsoleSupportCopyFailed'), { closeDelay: 6000, isError: true });
	}
}
...
async function clearModuleErrors() {
	try {
		...
		settingsStatus(i18n('settingsConsoleErrorsCleared'));
	} catch (e) {
		console.error('RES-Slim: could not clear module error log', e);
		settingsStatus(i18n('settingsConsoleErrorsClearFailed'), { closeDelay: 6000, isError: true });
	}
}
...
// The reader's choice, which may be `system`. Kept so the media-query listener
// knows whether a change in the desktop's colour scheme is any of its business.
let settingsThemeChoice: SettingsTheme = DEFAULT_SETTINGS_THEME;

function paintSettingsTheme(choice: SettingsTheme) {
	// The attribute takes the resolved preset, never the choice: every theme
	// block in options.scss is keyed on a preset id, so writing `system` there
	// paints neither of the two themes it stands for.
	const resolved = resolveSettingsTheme(choice, systemPrefersDark());
	document.documentElement.dataset.settingsTheme = resolved;
	updateThemeColorMeta(resolved);
	syncThemeSelector(choice);
}

function applySettingsTheme(theme: SettingsTheme, persist: boolean = true) {
	settingsThemeChoice = normalizeSettingsTheme(theme);
	paintSettingsTheme(settingsThemeChoice);
	if (!persist) return;
	try {
		localStorage.setItem(SETTINGS_THEME_STORAGE_KEY, settingsThemeChoice);
	} catch (e) {
		// Ignore storage failures; the theme can still be applied for this session.
	}
	// No message for a theme change: the picker's selected state is the feedback,
	// and the page repaints in front of the reader. Saying it again in words was
	// the single biggest source of the stack.
}

// Follow the desktop live. Without this, `system` would be read once at startup
// and a reader switching their OS to dark in the evening would keep the light
// console until they reloaded the page. Nothing is persisted here — the stored
// value is still `system`, and the resolved preset is derived every time.
function watchSystemColorScheme() {
	if (typeof matchMedia !== 'function') return;
	let query;
	try {
		query = matchMedia(SETTINGS_THEME_DARK_QUERY);
	} catch (e) {
		return;
	}
	if (!query) return;
	const repaint = () => {
		if (!isSystemSettingsTheme(settingsThemeChoice)) return;
		paintSettingsTheme(settingsThemeChoice);
	};
	// Safari below 14 has no `addEventListener` on a MediaQueryList, and this
	// bundle is also loaded by the Firefox build; `addListener` is the fallback
	// every one of them has.
	if (typeof query.addEventListener === 'function') query.addEventListener('change', repaint);
	else if (typeof query.addListener === 'function') query.addListener(repaint);
}
...
function applySettingsDensity(density: SettingsDensity, persist: boolean = true) {
	...
	settingsStatus(i18n(nextDensity === SETTINGS_DENSITY_DENSE ? 'settingsConsoleToastDensityDense' : 'settingsConsoleToastDensityComfortable'));
}
...
function applySettingsMotion(motion: SettingsMotion, persist: boolean = true) {
	...
	settingsStatus(i18n(nextMotion === SETTINGS_MOTION_REDUCE ? 'settingsConsoleToastMotionReduced' : 'settingsConsoleToastMotionSystem'));
}

async function exportSettingsToFile(): Promise<void> {
	try {
		const snapshot = await buildSnapshot({ appVersion: Metadata.version });
		downloadText(serializeSnapshot(snapshot), suggestedFilename(snapshot));
...
// Import, reset and the restore-point undo all replace every module blob and
// then reload, so that each module re-reads its options from storage.
//
// The staged edits have to go first. A dirty stage makes `handleBeforeUnload`
// return a string, which raises the browser's "Leave site?" dialog on the
// reload; Cancel then leaves the page rendering the values from before the
// import on top of storage that already holds the ones from after it, with
// nothing on screen saying the two disagree. They are also meaningless by then:
// a staged edit names a value in a profile that no longer exists.
//
// The delay is for the toast, which is the only thing that says what happened.
let profileWriteReloadPending = false;

function scheduleReloadAfterProfileWrite() {
	// Clearing the stage is not enough on its own. The console re-stages from the
	// live form on a frame-debounced `input`/`change` sweep, and the fields on
	// screen still hold the pre-import values -- so any event in the 600ms window,
	// down to the notification rendering, puts the edits straight back and arms
	// the guard again. Caught by the import test under load, having passed on a
	// quiet machine.
	// The reload is armed first, and the guard stands down after it. The other
	// order meant a throw in any of the four calls below left the flag set with no
	// reload coming, and `handleBeforeUnload` returned early for the rest of the
	// page's life -- the guard this exists to arm, dead again, silently.
	setTimeout(() => { location.reload(); }, 600);
	profileWriteReloadPending = true;
	// A row from the profile being replaced has nowhere to go back to.
	cancelPendingRowDeletions();
	Options.stage.reset();
	updateSaveButton();
...
async function importSettingsFromFile(file: File): Promise<void> {
	try {
		...
		writeRestoreReason(RESTORE_REASON_IMPORT);
		// The undo affordance cannot live in this toast because the reload destroys
		// it, so the restore point is persisted and offered on next load.
		scheduleReloadAfterProfileWrite();
	} catch (e) {
...
// Undoing 60 toggles by hand is not an option a person actually has, and six of
// the enabled-by-default modules have no visible control at all — so without
// this, a user who dislikes the defaults has no route back to them either.
//
// Expressed as an ordinary snapshot and applied through the same guarded path as
// an import, which is what buys it the restore point and the rollback-on-failure
// for free. A bespoke "delete all the keys" routine would have neither.
async function resetSettingsToDefaults(): Promise<void> {
	try {
		const snapshot = await buildDefaultsSnapshot({ appVersion: Metadata.version });
		const { diff } = await applySnapshotGuarded(snapshot, { appVersion: Metadata.version });
		showNotification({
			moduleID: 'settingsBackup',
			message: `${i18n('settingsConsoleResetSuccess')} (${describeDiff(diff)})`,
		}, 4000);
		writeRestoreReason(RESTORE_REASON_RESET);
		// Same as import: the undo affordance cannot survive the reload, so it is
		// offered on next load from the persisted restore point.
		scheduleReloadAfterProfileWrite();
	} catch (e) {
		const reason = e instanceof Error ? e.message : String(e);
		showNotification({ moduleID: 'settingsBackup', message: i18n('settingsConsoleResetFailed', reason) }, 8000);
		throw e;
	}
}

// Offered once, after the reload an import or a reset triggers. Without this the
// guarded apply would take a backup nobody could ever reach.
async function offerImportUndo(): Promise<void> {
	...
	// Both the import and the reset paths land here after their reload, so the
	// notice has to name whichever one happened. The reason is written just before
	// the reload; if it is missing — a restore point left by an older build, or a
	// browser that refused the write — the wording falls back to something true of
	// either rather than guessing.
	const wasReset = readRestoreReason() === RESTORE_REASON_RESET;

	const wrapper = document.createElement('div');
	const summary = document.createElement('p');
	summary.style.margin = '0 0 8px';
	summary.textContent = wasReset ?
		'Settings were reset to their defaults. The state from before the reset is still saved.' :
		'Settings were changed. The state from before that change is still saved.';
	wrapper.append(summary);

	const undoBtn = document.createElement('button');
	undoBtn.type = 'button';
	undoBtn.className = 'RESNotificationButtonBlue';
	undoBtn.textContent = wasReset ? 'Undo reset' : 'Undo';
	undoBtn.addEventListener('click', async () => {
		undoBtn.disabled = true;
		undoBtn.textContent = 'Restoring…';
		try {
			const { moduleCount } = await revertToRestorePoint();
			showNotification({ moduleID: 'settingsBackup', message: `Restored ${moduleCount} module${moduleCount === 1 ? '' : 's'} from before that change.` }, 4000);
			scheduleReloadAfterProfileWrite();
		} catch (e) {
			const reason = e instanceof Error ? e.message : String(e);
			showNotification({ moduleID: 'settingsBackup', message: `Could not restore the previous settings: ${reason}` }, 8000);
		}
	}, { once: true });
	...
	keepBtn.type = 'button';
	keepBtn.textContent = wasReset ? 'Keep defaults' : 'Keep changes';
	...
	showNotification({ moduleID: 'settingsBackup', header: wasReset ? 'Settings reset' : 'Settings import', message: wrapper });
}
...
function loadFromHash() {
	...
	load(moduleID, optionKey);
	if (isNarrowViewport()) {
		setSidebarCollapsed(moduleID ? moduleID !== SEARCH_MODULE_ID : true);
	}
}

function load(moduleID, optionKey) {
	if (moduleID === CONSOLE_PREFS_ROUTE || moduleID === DATA_WORKSPACE_ROUTE) {
		// Deep-linking straight to a utility tab still needs a populated sidebar
		// behind it, so draw the default module first.
		if (!currentModule) load(DEFAULT_MODULE_ID);
		showUtilityPage(moduleID);
		SettingsNavigation.setHash(SettingsNavigation.makeUrlHash(moduleID));
		return;
	}
	showUtilityPage(null);
	...
	// It resolved to the first Appearance module with the dead id still sitting in
	// the URL and nothing said, so the reader is looking at Night Mode believing
	// it is the thing they asked for.
	//
	// Searching for the name is the useful answer to "that is not here any more",
	// and it leaves a hash that means something. The name is said out loud too,
	// because a page that quietly shows you something else is the problem.
	if (moduleID && !requested) {
		settingsStatus(i18n('settingsConsoleUnknownModule', moduleID), { isError: true, closeDelay: 8000 });
		load(SEARCH_MODULE_ID, moduleID);
		return;
	}

	const mod = requested || Modules.get(DEFAULT_MODULE_ID);
...
function showUtilityPage(route: ?string) {
	if (openUtilityPage === route) return;
	openUtilityPage = route;

	const prefs = RESConsoleContainer.querySelector('#RESConsolePrefs');
	const data = RESConsoleContainer.querySelector('#RESDataWorkspace');
	const workspace = RESConsoleContainer.querySelector('#RESModuleWorkspace');
	if (prefs instanceof HTMLElement) prefs.hidden = route !== CONSOLE_PREFS_ROUTE;
	if (data instanceof HTMLElement) data.hidden = route !== DATA_WORKSPACE_ROUTE;
	if (workspace instanceof HTMLElement) workspace.hidden = route !== null;
	RESConsoleContainer.classList.toggle('is-console-prefs', route === CONSOLE_PREFS_ROUTE);
	RESConsoleContainer.classList.toggle('is-data-workspace', route === DATA_WORKSPACE_ROUTE);

	if (route === CONSOLE_PREFS_ROUTE) {
	...
	} else if (route === DATA_WORKSPACE_ROUTE) {
		updateConsoleBreadcrumb(i18n('dataWorkspaceTitle'));
		syncCategoryTabs(DATA_WORKSPACE_TAB_ID);
		if (data instanceof HTMLElement) data.setAttribute('aria-labelledby', 'RESCategoryTab-data');
		DataWorkspace.open();
	} else if (currentModule) {
		// `currentModule` does not change while a utility page is open. Returning
		// to that same module therefore skips drawConfigOptions(), so the
		// breadcrumb used to remain stuck even though the workspace and tab had
		// switched back. Restore both pieces of header state here.
		const isSearchWorkspace = currentModule === searchModule();
...
function highlightOption(mod, optionKey) {
	...
	requestAnimationFrame(() => {
		...
		if (optionElement.classList.contains('advanced') && !settingsNavigationModule().options.showAllOptions.value) {
			RESConsoleContainer.classList.add('advanced-options-enabled');
			// Deliberately a notification card rather than the status chip: 222
			// characters of marked-up explanation that has to stay up until the
			// reader has read it, which is not what a one-line transient chip is
			// for. `noDisable` because offering to silence a one-off explanation of
			// what just happened is not a setting anyone wants.
			showNotification({
				moduleID: 'settingsConsole',
				notificationID: 'advanced-link-opened',
				message: i18n('settingsConsoleAdvancedLinkOpenedNotice'),
				noDisable: true,
			}, Infinity);
		}
...
function create() {
	...
	RESConsoleContainer = consoleContainerTemplate({
		...
		showAllOptions: settingsNavigationModule().options.showAllOptions.value,
	});
	...
	// A message written for `confirm()`, which honours the blank line between its
	// two sentences. Interpolated into the modal as one string it renders as a
	// single run-on paragraph, so the paragraphs are rebuilt from the text.
	const promptBody = message => {
		const body = document.createElement('div');
		for (const paragraph of String(message).split(/\n{2,}/)) {
	...
	const resetButton = RESConsoleContainer.querySelector('#RESSettingsReset');
	if (resetButton instanceof HTMLButtonElement) {
		resetButton.addEventListener('click', async () => {
			// The one destructive control on this page, and the only one where a
			// misclick costs work the user cannot get back by clicking again. It is
			// undoable, but the undo lives behind a reload, so the cheap guard goes
			// in front of it as well.
			//
			// The console's own modal rather than `confirm()`. Inside the embedded
			// console the browser titles that dialog "An embedded page on this page
			// says", which reads as something other than this extension asking, and
			// it was the one native dialog on a page whose rule is to act and offer
			// an undo. `Alert.open` is focus-trapped, Escape-cancellable, and
			// rejects when the reader says no.
			try {
				await Alert.open(promptBody(i18n('settingsConsoleResetConfirm')), { cancelable: true });
			} catch (e) {
				return;
			}
			resetSettingsToDefaults().catch(() => {});
		}, true);
	}

	wireSelectorOverrideEditor();
	DataWorkspace.wire(RESConsoleContainer);
	...
	// Deliberately not built on open: nothing in the report is gathered until the
	// user asks for it.
	const supportBuild = RESConsoleContainer.querySelector('#RESSupportDumpBuild');
	if (supportBuild instanceof HTMLButtonElement) supportBuild.addEventListener('click', () => { buildSupportReport(); }, true);
	const supportCopy = RESConsoleContainer.querySelector('#RESSupportDumpCopy');
	if (supportCopy instanceof HTMLButtonElement) supportCopy.addEventListener('click', () => { copySupportReport(); }, true);

	wireActionLog();

	const RESAdvOptionsSpan = RESConsoleContainer.querySelector('#RESAllOptionsSpan');
	RESAdvOptionsSpan.setAttribute('title', i18n(settingsNavigationModule().options.showAllOptions.description));

	const RESAdvOptions: HTMLInputElement = (RESAdvOptionsSpan.querySelector('input'): any);
	RESAdvOptions.addEventListener('change', () => {
		settingsNavigationModule().options.showAllOptions.value = RESAdvOptions.checked;
		Options.save(settingsNavigationModule().options.showAllOptions);
		RESConsoleContainer.classList.toggle('advanced-options-enabled', RESAdvOptions.checked);
		if (currentModule === searchModule()) Search.search();
	}, true);
	...
	const search = () => {
		...
		if (!query) {
			if (currentModule === searchModule() && lastNonSearchModule) {
				load(lastNonSearchModule.moduleID);
			}
			return;
		}
		load(SEARCH_MODULE_ID, rawQuery);
	};
...
function activateTab(category: ?string) {
	...
	if (category === DATA_WORKSPACE_TAB_ID) {
		load(DATA_WORKSPACE_ROUTE);
		return;
	}
	if (category === SEARCH_TAB_ID) {
		load(SEARCH_MODULE_ID, Search.input().value.trim() || undefined);
		return;
	}
...
function updateSelectedModule(mod) {
	...
	const isSearchWorkspace = mod === searchModule();
...
// What the "Modified" chip counts.
//
// It counted staged changes alone, so saving them emptied the filter into "No
// modules match" -- the one moment a reader is most likely to be looking at it.
// A module is modified if it differs from what the extension ships with, or if
// there are unsaved edits against it. The first half is what `getModified`
// already computes for the support report; the second is what the chip used to
// be, and dropping it would take the chip away from a reader who has edits open.
//
// The item asked for `getModified()` alone. That reads the stored value, so a
// module the reader has only just changed would not be listed until they saved,
// which is the same complaint pointing the other way.
function isModuleModified(moduleID) {
	return getModuleStageCount(moduleID) > 0 || modifiedModuleIDs().has(moduleID);
}

let modifiedModuleIDsCache = null;

function modifiedModuleIDs(): Set<string> {
	if (!modifiedModuleIDsCache) modifiedModuleIDsCache = new Set(getModified().map(({ moduleID }) => moduleID));
	return modifiedModuleIDsCache;
}

// Stored values change on a save, an import, a reset and a restore, and nowhere
// else. Clearing this on every chip update instead put a walk of all 116 modules
// back on the per-keystroke path that had just been cleared off it.
function forgetModifiedModules() {
	modifiedModuleIDsCache = null;
}
...
function drawOptionInput(mod, optionName, optionObject, isTable) {
	...
	switch (optionObject.type) {
		case 'textarea':
			// `rows` and no spellcheck because the three options that use this hold
			// JSON and pasted lists, not prose. `textContent` rather than
			// `innerHTML` with an escape: the value is a string being put into a
			// text node, so there is no markup to escape and no round trip to get
			// wrong.
			thisOptionFormEle = _ce('textarea', {
				id: domId,
				type: 'textarea',
				moduleID: mod.moduleID,
				rows: String(optionObject.rows || 8),
				spellcheck: 'false',
			});
			thisOptionFormEle.textContent = String(optionObject.value ?? '');
			break;
		case 'list':
		case 'text':
		case 'hidden':
			...
			// A list is comma separated and nothing on screen said so, which is how
			// `askreddit pics` and `askreddit;pics` get typed into it. `listType` has
			// been carried on these options since they were introduced and read by
			// nothing; it is what says which example to show.
			if (optionObject.type === 'list') {
				const placeholderKey = LIST_PLACEHOLDER_KEYS[optionObject.listType];
				if (placeholderKey) thisOptionFormEle.setAttribute('placeholder', i18n(placeholderKey));
			}
			break;
		case 'color':
			...
			break;
		case 'button': {
			const { callback, text } = optionObject;
		...
		case 'password':
			...
			break;
		case 'boolean': {
			// Dispatched from the toggle itself. It used to be dispatched on
			// `RESConsoleContainer`, which worked only while the staging sweep
			// listened there too: an event dispatched on an ancestor never reaches a
			// listener on a descendant, so once the sweep moved to the options panel
			// every boolean flipped on screen and was silently discarded. Bubbling
			// from the control is what the rest of the panel already does.
		...
		case 'enum':
			...
			const enumValues = renderableValues(optionObject);
			enumValues.forEach((optionValue, index) => {
		...
		case 'select':
			...
			const selectValues = renderableValues(optionObject);
			selectValues.forEach((optionValue, index) => {
				const thisId = `${domId}-${index}`;
...
async function toggleModuleEnabled(moduleID: string) {
	...
	if (enable) {
		...
		if (permissions.length && !await Permissions.has(permissions)) {
			...
			try {
				...
				// `Permissions.request` rejects on a decline, and on a prompt tab the
				// reader closed, which the background resolves the same way. It used
				// to reject straight out of this function into no handler at all: the
				// switch did not move and nothing was said, so the only thing the
				// reader learned is that the toggle appears not to work, and the
				// obvious next move is to click it again and get the same prompt.
				//
				// Reading the result of `Permissions.has` instead does not work, and
				// was the first attempt at this: `request` never returns on a refusal,
				// so nothing after it runs.
				settingsStatus(
					i18n('settingsConsolePermissionDeclined', i18n(mod.moduleName) || moduleID, describeRequestedAccess(permissions)),
					{ isError: true, closeDelay: 8000 },
				);
				return;
			}
		}
	...
	updateFilterChipCounts();
	applyModuleFilter();
	settingsStatus(i18n(enable ? 'settingsConsoleToastModuleEnabled' : 'settingsConsoleToastModuleDisabled', i18n(mod.moduleName)));
}
...
function drawSettingsConsole() {
	...
	// Handle Escape before a search input's native keyup can move focus. If the
	// keyup lands on <body>, the console cannot tell it began inside a field and
	// closes with the user's staged edits still in memory.
	document.body.addEventListener('keydown', handleEscapeKey);
	window.addEventListener('beforeunload', handleBeforeUnload);
	// On the options panel, not on the whole console. `stageCurrentModuleOptions`
	// only ever reads `RESConfigPanelOptions`, so a keystroke in the data
	// workspace's search box or the selector editor could not stage anything --
	// it just swept every table, builder and input on the module panel and then
	// walked all 116 module buttons, once per character.
	RESConfigPanelOptions.addEventListener('input', autostageDebounce);
	RESConfigPanelOptions.addEventListener('change', autostageDebounce);
	...
	RESConfigPanelOptions.addEventListener('input', refreshOptionAdvice);
	RESConfigPanelOptions.addEventListener('change', refreshOptionAdvice);
}
...
async function saveAllStagedOptions() {
	...
	try {
		...
		// Stored values just changed, so what counts as modified has too.
		forgetModifiedModules();
	} catch (e) {
...
function discardAllStagedOptions() {
	...
	// The redraw below puts every deleted row back from stored values, so a
	// pending undo would add a second copy of one.
	cancelPendingRowDeletions();
	Options.stage.reset();
	...
	updateFilterChipCounts();
	applyModuleFilter();
	if (hadStaged) settingsStatus(i18n('settingsConsoleToastReverted'));
}
...
function drawConfigOptions(mod) {
	...
	function addTableButtons(thisTR) {
		...
		thisDeleteButton.addEventListener('click', () => {
			...
			const table = thisTR.closest('table.optionsTable');
			const container = thisTR.closest('.optionContainer');

			// Read before the row goes, and kept for the undo: five seconds is long
			// enough to leave for another module, and the row cannot be read back
			// out of a panel that has been torn down.
			const rowIndex = [...tbody.children].indexOf(thisTR);
			// Where the row sits in the *staged value*, which is not where it sits
			// in the table: the sweep drops rows whose cells are all blank, so a
			// half-typed row above this one shifts every later index by one.
			const stagedIndex = [...tbody.children].slice(0, rowIndex)
				.filter(tr => { const values = readTableRow(tr); return Array.isArray(values) && values.length > 0; })
				.length;
			const deletion = {
				moduleID: table && table.getAttribute('moduleID'),
				optionKey: table && table.getAttribute('optionName'),
				index: stagedIndex,
				domIndex: rowIndex,
				rowValues: readTableRow(thisTR),
			};
			pendingRowDeletions.add(deletion);

			// `remove()` takes the focused button off the page with it, which drops
			// focus to `<body>`: a keyboard reader deleting rows loses their place
			// after every one. Chosen before the removal, while the neighbours are
			// still there to choose from.
			const neighbour = thisTR.nextElementSibling || thisTR.previousElementSibling;
			const nextFocus = (neighbour && neighbour.querySelector('.deleteButton')) ||
				(container && container.querySelector('.addRowButton'));

			thisTR.dispatchEvent(new Event('change', { bubbles: true }));
			thisTR.remove();
			// Staged now rather than on the next frame. The undo has to be able to
			// read what this option holds without the deletion still being in a
			// debounce queue, and the sweep is idempotent, so running it twice costs
			// a pass over the panel and nothing else.
			stageCurrentModuleOptions();
			if (nextFocus instanceof HTMLElement) nextFocus.focus();

			CreateElement.undo(i18n('settingsConsoleRestoreDeletedRow')).then(() => {
				restoreDeletedRow({ row: thisTR, tbody, deletion });
			});
		});
		...
		// The name is short because it is read on every row. What the control does
		// goes in the title, which becomes the accessible description when there is
		// an `aria-label`, so it is announced after the name rather than instead of
		// it.
		thisHandle.title = i18n('settingsConsoleMoveRowHint');
		thisHandle.setAttribute('aria-label', i18n('settingsConsoleMoveRow'));
		// Enter and Space still do nothing here, because this is not an activation
		// control. Naming the keys that do work is the standard way to say so.
		thisHandle.setAttribute('aria-keyshortcuts', 'ArrowUp ArrowDown');
...
// One row of a table option, read as the values it holds. `null` when every cell
// is blank, which is how a half-typed new row stays out of the staged value.
//
// Also what the delete button captures before the row goes: an undo that arrives
// after the console has drawn another module has no row left to read.
function readTableRow(row: HTMLElement) {
	...
	return notAllBlank ? optionRow : null;
}

// Putting a deleted row back.
//
// The undo button lives on `document.body` for five seconds, which is long
// enough to click another module in the rail. The console tears the whole
// options panel down when it draws one, so the `tbody` the row came out of is
// then detached: appending to it restored the row into nothing at all, staged
// nothing, and took the button away as though it had worked.
//
// The element goes back when its table is still on screen, because that keeps
// the row's own inputs and whatever was typed into them. Otherwise the one row
// is put back into whatever the option holds now, at the index it came from.
//
// Into what it holds now, and not a snapshot of the whole table taken at delete
// time. That was the first attempt and it lost edits: the undo button lives for
// five seconds, so a reader can delete a row, fix a typo in another row and then
// leave, and writing the snapshot back discarded the typo fix. Two deletes and
// two undos could also drop a row outright, because the second snapshot was
// taken after the first delete.
//
// What made a snapshot look necessary was that staging runs on a frame-debounced
// sweep, so the staged value at delete time was a race. The delete stages itself
// synchronously instead, which is where that belonged.
// Deletions still waiting on their undo button, which lives for five seconds and
// stays as long as the pointer is on it. An undo has to know what has happened
// since, and two things can:
//
// Discard puts every deleted row back on its own -- it clears the stage and
// redraws the panel from stored values -- so a pending undo has nothing left to
// do, and splicing the row in anyway added a second copy. Import, reset and the
// restore-point undo replace the whole profile, where a row from the old one has
// no meaning at all.
//
// And restoring one row moves every row below it down, so any other deletion
// still waiting has to have its position adjusted or the two come back in the
// wrong order. Which order depends on which button the reader presses first.
// Two positions, because the two restore paths count differently: `domIndex` is
// the row's place in the table, `index` its place in the staged value, and the
// sweep drops all-blank rows from the second but not the first.
const pendingRowDeletions: Set<{ moduleID: ?string, optionKey: ?string, index: number, domIndex: number, rowValues: mixed }> = new Set();

function cancelPendingRowDeletions() {
	pendingRowDeletions.clear();
}

function restoreDeletedRow({ row, tbody, deletion }) {
	const wasPending = pendingRowDeletions.delete(deletion);
	// `document.contains` rather than `isConnected`: Flow 0.84 ships the 2018 DOM
	// and does not know the property.
	// Whether the table is still on screen or not, the rows below this one move
	// down by one, so anything else still waiting to be restored has moved with
	// them.
	const shiftOthers = () => {
		for (const other of pendingRowDeletions) {
			if (other.moduleID !== deletion.moduleID || other.optionKey !== deletion.optionKey) continue;
			if (other.index >= deletion.index) other.index += 1;
			if (other.domIndex >= deletion.domIndex) other.domIndex += 1;
		}
	};

	if (document.contains(tbody)) {
		tbody.insertBefore(row, tbody.children[deletion.domIndex] || null);
		tbody.dispatchEvent(new Event('change', { bubbles: true }));
		if (wasPending) shiftOthers();
		// The undo button has already removed itself by the time this runs, so
		// focus is on `<body>` unless something takes it -- the same place the
		// delete used to leave it.
		focusRowControl(row);
		return;
	}

	const { moduleID, optionKey, index, rowValues } = deletion;
	// Not pending any more means Discard or a profile write already put it back.
	if (!wasPending || !moduleID || !optionKey || !rowValues) return;

	const option = Modules.get(moduleID).options[optionKey];
	const staged = Options.stage.get(moduleID);
	const current = staged && staged[optionKey] ? staged[optionKey].value : option.value;
	const values = Array.isArray(current) ? [...current] : [];
	values.splice(Math.min(index, values.length), 0, rowValues);
	shiftOthers();
	// The same sort the sweep applies, or the restored value would be ordered
	// differently from one the reader had produced by hand.
	if (typeof option.sort === 'function') values.sort(option.sort);

	Options.stage.add(moduleID, optionKey, values);
	updateSaveButton();
	updateModuleStageMarkers();
	updateFilterChipCounts();
	// And the panel is redrawn from the staged value, so a reader who came back
	// before pressing undo sees the row rather than only the save button change.
	if (currentModule && currentModule.moduleID === moduleID) {
		drawConfigOptions(currentModule);
		const restored = RESConfigPanelOptions.querySelector(`#tbody_${optionKey} tr:nth-child(${index + 1})`);
		if (restored) focusRowControl(restored);
	}
}

// Where focus goes when a row appears or disappears under a keyboard reader.
// Its own delete button if it has one, so the next Enter deletes the row they
// are looking at.
function focusRowControl(row) {
	const target = row.querySelector('.deleteButton') || row.querySelector('button, input, select, textarea');
	if (target instanceof HTMLElement) target.focus();
}

// An example rather than a sentence: the format of a comma-separated list is
// clearer from two entries than from any description of it.
const LIST_PLACEHOLDER_KEYS = { subreddits: 'settingsConsoleListSubredditsPlaceholder' };
...
function handleEscapeKey(event: KeyboardEvent) {
	...
	// A keycode field is waiting for a keystroke to bind, and Escape over that
	// prompt means cancel it. Blurring is what ends the capture -- there is a
	// blur listener on the panel that closes the prompt -- and it leaves the
	// field's value alone, which is the whole reason Escape stopped treating it
	// as text entry. Without this the console closed instead, and the capture was
	// still live when the keystroke reached it.
	const escapeTarget = event.target;
	if (escapeTarget instanceof HTMLElement && escapeTarget.matches('input[displayonly]')) {
	...
	if (isTextEntry(target)) {
		...
		event.preventDefault();
		// Clear the edit, publish the same input event as an ordinary edit so the
		// search workspace follows it, then step focus out. A second Escape now
		// belongs to the console and closes it.
		if (!field.isContentEditable && field.value) {
			field.value = '';
...
function handleBeforeUnload(event: Event) {
	// An import, a reset or a restore already replaced every module blob and is
	// reloading on purpose. There is nothing left on this page worth keeping, and
	// asking about it would be asking whether to abandon changes that no longer
	// exist.
	if (profileWriteReloadPending) return;
	if (!Options.stage.isDirty()) return;

	// This returned the confirmation string and nothing else, which armed nothing:
	// `addEventListener` ignores what a listener returns. Only an `onbeforeunload`
	// *property* ever honoured a returned string, and this has been a listener for
	// as long as it has existed -- so the console's unsaved-changes guard has
	// never once stopped a navigation. Verified in the bundled Chromium: a
	// listener that returns a string does not prompt, and one that calls
	// `preventDefault()` both prompts and cancels the reload.
	//
	// `returnValue` is the legacy spelling, kept because Safari still needs it.
	// The string itself is ignored by every current browser, which shows its own
	// wording; it stays sourced from the same locale key as the in-page
	// confirmation so the two cannot drift.
	event.preventDefault();
	(event: any).returnValue = getAbandonChangesConfirmation();
}
...
function notifyCloseBlockedByUnsavedChanges() {
	...
	// No chip here. `globalStageBar` above is a `role="status"` sibling of the
	// chip and has just been given this exact sentence; writing it into both
	// shows it twice on screen and announces it twice to a screen reader.
...
function applyModuleFilter() {
	...
	for (const row of rows) {
		...
		const modified = isModuleModified(moduleID);
		let show = true;
...
function updateFilterChipCounts() {
	...
	for (const m of scoped) {
		...
		if (isModuleModified(m.moduleID)) modified++;
	}
...
function renderDriftRecords(list, records) {
	list.replaceChildren(...records.map(record => {
		...
		heading.textContent = describeDriftScope(record.pageType);
	...
	if (copy instanceof HTMLElement && !copy.dataset.wired) {
		...
		copy.addEventListener('click', async () => {
			...
			try {
				...
				list.after(area);
				area.select();
				if (status instanceof HTMLElement) status.textContent = 'Clipboard unavailable. The report is selected above, so copy it by hand.';
			}
		});
	}

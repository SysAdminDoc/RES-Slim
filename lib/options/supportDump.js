...
export async function buildSupportDump(now: number = Date.now()): Promise<string> {
	const [diagnostics, errors, drift, pageLog] = await Promise.all([
		requestPageDiagnostics(),
		getModuleErrorLog().catch(() => []),
		readSelectorDrift().catch(() => ({})),
		// The same frame the timings come from, asked at the same moment, so the
		// report describes one page rather than two states of it.
		// The reddit page, not this extension's origin: a request posted to our own
		// would never be delivered and never rejected either.
		requestActionLog(context.origin).catch(() => null),
	]);
	...
	return formatSupportDump({
		...
		errors,
		drift,
		pageActions: pageLog ? pageLog.entries : null,
		pageActionsTotal: pageLog ? pageLog.total : null,
	});
}

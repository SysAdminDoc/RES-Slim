...
import { canPersistFeatureData, i18n } from '../environment';
...
import { frameDebounce } from '../utils/async';
import {
...
type DataSet = {|
	id: string,
	label: string,
	// The `canPersistFeatureData` policy this set is governed by. The database is
	// the extension's, and a private window shares it with the normal profile, so
	// a set the policy blocks must not be readable from one.
	featureId: string,
...
const SETS: DataSet[] = [
	...
	},
	{
		...
		load: () => readRecords('subredditEmotes'),
		accountOf: null,
		matches: (record, query) => text(record.subreddit).toLowerCase().includes(query),
		row: record => ({
			title: `r/${text(record.subreddit)}`,
			meta: [
				`${Object.keys(record.emotes || {}).length} emoji`,
				dateLabel(record.fetchedAt),
			].filter(Boolean).join(' · '),
		}),
		exportName: 'res-slim-subreddit-emoji',
		exportPayload: records => ({ schemaVersion: 1, exportedAt: Date.now(), maps: records }),
		remove: records => writeRecords('subredditEmotes', [], records.map(record => record.subreddit)),
		restore: records => writeRecords('subredditEmotes', records),
	},
	{
		id: 'mediaManifest',
...
// The empty state is a row of the list, so `replaceChildren` takes it out along
// with the rows and a later `find` cannot get it back. Held here instead, and
// dropped when a new console is wired.
let emptyRow = null;

function renderRows() {
	const list = find('#RESDataWorkspaceRows');
	if (!emptyRow) emptyRow = find('#RESDataWorkspaceEmpty');
	...
	if (emptyRow) {
		emptyRow.hidden = state.visible.length > 0;
		// Put back into the list `replaceChildren` above just emptied. It lives
		// inside the box rather than under it: an empty bordered panel with a line
		// of text below it reads as two things, one of them broken.
		if (!emptyRow.hidden) list.append(emptyRow);
	}
}
...
async function reload() {
	const set = getDataSet(state.setId);
	if (!canPersistFeatureData((set.featureId: any))) {
		state.records = [];
		renderAccounts();
		applyFilters();
		setStatus(i18n('dataWorkspacePrivate', set.label), 'error');
	...
	try {
		...
		// The same two refreshers the success path runs. Without them the account
		// picker keeps offering the *previous* set's accounts over an empty list,
		// and the import panel stays where the last set left it.
		renderAccounts();
		applyFilters();
		syncImportVisibility();
		return;
...
export function wire(root: HTMLElement) {
	container = root;
	// A new console means a new element; the old one belongs to a document that
	// is on its way out.
	emptyRow = null;
	...
	if (search instanceof HTMLInputElement) {
		// Filtering rebuilds up to 200 rows. Held to one rebuild per frame, so
		// holding a key down costs one render rather than one per character.
		const filterSoon = frameDebounce(applyFilters);
		search.addEventListener('input', () => { state.query = search.value; disarmPurge(); filterSoon(); });
	}

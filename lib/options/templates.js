	...
	<div id="RESConsoleContainer" class="${showAllOptions ? 'advanced-options-enabled' : ''}">
		...
		<header id="RESConsoleHeader">
			<div id="RESConsoleTopBar" class="RESDialogTopBar">
				...
				<div class="consoleHeaderActions">
					<div id="RESConsoleStatus" class="consoleStatusChip" role="status" aria-live="polite" hidden></div>
		...
		<div id="RESConsoleContent">
			...
			<main id="RESConfigPanelOptions">
				...
				<section id="RESModuleWorkspace" class="workspaceShell" role="tabpanel" aria-labelledby="RESModuleWorkspaceName">
					<header class="moduleHeader">
						...
						<div class="moduleHeaderTopRow">
							<div class="moduleHeaderCopy">
								<h2 id="RESModuleWorkspaceName" class="moduleName">${i18n('settingsConsoleModuleNamePlaceholder')}</h2>
			...
			<section id="RESConsolePrefs" class="consolePrefsShell" role="tabpanel" hidden>
				...
				<div class="consolePrefsGrid">
					...
					<!--
						What the extension did to the page you came from. A reader cannot
						otherwise tell which filter hid a post, which expando was refused
						or which host is in the penalty box: each is a decision made
						silently, and the most concrete thing readers asked for this
						quarter was a way to check them.
					-->
					<section id="RESActionLogPanel" class="utilityPanel utilityPanel--actions" aria-labelledby="RESActionLogTitle">
						<div class="utilityPanelHeader">
							<span id="RESActionLogTitle" class="utilityPanelTitle">${i18n('settingsConsoleActionLogTitle')}</span>
							<span class="utilityPanelMeta">${i18n('settingsConsoleActionLogMeta')}</span>
						</div>
						<div class="utilityActionStack consoleControlGroup" role="group" aria-label="${i18n('settingsConsoleActionLogGroup')}">
							<button id="RESActionLogRefresh" type="button" class="settingsIoButton">${i18n('settingsConsoleActionLogRefresh')}</button>
							<label class="utilityPanelFilter" for="RESActionLogFilter">
								${i18n('settingsConsoleActionLogFilterLabel')}
								<select id="RESActionLogFilter" class="select"></select>
							</label>
						</div>
						<div id="RESActionLogList" class="actionLogList"></div>
						<p id="RESActionLogStatus" class="moduleErrorLogStatus" role="status" aria-live="polite"></p>
					</section>
			...
			<section id="RESDataWorkspace" class="dataWorkspaceShell" role="tabpanel" hidden>
				...
				<ul id="RESDataWorkspaceRows" class="dataWorkspaceRows">
					<li id="RESDataWorkspaceEmpty" class="dataWorkspaceEmpty" role="status" hidden>${i18n('dataWorkspaceEmpty')}</li>
				</ul>
					...
					${modules.map(({ isEnabled, alwaysEnabled, moduleID, translatedName, description, shortDescription }) => string._html`
						<li class="moduleRow ${isEnabled ? 'is-enabled' : ''} ${alwaysEnabled ? 'is-locked' : ''}" data-module-id="${moduleID}">
							...
							${alwaysEnabled ? string._html`
								<!--
									The same token every other row shows. This borrowed the filter
									chip's label, which is capitalised because it heads a filter, so
									one row in the rail read "On" beside "on" on all the others.
									That it cannot be switched off is carried by the name, the
									tooltip and the is-locked class, which is where it belongs.
									(No backticks in here: this sits inside a template literal.)
								-->
								<span class="moduleRowLock" aria-label="${i18n('settingsConsoleStateAlwaysOn')}" title="${i18n('settingsConsoleStateAlwaysOn')}">${i18n('toggleOn')}</span>
							` : string._html`

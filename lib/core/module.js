...
export class Module<RawOpt: { [string]: any }, Opt: { [string]: ModuleOption<RawOpt> } = RawOpt> /*:: extends Indexable */ {
	...
	// The signal a stage is handed is not always the same lifetime, and the
	// difference matters.
	//
	// On the ordinary page load it is the module lifecycle signal: as long as the
	// document, ending when the document really goes away rather than when it is
	// put into the back/forward cache.
	//
	// On a route change it is the signal for *that route*. `_runNewlyEligibleStage`
	// offers a page stage to a module whose `include` has only now started
	// matching, and hands it `getRouteSignal()`, which is aborted and replaced the
	// next time the reader navigates. So work started from a stage that ran
	// because of a route change ends with that route, and the stage will not be
	// offered again -- a module that already ran a stage never gets it twice,
	// because that is how you end up with two of every observer it made.
	//
	// A module that wants to do something on every route, rather than once, asks
	// `armOnRoute` in `core/modules/modules.js` instead of relying on the stage.
	//
	// no default value for cleaner profiling (modules without a stage defined won't be timed)
	onInit: ((signal: AbortSignal) => Promise<void> | void) | void = undefined;
	beforeLoad: ((signal: AbortSignal) => Promise<void> | void) | void = undefined;
	contentStart: ((signal: AbortSignal) => Promise<void> | void) | void = undefined;
	go: ((signal: AbortSignal) => Promise<void> | void) | void = undefined;
	afterLoad: ((signal: AbortSignal) => Promise<void> | void) | void = undefined;
	always: ((signal: AbortSignal) => Promise<void> | void) | void = undefined;
...
type CommonOptionProps<Ctx> = {|
	...
	onChange?: () => void,
	// Inline advice rendered under the control in the settings console.
	//
	// Unlike `onChange` this *is* passed the value, and the module's other live
	// values with it, because the whole point is judging combinations the option
	// cannot see on its own — an accent colour is syntactically valid at any
	// darkness and only the palette chosen above it decides whether it is
	// readable. Returning null means nothing to say. The suggestion is offered,
	// never applied: the user picked that value on purpose.
	advise?: (value: any, values: { [string]: any }) => ?{|
		message: string,
...
// A `text` whose value is not a word. The three that carry one are a nested rule
// set, a pasted tag map and a pasted subreddit list, all of them multi-kilobyte
// and two of them line-delimited, and a single-line `<input>` showed the first
// forty characters and scrolled the rest out of sight. Same stored string, same
// staging path; the only difference is the element the console draws.
export type TextareaOption<Ctx> = {|
	type: 'textarea',
	value: string,
	rows?: number,
	...CommonOptionProps<Ctx>,

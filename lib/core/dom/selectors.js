...
export function normalizeSelectorOverrides(raw: mixed, syntaxProbe?: (string) => mixed): SelectorOverrides {
	...
	for (const appType of Object.keys(input.selectors)) {
		if (!Object.keys(baseSurfaceMaps).includes(appType)) throw new InvalidSelectorOverrideError(`Unknown renderer: ${appType}.`);
		...
		for (const surfaceName of Object.keys(rendererInput)) {
			if (!Object.keys(baseSurfaceMaps[appType]).includes(surfaceName)) throw new InvalidSelectorOverrideError(`Unknown ${appType} surface: ${surfaceName}.`);
			...
			const effectiveStable = surfaceOutput.stable || baseSurfaceMaps[appType][surfaceName].stable;
			const effectiveFallback = surfaceOutput.fallback || baseSurfaceMaps[appType][surfaceName].fallback;
			const duplicate = effectiveFallback.find(selector => effectiveStable.includes(selector));
			if (duplicate) throw new InvalidSelectorOverrideError(`selectors.${appType}.${surfaceName} repeats "${duplicate}" across stable and fallback.`);

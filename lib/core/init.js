...
if (typeof document !== 'undefined' && document.addEventListener) {
	document.addEventListener('reddit.urlChanged', () => {
		_startRouteScope();
		// Every route-derived memo, not only the page type. The other five --
		// subreddit, multireddit, domain, profile, quarantine -- used to be cleared
		// from a promise callback, which is a microtask after this listener has
		// already run the `beforeLoad` pass below against the route just left.
		// `currentLocation` clears them on this same event too; doing it here as
		// well means the order these two listeners were registered in is not
		// something a reader of this file has to know.
		clearLocationCaches();
...
afterLoad.then(() => {
	// Assigning the function is free; calling it is not. The summary copies the
	// whole measure buffer and runs a regex over every entry, and this ran on
	// every page load in production to decide whether to print a console warning
	// nobody had asked for. The reader who wants it can call
	// `window.rsmDiagnostics()`, and the support dump asks for it explicitly.

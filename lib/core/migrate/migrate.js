...
import { sameColor } from '../../utils/cssColor';
import { takeRightWhile, isObject, mapKeys, uniqBy } from '../../utils/functional';
...
const migrations = [
	{
		...
		async go() {
			await Migrators.forceUpdateOption('stylesheet', 'loadStylesheets', rows =>
				rows && rows.filter(row => row[0] !== 'https://cdn.example.invalid/updates.css'),
			);
		...
		async go() {
			...
			const remappedEntries = Object.entries(hiddenThings).reduce((acc, [k, v]) => {
				...
				const collapsedThings = v.reduce((acc, k) => { acc[k] = true; return acc; }, {});
		...
		async go() {
			// This used to condense the per-version `RES.firstRun.*` keys into a
			// single `highestVersion`. Upstream read that key to decide whether to
			// announce an update; this fork stripped the reader and kept the writer,
			// so it sat in every migrated profile's storage being consulted by
			// nothing. The update notice is driven by `onInstalled.previousVersion`
			// instead, which is knowable without keeping any state at all.
			//
			// The cleanup half is the part that was ever load-bearing, and it stays.
			const firstRunStorage = Storage.wrapPrefix('RES.firstRun.', () => (null: null | boolean));
			await firstRunStorage.deleteMultiple(Object.keys(await firstRunStorage.getAll()));
		},
	},
...
export async function migrate() {
	...
	await forEachSeq(migrations, async currentMigration => {
		...
		// Checkpoint, in case the next migration crashes. Must be awaited: an MV3
		// service worker can be torn down between iterations, and migrate() runs on
		// every worker start — an unawaited write means the checkpoint may never
		// land and already-applied migrations replay on the next wake.
		await Storage.set('RESOptionsVersion', currentMigration.versionNumber);
	});
...
async function getMigrationsToRun() {
	...
	if (lastMigratedVersion === false) {
		...
		// Awaited for the same reason as the checkpoint above.
		await Storage.set('RESOptionsVersion', migrations.slice(-1)[0].versionNumber);
		return [];
	}
...
function colorsMatch(fg, bg) {
	if (fg === bg) return true;
	if (!fg || !bg) return false;

	// `sameColor` replaced tinycolor2 here; see lib/utils/cssColor.js for why the
	// named-colour case is deliberately out of scope.
	return sameColor(fg, bg);
}

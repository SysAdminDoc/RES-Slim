...
import { isEqual } from '../../utils/functional';
...
export function _loadModuleOptions() {
	...
	return Promise.all(Modules.all().map(async module => {
		...
		for (const [key, storedValue] of Object.entries(storedOptions)) {
			...
			//
			// `Object.hasOwn` rather than truthiness: `module.options['__proto__']`
			// is `Object.prototype`, which is truthy, so the next line wrote through
			// it onto the prototype of every object on the page. An imported
			// settings file is the way such a key reaches storage, and the parser
			// now refuses it too — this is the second half of that guard, and the
			// one that also covers a key already sitting in a profile's storage.
			if (!Object.hasOwn(module.options, key)) continue;
...
function prune(module, storedOptions) {
	...
	for (const [key, { value }] of Object.entries(storedOptions)) {
		...
		if (!module.options[key]) {
			...
			storage.deletePath(module.moduleID, key);
			continue;
		}

...
export type SettingsSnapshot = {|
	...
	// Names refused on the way in, as `moduleID` or `moduleID.optionKey`, so the
	// import can say what it ignored. Silently dropping them would leave the
	// reader believing a file imported whole when part of it was refused. Always
	// empty for a snapshot this build created; only an imported file can carry
	// one.
	droppedKeys: string[],
|};
...
// The same names are just as dangerous one level down, as option keys inside a
// module's blob. `_loadModuleOptions` walks those with `Object.entries` and did
// `if (!module.options[key]) continue;`, and `module.options['__proto__']` is
// `Object.prototype` — truthy — so the next line wrote through it onto the
// prototype of every object in the page. `JSON.parse` produces an *own*
// enumerable `__proto__` key, so this arrives intact from a file.
function sanitizeOptionBlob(blob: ModuleOptionsBlob): {| blob: ModuleOptionsBlob, dropped: string[] |} {
	const out: ModuleOptionsBlob = {};
	const dropped = [];
	for (const [key, value] of Object.entries(blob)) {
		if (RESERVED_MODULE_IDS.has(key)) { dropped.push(key); continue; }
		out[key] = value;
	}
	return { blob: out, dropped };
}
...
export function parseSnapshot(input: mixed): SettingsSnapshot {
	...
	for (const [moduleID, blob] of Object.entries((modules: any))) {
		if (!isPlainObject(blob) || RESERVED_MODULE_IDS.has(moduleID)) {
			if (RESERVED_MODULE_IDS.has(moduleID)) droppedKeys.push(moduleID);
			continue;
		}
		const cleaned = sanitizeOptionBlob((blob: any));
		for (const key of cleaned.dropped) droppedKeys.push(`${moduleID}.${key}`);
		sanitized[moduleID] = (cleaned.blob: any);
	}
...
export function describeSnapshotOrigin(snapshot: SettingsSnapshot, currentAppVersion: string): ?string {
	...
	// Named rather than counted: a reader who wrote the file wants to know which
	// line was refused, and a reader who did not wants to know something in it
	// tried to use a reserved name.
	//
	// Defaulted, because a restore point written by a build before this field
	// existed is deserialised straight back into this shape.
	const dropped = snapshot.droppedKeys || [];
	if (dropped.length) {
		parts.push(`ignored ${dropped.length} reserved name${dropped.length === 1 ? '' : 's'}: ${dropped.join(', ')}`);
	}

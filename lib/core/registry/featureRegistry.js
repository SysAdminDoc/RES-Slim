...
export function createFeatureRegistry(services = {}) {
	...
	function reportError(feature, stage, error) {
		...
		if (services.errorLog && services.errorLog.record) {
			services.errorLog.record(error, { featureId: feature.id, stage });
		}
		const toast = services.toast && (services.toast.showToast || services.toast);

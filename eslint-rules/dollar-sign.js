 ...
 */
// Vendored from eslint-plugin-dollar-sign@1.0.2. Logic unchanged; wrapper
// modernized for ESLint 10. See index.js in this directory.
'use strict';

export default {
	// `fixable` is mandatory in the modern format for a rule that reports a fix;
	// without it ESLint throws rather than silently dropping the fix.
	meta: {
		type: 'suggestion',
		fixable: 'code',
		schema: [{ enum: ['ignoreProperties'] }],
	},
	create,
};
...
function create(context) {
	...
	return {
		...
		'Program:exit': function(node) {
			// was `context.getScope()`, removed in ESLint 9. At `Program:exit` the
			// current scope is the global scope, which is what `getScope(node)`
			// returns for the Program node.
			checkVarDeclarations(sourceCode.getScope(node));
		}
	};
}

...
![Version](https://img.shields.io/badge/version-0.57.0-blue) ![License](https://img.shields.io/badge/license-GPL--3.0-green) ![Platform](https://img.shields.io/badge/platform-JavaScript-lightgrey)
...
**Media tweaks**: `showImages` inline expando engine plus all 71 host handlers (imgur, youtube, reddit-native, Mastodon, Threads, etc.).

**Appearance**: `pageTheme` defaults to **Classic Reddit**, a measured white-and-blue recreation of the old desktop interface, and on current Reddit it rebuilds the page to match. The left rail goes. The right rail becomes a 300px sidebar of flat 12px panels. Listing rows are 72px with a 43px vote column and a 70px thumbnail, and each one reads the way it used to: a 16px Verdana title with the `(domain)` after it, a `submitted 3 hr. ago by author` line under that, then bold grey `131 comments share` links. The vote column paints old Reddit's arrow glyphs, grey until you vote and orange or periwinkle after, with the score between them. Posts with no preview get the old link and document placeholders. The header keeps Reddit's wordmark and puts a plain search box and the pale `want to join? sign up or log in` corner beside it. The community strip continues the header's blue with a flat Create Post button and a small blue Join pill. Community highlights become one line of links. On a thread the post is the same row with its media below, and comments nest 18px per level with the `[-]` collapse marker. Native voting, commenting, sharing, collapsing, streaming and routing all stay Reddit's own: the controls never leave their shadow roots. A few small stylesheets are placed inside those roots to move and repaint them, and Shadow Parts carry the palette colours across the boundary. The **layout is not tied to the palette**. The same geometry applies on all eleven palettes, so OLED, Graphite, Midnight, Catppuccin, Tokyo Night, Rosé Pine, Nord, Dracula, Gruvbox and Solarized give you the old shape in dark colours on both renderers, and every palette sets `color-scheme` so scrollbars and form controls follow. Accent correction works in both directions, so a custom colour stays readable on light and dark surfaces.

**Current Reddit compatibility**: a Web3X/Shreddit adapter translates live post and comment components into the semantic vocabulary used by RES-Slim. Theme controls and the high-value DOM features work across both interfaces, including filtering, promoted-post removal, absolute timestamps, clean outbound links, author context, role highlights, user tags, vote history, layout controls, and scroll restore. The adapter follows streamed posts, nested comments, native comment collapse, SPA navigation, and native vote/action controls. On discussions it maps Reddit's live `details` header, composer, sort/search toolbar, action roots, and nested rail to the old Reddit layout. Each reply adds an 18px indent, the `[-]` marker stays aligned, and native actions keep working without their rounded wrappers. It also follows Reddit's nested `reddit-header-action-items` header and the current `rpl-action-bar` vote structure. Stable controls are exposed as CSS parts for shared paint rules. The smaller per-post stylesheet is gated by the refined-layout toggle rather than by the palette, so the left vote rail appears under every theme.
...
Saved-content sync and tag edits still work for the current private tab, but the
temporary index is discarded when it closes. IndexedDB stays unopened for these
features in private windows.
...
**Settings console**: three-column desktop command center with a persistent category rail, focused module rail, editorial settings workspace, explicit On/Off labels, staged-change controls, theme/density/motion controls, page-specific privacy and permission states, and portable data actions. The theme picker opens on **Match system**, which follows your desktop's light or dark setting and switches live when you change it. Pick one of the nine themes instead and it stays picked. Global search and Console preferences use the full workspace instead of retaining an unrelated empty module rail.

Console preferences carries a **This page** panel: what the extension did to the page you opened the settings from. Which filter hid which post and by which rule, which promoted records were removed, which expandos were built or refused, and which hosts were suspended after repeated failures. It answers the question a filter cannot answer for itself, which is whether you can trust it. The list is kept in memory for that tab only, is never written anywhere, and is dropped when you navigate. It holds decisions, not the posts they were about.

Console preferences also carries a support report. Press Build report and you get your version, your browser, the modules whose enablement or options you have changed, the slowest modules by stage, recent module failures and any selector drift, as plain text to paste into a bug report. It is built only when you press the button and never stored. Anything that could carry something private is described rather than printed, so a subreddit list reports its count and a text field its length. That applies to the **This page** panel too: the report carries how many times each module hid, dimmed, removed or refused something, and across how many rules or hosts, but none of the post ids or hostnames the panel shows you. A post id is not anonymous, and a report is something you paste in public. Module timings are measured on the reddit page, so open the console from one if you want them included.

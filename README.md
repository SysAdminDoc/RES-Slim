...
Console preferences carries a **This page** panel: what the extension did to the page you opened the settings from. Which filter hid which post and by which rule, which promoted records were removed, which expandos were built or refused, and which hosts were suspended after repeated failures. It answers the question a filter cannot answer for itself, which is whether you can trust it. The list is kept in memory for that tab only and is never written anywhere; it holds decisions, not the posts they were about.

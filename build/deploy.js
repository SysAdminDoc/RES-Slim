...
function deployChromeBeta() {
	...
	loadOptionalDeployer('chrome-extension-deploy')
		.then(chromeDeploy => chromeDeploy({
...
function deployFirefoxStable() {
	...
	loadOptionalDeployer('firefox-extension-deploy')
		.then(firefoxDeploy => firefoxDeploy({
...
async function loadOptionalDeployer(packageName) {
	try {
		...
		throw new Error(
			`Missing optional deployment dependency "${packageName}". Install deploy-only dependencies before running this script.`,
			{ cause: error },
		);
	}
}
